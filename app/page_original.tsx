    ...
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden' }}>
      ...
      <aside style={{ width: 300, height: '100vh', overflowY: 'auto', padding: 16, display: 'flex', flexDirection: 'column', gap: 14, borderRight: '1px solid var(--border)', background: 'var(--surface)', flexShrink: 0 }}>
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 2 }}>
            ...
            <span style={{ fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 12, letterSpacing: '0.05em' }}>
              ...
              <span style={{ color: '#16220F' }}>AMPO</span>
            </span>
          </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <span className={backendOk ? 'pulse' : ''} style={{ width: 7, height: 7, borderRadius: '50%', display: 'inline-block', background: backendOk === null ? '#3F5233' : backendOk ? 'var(--green)' : 'var(--red)' }}/>
        ...
        <section>
          ...
          <button onClick={() => { setMododibujo(m => !m); setSeleccionando(false) }} style={{ width: '100%', marginTop: 6, padding: '9px', borderRadius: 8, background: mododibujo ? 'rgba(15, 74, 112,0.15)' : 'var(--surface2)', border: `1px solid ${mododibujo ? 'var(--blue)' : 'var(--border)'}`, color: mododibujo ? 'var(--blue)' : 'var(--text)', fontFamily: 'var(--mono)', fontSize: 11, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, transition: 'all 0.2s' }}>
            {mododibujo ? '✕ CANCELAR DIBUJO' : '✏ DIBUJAR PARCELA A MANO'}
          </button>
          {mododibujo && <div style={{ marginTop: 6, padding: '8px 10px', borderRadius: 6, background: 'rgba(15, 74, 112,0.06)', border: '1px solid rgba(15, 74, 112,0.2)', fontSize: 10, color: 'var(--blue)', fontFamily: 'var(--mono)', lineHeight: 1.6 }}>✏ Haz clic en el mapa para dibujar el contorno.<br/>Doble clic para cerrar el polígono.</div>}

          {parcGeojson && (
            <button onClick={deseleccionar} style={{ width: '100%', marginTop: 6, padding: '7px', borderRadius: 6, background: 'transparent', border: '1px solid rgba(110, 36, 23,0.3)', color: '#6E2417', fontFamily: 'var(--mono)', fontSize: 10, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5 }}>
            ...
            <section>
              ...
              {productos.length > 0 && (
                <div style={{ marginTop: 8 }}>
                  ...
                  {/* Ver imagen real */}
                  <button onClick={verImagenRGB} disabled={cargando} style={{ width: '100%', marginTop: 8, padding: '9px', borderRadius: 6, background: modoVista === 'rgb' ? 'rgba(92, 63, 8,0.15)' : 'var(--surface2)', border: `1px solid ${modoVista === 'rgb' ? 'var(--amber)' : 'var(--border)'}`, color: modoVista === 'rgb' ? 'var(--amber)' : 'var(--text)', fontFamily: 'var(--mono)', fontSize: 11, fontWeight: 600, cursor: cargando ? 'wait' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, transition: 'all 0.2s' }}>
            ...
            <section>
              ...
              {Object.keys(kgHaCalculado).length > 0 && (
                <div style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '8px 10px', marginBottom: 8 }}>
                  ...
                  <button
                    onClick={() => setKgPorHa({})}
                    style={{ marginTop: 6, fontSize: 9, color: '#6E2417', fontFamily: 'var(--mono)', background: 'transparent', border: 'none', cursor: 'pointer', textDecoration: 'underline' }}
                  >
                    Borrar valores
                  </button>
                </div>
        ...
        {parcGeojson && (
          <>
            ...
            <section>
              ...
              <button onClick={calcularFlujo} disabled={calculandoFlujo} style={{ width: '100%', padding: '10px', borderRadius: 8, background: calculandoFlujo ? 'var(--surface2)' : 'rgba(15, 74, 112,0.15)', border: '1px solid var(--blue)', color: 'var(--blue)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 12, cursor: calculandoFlujo ? 'wait' : 'pointer', letterSpacing: '0.06em', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, transition: 'all 0.2s' }}>
              ...
              {flujoStats && (
                <div style={{ marginTop: 10 }}>
                  ...
                  <button
                    ...
                    style={{ width: '100%', marginTop: 8, padding: '8px', borderRadius: 6, background: mostrarFlujo ? 'rgba(15, 74, 112,0.15)' : 'var(--surface2)', border: `1px solid ${mostrarFlujo ? 'var(--blue)' : 'var(--border)'}`, color: mostrarFlujo ? 'var(--blue)' : 'var(--muted)', fontFamily: 'var(--mono)', fontSize: 10, cursor: 'pointer', fontWeight: 600 }}
                  >
          ...
          {mostrarHistorico && (
            <div style={{ marginTop: 8 }}>
              {historico.length === 0 ? (
                ...
                <>
                  <button onClick={() => { if (confirm('¿Borrar todo el histórico?')) { setHistorico([]); localStorage.removeItem('sigpac_historico') } }} style={{ fontSize: 9, color: '#6E2417', fontFamily: 'var(--mono)', background: 'transparent', border: 'none', cursor: 'pointer', marginBottom: 8, textDecoration: 'underline' }}>
                    Borrar histórico
                  </button>
                    ...
                    <div key={entrada.id} style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '8px 10px', marginBottom: 6 }}>
                      ...
                      {entrada.geojson && (
                        <button
                          ...
                          style={{ width: '100%', padding: '5px', borderRadius: 5, background: 'rgba(31, 74, 40,0.08)', border: '1px solid rgba(31, 74, 40,0.2)', color: 'var(--green)', fontFamily: 'var(--mono)', fontSize: 10, cursor: 'pointer', letterSpacing: '0.04em' }}
                        >
                          ▶ CARGAR ANÁLISIS
                        </button>
                      )}
      ...
      <div style={{ flex: 1, position: 'relative' }}>
        <MapView
          ...
          imagenUrl={imagenUrl}
          indiceColor={modoVista === 'rgb' ? '#5C3F08' : '#1F4A28'}
