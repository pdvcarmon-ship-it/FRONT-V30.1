...
export default function Home() {
  ...
  // Filtros de la pestaña COSECHAS (vista global, todas las parcelas)
  // Filtros de la pestaña COSECHAS (vista global): igual cascada Campaña → Finca → Parcela
  // que en el formulario de "+ NUEVA TAREA". Las parcelas no se listan hasta tener campaña y
  // finca elegidas; si no se marca ninguna parcela en concreto, se ven todas las de la finca.
  const [cosTabCampanaId, setCosTabCampanaId] = useState('')
  const [cosTabFincaId, setCosTabFincaId] = useState('')
  const [cosTabParcelaIds, setCosTabParcelaIds] = useState<string[]>([])
  // Formulario "+ NUEVA COSECHA" (también usado para editar)
  ...
  const [cosCultivo, setCosCultivo] = useState('')
  const [cosVariedad, setCosVariedad] = useState('')
  ...
  const abrirNuevaCosecha = (p?: ParcelaGuardada) => {
    ...
    setCosCultivo(p?.cultivo || '')
    setCosVariedad(p?.variedad || '')
  ...
  // Atajo desde la pestaña COSECHAS: si hay exactamente una parcela marcada en la cascada de
  // filtros, se usa para precargar el formulario; si no, al menos se precarga la campaña elegida.
  const abrirNuevaCosechaConContexto = (campanaId: string, parcelaIds: string[]) => {
    const parcela = parcelaIds.length === 1 ? misParcelas.find(p => p.id === parcelaIds[0]) : undefined
    abrirNuevaCosecha(parcela)
    if (!parcela && campanaId) setCosCampanaId(campanaId)
  }

  // Al elegir/cambiar la parcela en el formulario (sobre todo útil desde la pestaña
  // COSECHAS, donde no hay una parcela ya fijada), se rellenan cultivo/campaña por
  // comodidad — pero solo si se está creando una cosecha nueva, no al editar.
  const seleccionarParcelaEnFormCosecha = (parcelaId: string) => {
    setCosParcelaId(parcelaId)
    if (!cosEditar) {
      const p = misParcelas.find(x => x.id === parcelaId)
      if (p) {
        setCosCultivo(p.cultivo)
        setCosVariedad(p.variedad || '')
        if (!cosCampanaId) setCosCampanaId(p.campanaId || '')
      }
    }
  }
  ...
  const abrirEditarCosecha = (c: Cosecha) => {
    ...
    setCosCultivo(c.cultivo)
    setCosVariedad(c.variedad || '')
  ...
  const guardarCosecha = async () => {
    ...
    if (!cosCultivo.trim()) { setCosError('El cultivo es obligatorio'); return }
    ...
    const nueva: Cosecha = {
      ...
      parcelaId: cosParcelaId, campanaId: cosCampanaId, cultivo: cosCultivo.trim(),
      variedad: cosVariedad.trim() || undefined, fechaCosecha: cosFecha,
  ...
  const guardarComercializacion = async () => {
    if (!comFormCosechaId || !session) return
    ...
    // La cantidad total comercializada nunca debería superar la cosechada, cuando esta se conoce.
    if (cosecha?.cantidad) {
      const yaComercializado = totalComercializado(comFormCosechaId) - (comEditarId ? Number(comercializaciones.find(c => c.id === comEditarId)?.cantidad || 0) : 0)
    ...
    const nueva: Comercializacion = {
      id: comEditarId || String(Date.now()),
      ...
      fechaRegistro: comEditarId ? (comercializaciones.find(c => c.id === comEditarId)?.fechaRegistro || new Date().toLocaleDateString('es-ES')) : new Date().toLocaleDateString('es-ES'),
    }
    setComercializaciones(comEditarId ? comercializaciones.map(c => c.id === comEditarId ? nueva : c) : [nueva, ...comercializaciones])
    try {
      await supabase.from('comercializaciones').upsert({
                                      ...
                                      {comFormCosechaId === c.id && misPermisos['parcelas.crear_editar'] ? (
                                        <div style={{ background: 'var(--surface)', border: '1px solid var(--blue)', borderRadius: 6, padding: 8, display: 'flex', flexDirection: 'column', gap: 6 }}>
                                          ...
                                          <div style={{ display: 'flex', gap: 6 }}>
                                            ...
                                            <button onClick={() => { setComFormCosechaId(null); setComEditarId(null) }} style={{ padding: '6px 10px', borderRadius: 5, background: 'transparent', border: '1px solid var(--border)', color: 'var(--muted)', fontFamily: 'var(--mono)', fontSize: 10, cursor: 'pointer' }}>Cancelar</button>
                                          </div>
                                        </div>
                                      ) : misPermisos['parcelas.crear_editar'] ? (
...
{/* - Pestaña EQUIPOS - */}
        ...
        {pestana === 'cosechas' && (() => {
          // Cultivo de una parcela para una campaña concreta: el actual si coincide con esa
          // campaña (o la parcela no tiene campaña asignada), si no el histórico guardado para
          // esa campaña, y si tampoco hay histórico, el actual. (Igual que en "+ NUEVA TAREA".)
          const cultivoParcelaEnCampanaCos = (p: ParcelaGuardada, campanaId: string): string => {
            if (!campanaId || !p.campanaId || p.campanaId === campanaId) return p.cultivo
            const hist = historicoCultivos.find(h => h.parcelaId === p.id && h.campanaId === campanaId)
            return hist ? hist.cultivo : p.cultivo
          }
          const parcelasFinca = misParcelas.filter(p => p.fincaId === cosTabFincaId)
          const idsVisibles = new Set((cosTabParcelaIds.length > 0 ? parcelasFinca.filter(p => cosTabParcelaIds.includes(p.id)) : parcelasFinca).map(p => p.id))
          const listo = !!cosTabCampanaId && !!cosTabFincaId
          const cosechasTab = !listo ? [] : cosechas
            .filter(c => c.campanaId === cosTabCampanaId && idsVisibles.has(c.parcelaId))
            .sort((a, b) => (b.fechaCosecha || '').localeCompare(a.fechaCosecha || ''))
          ...
          return (
            <div style={{ flex: 1, overflow: 'auto', padding: isMobile ? 12 : 24 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10, marginBottom: 16 }}>
                ...
                {misPermisos['parcelas.crear_editar'] && (
                  <button onClick={() => abrirNuevaCosechaConContexto(cosTabCampanaId, cosTabParcelaIds)} style={{ padding: '7px 14px', borderRadius: 6, background: 'rgba(61,220,110,0.1)', border: '1px solid var(--green)', color: 'var(--green)', fontFamily: 'var(--mono)', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>+ NUEVA COSECHA</button>
                )}
              </div>

              <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: 10, marginBottom: 14 }}>
                <div>
                  <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase' }}>1. Campaña</label>
                  <select value={cosTabCampanaId} onChange={e => { setCosTabCampanaId(e.target.value); setCosTabParcelaIds([]) }}
                    style={{ width: '100%', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '9px 12px', color: 'var(--text)', fontSize: 13, fontFamily: 'var(--sans)', outline: 'none' }}>
                    <option value="">Selecciona una campaña</option>
                    {campanas.map(c => <option key={c.id} value={c.id}>{c.nombre}</option>)}
                  </select>
                ...
                <div>
                  <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase' }}>2. Finca</label>
                  <select value={cosTabFincaId} onChange={e => { setCosTabFincaId(e.target.value); setCosTabParcelaIds([]) }}
                    style={{ width: '100%', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '9px 12px', color: 'var(--text)', fontSize: 13, fontFamily: 'var(--sans)', outline: 'none' }}>
                    <option value="">Selecciona una finca</option>
                    {fincas.map(f => <option key={f.id} value={f.id}>{f.nombre}</option>)}
                  </select>
              ...
              <div style={{ marginBottom: 20 }}>
                <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase' }}>3. Parcela/s</label>
                {!listo ? (
                  <div style={{ fontSize: 11, color: 'var(--muted)', fontFamily: 'var(--mono)', padding: '8px 10px', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6 }}>
                    Selecciona campaña y finca para ver las parcelas disponibles.
                  </div>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, maxHeight: 120, overflow: 'auto', padding: 8, background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6 }}>
                    {parcelasFinca.map(p => {
                      const sel = cosTabParcelaIds.includes(p.id)
                      const cultivo = cultivoParcelaEnCampanaCos(p, cosTabCampanaId)
                      return (
                        <button key={p.id} onClick={() => setCosTabParcelaIds(sel ? cosTabParcelaIds.filter(id => id !== p.id) : [...cosTabParcelaIds, p.id])}
                          style={{ padding: '5px 10px', borderRadius: 14, background: sel ? 'rgba(61,220,110,0.15)' : 'var(--surface)', border: `1px solid ${sel ? 'var(--green)' : 'var(--border)'}`, color: sel ? 'var(--green)' : 'var(--muted)', fontFamily: 'var(--mono)', fontSize: 10, cursor: 'pointer' }}>
                          {p.nombre ? `${p.nombre} (${cultivo})` : cultivo}
                        </button>
                      )
                    })}
                    {parcelasFinca.length === 0 && <span style={{ fontSize: 10, color: 'var(--muted)', fontFamily: 'var(--mono)' }}>No hay parcelas guardadas en esta finca</span>}
                  </div>
                )}
              </div>

              {!listo ? (
                <div style={{ fontSize: 12, color: 'var(--muted)', fontFamily: 'var(--mono)', padding: '20px 0' }}>Selecciona una campaña y una finca para ver sus cosechas.</div>
              ) : cosechasTab.length === 0 ? (
                <div style={{ fontSize: 12, color: 'var(--muted)', fontFamily: 'var(--mono)', padding: '20px 0' }}>No hay cosechas registradas para esta selección.</div>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {cosechasTab.map(c => {
                    ...
                    const comercializacionesCosecha = comercializaciones.filter(x => x.cosechaId === c.id).sort((a, b) => (b.fecha || '').localeCompare(a.fecha || ''))
                    return (
                      <div key={c.id} style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 8, overflow: 'hidden' }}>
                        <div onClick={() => setCosechaExpandidaId(expandida ? null : c.id)} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '11px 14px', cursor: 'pointer' }}>
                          <div>
                            ...
                            <div style={{ fontSize: 11, fontFamily: 'var(--mono)', color: 'var(--muted)', marginTop: 3 }}>
                              📁 {p?.nombre || p?.cultivo || 'Parcela eliminada'} · {campanas.find(camp => camp.id === c.campanaId)?.nombre || 'Sin campaña'} · {formatFechaEs(c.fechaCosecha)}
                            </div>
                          </div>
                          <span style={{ color: 'var(--muted)', fontSize: 12 }}>{expandida ? '▲' : '▼'}</span>
                        </div>
          ...
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 2500, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
            <div style={{ background: 'var(--surface)', border: '1px solid var(--green)', borderRadius: 12, width: '100%', maxWidth: 420, padding: 24, maxHeight: '90vh', overflowY: 'auto' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                <div>
                  <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase' }}>Parcela <span style={{ color: '#ff6b6b' }}>*</span></label>
                  <select value={cosParcelaId} onChange={e => seleccionarParcelaEnFormCosecha(e.target.value)}
                    style={{ width: '100%', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '9px 12px', color: 'var(--text)', fontSize: 13, fontFamily: 'var(--sans)', outline: 'none' }}>
                    <option value="">Selecciona una parcela</option>
                    {misParcelas.map(p => <option key={p.id} value={p.id}>{p.nombre || formatCultivoVariedad(p.cultivo, p.variedad)}</option>)}
                  </select>
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase' }}>Campaña <span style={{ color: '#ff6b6b' }}>*</span></label>
                  <select value={cosCampanaId} onChange={e => setCosCampanaId(e.target.value)}
                    style={{ width: '100%', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '9px 12px', color: 'var(--text)', fontSize: 13, fontFamily: 'var(--sans)', outline: 'none' }}>
                    <option value="">Selecciona una campaña</option>
                    {campanas.map(c => <option key={c.id} value={c.id}>{c.nombre}</option>)}
                  </select>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                  <div>
                    <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase' }}>Cultivo <span style={{ color: '#ff6b6b' }}>*</span></label>
                    <input value={cosCultivo} onChange={e => setCosCultivo(e.target.value)} placeholder="Cultivo"
                      style={{ width: '100%', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '9px 12px', color: 'var(--text)', fontSize: 13, fontFamily: 'var(--sans)', outline: 'none' }} />
                  </div>
                  <div>
                    <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase' }}>Variedad</label>
                    <input value={cosVariedad} onChange={e => setCosVariedad(e.target.value)} placeholder="Opcional"
                      style={{ width: '100%', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '9px 12px', color: 'var(--text)', fontSize: 13, fontFamily: 'var(--sans)', outline: 'none' }} />
                  </div>
                </div>
                <div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 10 }}>
                  <div>
                    <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase' }}>Materia cosechada</label>
                ...
                <div>
                  ...
                  <textarea value={cosObservaciones} onChange={e => setCosObservaciones(e.target.value)} rows={2} placeholder="Notas adicionales..."
                    style={{ width: '100%', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '9px 12px', color: 'var(--text)', fontSize: 13, fontFamily: 'var(--sans)', outline: 'none', resize: 'vertical' }} />
                </div>
