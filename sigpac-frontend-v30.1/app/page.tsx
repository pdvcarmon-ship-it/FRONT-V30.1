...
export default function Home() {
  ...
  const misParcelasVisibles = useMemo(() => {
    ...
    if (fincasAsignadas.length === 0) return misParcelas
    return misParcelas.filter(p => p.fincaId && fincasAsignadas.includes(p.fincaId))
  }, [misParcelas, personal, session])
  ...
  const guardarTareaGeneral = async () => {
    if (!misPermisos['tareas.crear_asignar']) return
    if (!session) return
  ...
  const marcarTareaRealizada = async (t: Tarea) => {
    ...
    if (t.tipo === 'Tratamiento fitosanitario') {
      if (!misPermisos['tratamientos.crear_editar']) { alert('No tienes permiso para completar tratamientos fitosanitarios.'); return }
...
{/* - Pestaña EQUIPOS - */}
        ...
        {pestana === 'permisos' && (
          <PermisosTab session={session} personal={personal} setPersonal={setPersonal} isMobile={isMobile} fincas={fincas} />
        )}
