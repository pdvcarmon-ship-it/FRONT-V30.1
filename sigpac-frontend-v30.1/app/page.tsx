...
type Pestaña = 'mapa' | 'mis_parcelas' | 'tratamientos' | 'personal' | 'equipos' | 'riego' | 'tareas'
...
export default function Home() {
  ...
  const guardarTratamiento = async () => {
    ...
    if (tRealizada && campanaElegida && tFecha < campanaElegida.fechaInicio) {
      setTError(`La fecha del tratamiento (${tFecha}) es anterior al inicio de la campaña "${campanaElegida.nombre}" (${campanaElegida.fechaInicio}). Elige una fecha dentro de la campaña, o selecciona otra.`)
      return
    }
    if (tRealizada && campanaElegida && tFecha > campanaElegida.fechaFin) {
    ...
    if (!tRealizada) {
      ...
      const nuevaTarea: Tarea = {
        ...
        fechaRegistro: tareas.find(x => x.id === tareaEnCreacionId)?.fechaRegistro || new Date().toLocaleDateString('es-ES'),
      }
      setTareas(tareaEnCreacionId ? tareas.map(t => t.id === nuevaTarea.id ? nuevaTarea : t) : [nuevaTarea, ...tareas])
      try {
        await supabase.from('tareas').upsert({
          ...
          borrador_tratamiento: nuevaTarea.borradorTratamiento, fecha_registro: nuevaTarea.fechaRegistro,
        })
      } catch (e) { console.error('Error guardando tarea (borrador de tratamiento):', e) }
    ...
    const nuevo: Tratamiento = {
      ...
      ...datosComunes,
      fechaRegistro: tratEditar?.fechaRegistro || new Date().toLocaleDateString('es-ES'),
    ...
    // Si venía de una tarea (pendiente que ahora se completa), se marca como realizada y se enlaza
  ...
  // Convierte directamente el borrador en un tratamiento realizado, sin reabrir el formulario
  const marcarTareaRealizada = async (t: Tarea) => {
    if (!session) return
    ...
    const actualizada: Tarea = { ...t, estado: 'realizada' }
    setTareas(tareas.map(x => x.id === t.id ? actualizada : x))
    try { await supabase.from('tareas').update({ estado: 'realizada' }).eq('id', t.id) } catch (e) { console.error(e) }
  }
        ...
        <div style={{ display: 'flex', alignItems: 'center', minHeight: isMobile ? 54 : 48 }}>
            ...
            <div data-menu style={{ position: 'relative', flexShrink: 0 }}>
              ...
              {menuAbierto && (
                <div style={{ position: 'absolute', top: '100%', left: 0, zIndex: 6000, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '0 0 10px 10px', boxShadow: '0 8px 24px rgba(0,0,0,0.5)', minWidth: 220 }}>
                  {(['mapa', 'mis_parcelas', 'tratamientos', 'tareas', 'riego', 'personal', 'equipos'] as Pestaña[]).map(tab => (
                    ...
                    >
                      {tab === 'mapa' ? '🗺  MAPA' : tab === 'mis_parcelas' ? '📁  MIS PARCELAS' : tab === 'tratamientos' ? '🧪  TRATAMIENTOS' : tab === 'tareas' ? '📋  TAREAS' : tab === 'riego' ? '💧  RIEGO' : tab === 'personal' ? '👷  PERSONAL' : '🚜  EQUIPOS'}
                    </button>
                  ))}
                </div>
              )}
          ...
          {isMobile ? (
            <span style={{ flex: 1, fontFamily: 'var(--mono)', fontSize: 13, fontWeight: 700, color: 'var(--green)', paddingLeft: 4 }}>
              {pestana === 'mapa' ? '🗺 MAPA' : pestana === 'mis_parcelas' ? '📁 MIS PARCELAS' : pestana === 'tratamientos' ? '🧪 TRATAMIENTOS' : pestana === 'tareas' ? '📋 TAREAS' : pestana === 'riego' ? '💧 RIEGO' : pestana === 'personal' ? '👷 PERSONAL' : '🚜 EQUIPOS'}
            </span>
          ) : (
...
{/* - Pestaña EQUIPOS - */}
        ...
        {pestana === 'tareas' && (() => {
          ...
          return (
            <div style={{ display: 'flex', height: 'calc(100vh - 140px)', gap: isMobile ? 0 : 16, flexDirection: isMobile ? 'column' : 'row' }}>
              ...
              <div style={{ width: isMobile ? '100%' : '25%', minWidth: isMobile ? undefined : 260, display: 'flex', flexDirection: 'column', borderRight: isMobile ? 'none' : '1px solid var(--border)', paddingRight: isMobile ? 0 : 12 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                ...
                <div style={{ flex: 1, overflow: 'auto', display: 'flex', flexDirection: 'column', gap: 8 }}>
                  ...
                  {tareasFiltradas.map(t => {
                    ...
                    const parcelasStr = t.parcelaIds.map(id => misParcelas.find(p => p.id === id)?.nombre || misParcelas.find(p => p.id === id)?.cultivo).filter(Boolean).join(', ')
                    return (
                      <button key={t.id} onClick={() => setTareaSeleccionadaId(t.id)}
                        ...
                        <div style={{ fontFamily: 'var(--mono)', fontSize: 12, fontWeight: 700, color: 'var(--text)', marginBottom: 3 }}>
                          {t.estado === 'realizada' ? '🟢' : color === '#ff6b6b' ? '🔴' : '🟠'} {nombreTarea(t).toUpperCase()}
                        </div>
                        ...
                        <div style={{ fontSize: 10, color: 'var(--muted)', fontFamily: 'var(--mono)' }}>{nombrePersona(t.ejecutorId)}</div>
                      </button>
                    )
                  })}
                </div>
              ...
              <div style={{ flex: 1, overflow: 'auto', padding: isMobile ? '16px 0 0' : '0 4px' }}>
                {!tareaSel ? (
                  ...
                  const parcelasStr = tareaSel.parcelaIds.map(id => misParcelas.find(p => p.id === id)?.nombre || misParcelas.find(p => p.id === id)?.cultivo).filter(Boolean).join(', ') || '—'
                  ...
                  return (
                    <div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16, flexWrap: 'wrap', gap: 10 }}>
                        <div>
                          ...
                          <div style={{ fontFamily: 'var(--mono)', fontSize: 12, fontWeight: 700, color, marginTop: 4 }}>
                            {tareaSel.estado === 'realizada' ? '🟢 REALIZADA' : color === '#ff6b6b' ? '🔴 PENDIENTE' : '🟠 PENDIENTE'}
                          </div>
                        ...
                        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                          {tareaSel.estado !== 'realizada' && (
                            <button onClick={() => marcarTareaRealizada(tareaSel)} style={{ padding: '8px 14px', borderRadius: 6, background: 'var(--green)', border: 'none', color: 'var(--bg)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 11, cursor: 'pointer' }}>✓ MARCAR COMO REALIZADA</button>
                          )}
                      ...
                      {[
                        ...
                        { titulo: 'PERSONAL', filas: [['Quién crea', nombrePersona(tareaSel.creadoPor)], ['Quién ejecuta', nombrePersona(tareaSel.ejecutorId)]] },
                  ...
                  <div style={{ background: 'var(--surface)', border: '1px solid var(--green)', borderRadius: 12, width: '100%', maxWidth: 480, maxHeight: '90vh', overflow: 'auto' }}>
                    ...
                    <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 14 }}>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                        <div>
                          <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase' }}>Campaña</label>
                          <select value={tarCampanaId} onChange={e => setTarCampanaId(e.target.value)} style={{ width: '100%', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '9px 12px', color: 'var(--text)', fontSize: 13, fontFamily: 'var(--sans)', outline: 'none' }}>
                            <option value="">Sin campaña</option>
                            {campanas.map(c => <option key={c.id} value={c.id}>{c.nombre}</option>)}
                          </select>
                        </div>
                      ...
                      <div>
                        ...
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, maxHeight: 120, overflow: 'auto', padding: 8, background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6 }}>
                          {misParcelas.filter(p => !tarFincaId || p.fincaId === tarFincaId).map(p => {
        ...
        {pestana === 'riego' && (
          <RiegoTab session={session} fincas={fincas} misParcelas={misParcelas} isMobile={isMobile}
            sectorInicial={riegoSectorFocus} onConsumirSectorInicial={() => setRiegoSectorFocus(null)} />
        )}
