    ...
    try {
        ...
        if (comercializacionesRes.data) {
          setComercializaciones(comercializacionesRes.data.map((c: any) => ({
            ...
            unidad: c.unidad || 'kg', comprador: c.comprador || undefined,
            observaciones: c.observaciones || undefined, fechaRegistro: c.fecha_registro || '',
          })))
        }
  ...
  const guardarComercializacion = async () => {
    ...
    if (comEditarId) {
      ...
      const nueva: Comercializacion = {
        ...
        comprador: comComprador.trim() || undefined, observaciones: comObservaciones.trim() || undefined,
    ...
    if (comFormParteKey) {
      ...
      const nuevas: Comercializacion[] = reparto.map((r, i) => ({
        ...
        comprador: comComprador.trim() || undefined, observaciones: comObservaciones.trim() || undefined, fechaRegistro,
      }))
                                      ...
                                      <div style={{ display: 'flex', flexDirection: 'column', gap: 5, marginBottom: 8 }}>
                                        {comercializacionesCosecha.map(com => (
                                          <div key={com.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 5, padding: '6px 8px', fontSize: 10, fontFamily: 'var(--mono)' }}>
                                            <span style={{ color: 'var(--text)' }}>{formatFechaEs(com.fecha)} → {formatCantidadEs(Number(com.cantidad))} {com.unidad}{com.comprador ? ` → ${com.comprador}` : ''}</span>
