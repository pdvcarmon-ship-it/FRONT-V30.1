...
export default function Home() {
  ...
  const cancelarFormulario = () => {
    ...
    setParcelaEditar(null)
    setFormError('')
    // Deselect map parcela if it was a new parcela (not editing existing)
    if (!parcelaEditar) {
      setParcGeojson(null)
      setParcelaInfo(null)
      setParcelaSupHa(0)
  ...
  const handleMapClick = useCallback(async (lat: number, lon: number) => {
    ...
    if (isMobile) setPanelAbierto(false)  // Hide panel while loading
    setParcGeojson(null); setParcelaInfo(null); setProductos([])
    setParcelaSupHa(0); setParcelaVistaEnMapa(null); resetear()
    if (isMobile) setPanelAbierto(true)  // Auto-open panel on mobile when parcela found
    try {
        ...
        <div style={{ display: 'flex', alignItems: 'center', minHeight: isMobile ? 54 : 48 }}>
          ...
          {isMobile ? (
            ...
            <div style={{ display: 'flex', flex: 1 }}>
              {(['mapa', 'mis_parcelas', 'tratamientos', 'personal'] as Pestaña[]).map(tab => (
                <button key={tab} onClick={() => { setPestana(tab); if (tab === 'mapa') { setFormularioVisible(false); setParcelaEditar(null) } }}
            ...
            <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              {!tratParcelaCtx && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  {/* Paso 1: Cultivo */}
                  <div>
                    <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.08em' }}>1. Cultivo <span style={{ color: '#ff6b6b' }}>*</span></label>
                    <select value={tCultivoSeleccionado} onChange={e => { setTCultivoSeleccionado(e.target.value); setTFincaSeleccionada(''); setTParcelas([]) }}
                  ...
                  {tCultivoSeleccionado && fincas.length > 1 && (
                    <div>
                      <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.08em' }}>2. Finca <span style={{ color: '#ff6b6b' }}>*</span></label>
                      <select value={tFincaSeleccionada} onChange={e => { setTFincaSeleccionada(e.target.value); setTParcelas([]) }}
                  ...
                  {/* Paso 3: Parcelas filtradas */}
                  {tCultivoSeleccionado && (
                    <div>
                      <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.08em' }}>
                        {fincas.length > 1 ? '3.' : '2.'} Parcelas <span style={{ color: '#ff6b6b' }}>*</span>
