...
export default function Home() {
  ...
  // Igual que arriba pero genérico (para cualquier campaña, sin cultivo concreto ya elegido):
  // ¿esta parcela "existió" en esa campaña, ya sea porque es su campaña actual o porque hay un
  // cultivo histórico suyo registrado para esa campaña? Si no, no debe aparecer en absoluto.
  const parcelaExisteEnCampana = (p: ParcelaGuardada, campanaId: string): boolean => {
    if (!campanaId) return true
    if (p.campanaId === campanaId) return true
    return historicoCultivos.some(h => h.parcelaId === p.id && h.campanaId === campanaId)
  }

  // El cultivo que tenía esa parcela EN esa campaña concreta (actual si coincide, si no el histórico)
  const cultivoEnCampana = (p: ParcelaGuardada, campanaId: string): string => {
    if (!campanaId || p.campanaId === campanaId) return p.cultivo
                    ...
                    <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 14 }}>
                      ...
                      <div>
                        ...
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, maxHeight: 120, overflow: 'auto', padding: 8, background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6 }}>
                          {misParcelas
                            .filter(p => (!tarFincaId || p.fincaId === tarFincaId) && parcelaExisteEnCampana(p, tarCampanaId))
                            .map(p => {
                            const sel = tarParcelaIds.includes(p.id)
                            const cultivo = cultivoEnCampana(p, tarCampanaId)
                            return (
                          ...
                          {misParcelas.filter(p => (!tarFincaId || p.fincaId === tarFincaId) && parcelaExisteEnCampana(p, tarCampanaId)).length === 0 && (
                            <span style={{ fontSize: 10, color: 'var(--muted)', fontFamily: 'var(--mono)' }}>
                              {tarCampanaId ? 'Ninguna parcela existía en esta campaña' : 'No hay parcelas guardadas'}
                            </span>
                          )}
                        </div>
                      </div>
