...
const UbicacionMapModal = dynamic(() => import('./components/UbicacionMapModal'), { ssr: false })
...
type Pestaña = 'mapa' | 'mis_parcelas' | 'tratamientos' | 'personal' | 'equipos' | 'riego' | 'tareas' | 'cosechas' | 'permisos'
...
interface Tratamiento {
  ...
  conAsesor?: boolean              // Cultivo/superficie objeto de asesoramiento GIP (cuaderno de campo, sección 3.1 bis)
  justificacionActuacion?: string  // Justificación de la actuación (cuaderno de campo, sección 3.1 bis)
}

const TIPOS_TAREA = ['Tratamiento fitosanitario', 'Alternativa no química', 'Abonado', 'Poda', 'Laboreo', 'Siembra', 'Recolección', 'Riego', 'Mantenimiento', 'Otros'] as const
...
interface Tarea {
  ...
  horasPersonalId?: string        // Trabajador al que se le imputan esas horas
  // Campos específicos de las tareas de tipo "Alternativa no química" — se usan para rellenar
  // el cuaderno de campo, sección 3.1 bis (solamente cultivos/superficies con asesoramiento GIP)
  plaga?: string
  justificacionActuacion?: string
  tipoMedida?: string
  intensidadMedida?: string
...
const colorPrioridadTarea = (t: Tarea): '#1F4A28' | '#6E2417' | '#8A4B12' => {
  if (t.estado === 'realizada') return '#1F4A28'
  return diasHastaTarea(t.fechaPrevista) <= 5 ? '#6E2417' : '#8A4B12'
}
...
// - GEOMETRÍA: punto en polígono, distancia y coordenadas UTM -
// Usadas para rellenar automáticamente la sección 2.2 del Cuaderno de Campo (puntos de
// captación de agua) a partir de las Ubicaciones marcadas en el mapa.

// Ray casting simple sobre un anillo de coordenadas [lng, lat]
function rayCastingDentro(x: number, y: number, anillo: number[][]): boolean {
  let dentro = false
...
// ¿El punto (lat, lng) cae dentro de alguno de los polígonos del geojson de una parcela?
function puntoEnPoligono(lat: number, lng: number, geojson: any): boolean {
  ...
  for (const f of features) {
    ...
    for (const poli of poligonos) {
      if (poli[0] && rayCastingDentro(lng, lat, poli[0])) return true
    }
  }
  return false
}

// Distancia mínima (en metros) de un punto (lat, lng) al borde del/de los polígono/s de una parcela
function distanciaAPoligonoM(lat: number, lng: number, geojson: any): number {
  const mPerDegLat = 111320
  const mPerDegLon = 111320 * Math.cos((lat * Math.PI) / 180)
  const distPuntoSegmento = (px: number, py: number, x1: number, y1: number, x2: number, y2: number): number => {
  ...
  const x = lng * mPerDegLon, y = lat * mPerDegLat
  let min = Infinity
  const features = geojson?.features || []
  for (const f of features) {
    const geom = f?.geometry
    ...
    for (const poli of poligonos) {
      ...
      for (let i = 0; i < anillo.length - 1; i++) {
        const d = distPuntoSegmento(x, y, anillo[i][0] * mPerDegLon, anillo[i][1] * mPerDegLat, anillo[i + 1][0] * mPerDegLon, anillo[i + 1][1] * mPerDegLat)
        if (d < min) min = d
      }
    }
  }
...
// Conversión de lat/lng (WGS84) a coordenadas UTM — fórmulas estándar de Snyder, precisión de metros
function latLngToUTM(lat: number, lng: number): { zona: number; banda: string; x: number; y: number } {
  const a = 6378137.0
  ...
  const lngOrigen = (zona - 1) * 6 - 180 + 3
  const latRad = (lat * Math.PI) / 180
  const dLngRad = ((lng - lngOrigen) * Math.PI) / 180

  const N = a / Math.sqrt(1 - e2 * Math.sin(latRad) ** 2)
  const T = Math.tan(latRad) ** 2
  const C = ePrima2 * Math.cos(latRad) ** 2
  const A = Math.cos(latRad) * dLngRad
  ...
  const x = k0 * N * (A + ((1 - T + C) * A ** 3) / 6 + ((5 - 18 * T + T ** 2 + 72 * C - 58 * ePrima2) * A ** 5) / 120) + 500000
  let y = k0 * (M + N * Math.tan(latRad) * (A ** 2 / 2 + ((5 - T + 9 * C + 4 * C ** 2) * A ** 4) / 24 + ((61 - 58 * T + T ** 2 + 600 * C - 330 * ePrima2) * A ** 6) / 720))
  if (lat < 0) y += 10000000

  const bandas = 'CDEFGHJKLMNPQRSTUVWXX'
  const banda = bandas[Math.min(Math.max(Math.floor((lat + 80) / 8), 0), bandas.length - 1)] || ''
  return { zona, banda, x: Math.round(x), y: Math.round(y) }
}
...
// Un "punto de captación de agua" (pozo, canal, balsa...) asociado a una finca y a las parcelas
// que abastece — para rellenar la sección 2.2 del Cuaderno de Campo (RD 1311/2012).
interface Ubicacion {
  id: string
  fincaId: string
  nombre: string        // p.ej. "Pozo Norte", "Canal de riego"
  lat: number
  lng: number
  parcelaIds: string[]  // parcelas de esa finca a las que abastece/afecta esta ubicación
...
// Unidades admitidas para la cosecha — de momento solo se usa "kg" en la práctica, pero
// se deja preparado para poder añadir otras más adelante sin cambiar el modelo de datos.
const UNIDADES_COSECHA = ['kg', 't', 'l', 'arrobas', 'sacos', 'unidades'] as const

// Un registro de cosecha obtenido en una parcela, para una campaña y un cultivo concretos.
// La materia cosechada es opcional (puede no haberse pesado/registrado todavía). Puede
// comercializarse en uno o varios envíos — ver Comercializacion, más abajo.
interface Cosecha {
  id: string
  parcelaId: string
  campanaId: string
  ...
  variedad?: string
  fechaCosecha: string
  cantidad?: string   // materia cosechada; si no se ha registrado, se deja vacío (nunca "0")
  unidad: string        // 'kg' por defecto
  observaciones?: string
  fechaRegistro: string
}

// Una venta/entrega de (parte de) la materia de una Cosecha. Una misma cosecha puede tener
// varias comercializaciones — el pendiente se calcula como cantidad de la cosecha menos la
// suma de todas sus comercializaciones.
interface Comercializacion {
  id: string
  cosechaId: string
  fecha: string
  cantidad: string
  unidad: string
  comprador?: string
  compradorDniNif?: string
  compradorDireccion?: string
  compradorRgseaa?: string
  numAlbaranFactura?: string
  numLote?: string
  observaciones?: string
  fechaRegistro: string
  // Cuando una comercialización de "parte" se reparte entre varias cosechas (FIFO), todas las
  // filas resultantes comparten este id — permite volver a agruparlas como una única venta en
  // el cuaderno de campo (sección 5).
  ventaId?: string
}
...
export default function Home() {
  ...
  // Asesoramiento GIP (cuaderno de campo, sección 3.1 bis) — solo aplica a cultivos/superficies
  // objeto de asesoramiento; al marcarlo se piden los datos que exige esa sección.
  const [tConAsesor, setTConAsesor] = useState(false)
  const [tJustificacionActuacion, setTJustificacionActuacion] = useState('')
  ...
  // - Ubicaciones (pozos, canales... para el Cuaderno de Campo, sección 2.2) -
  const [ubicaciones, setUbicaciones] = useState<Ubicacion[]>([])
  const [ubicacionEditar, setUbicacionEditar] = useState<Ubicacion | null>(null)
  const [formUbicacionPaso, setFormUbicacionPaso] = useState<'' | 'finca' | 'mapa' | 'detalles'>('')
  const [ubicacionFincaId, setUbicacionFincaId] = useState('')
  const [ubicacionPunto, setUbicacionPunto] = useState<{ lat: number; lng: number } | null>(null)
  const [ubicacionNombre, setUbicacionNombre] = useState('')
  const [ubicacionParcelaIds, setUbicacionParcelaIds] = useState<string[]>([])
  ...
  // Campos específicos de "Alternativa no química" (cuaderno de campo, sección 3.1 bis)
  const [tarPlaga, setTarPlaga] = useState('')
  const [tarJustificacionActuacion, setTarJustificacionActuacion] = useState('')
  const [tarTipoMedida, setTarTipoMedida] = useState('')
  const [tarIntensidadMedida, setTarIntensidadMedida] = useState('')
  const [tarEficacia, setTarEficacia] = useState<'' | 'Buena' | 'Regular' | 'Mala'>('')
  ...
  // Registro de cosecha comercializada — Cosechas de cada parcela y sus comercializaciones.
  const [cosechas, setCosechas] = useState<Cosecha[]>([])
  const [comercializaciones, setComercializaciones] = useState<Comercializacion[]>([])
  const [cosechaExpandidaId, setCosechaExpandidaId] = useState<string | null>(null)
  // Filtros de la pestaña COSECHAS (vista global): Campaña / Finca / Cultivo, sobre la lista
  // de cosechas ya registradas (no afectan al formulario de alta, que tiene su propia cascada).
  const [filtroCampanaCosechasTab, setFiltroCampanaCosechasTab] = useState('')
  const [filtroFincaCosechasTab, setFiltroFincaCosechasTab] = useState('')
  const [filtroCultivoCosechasTab, setFiltroCultivoCosechasTab] = useState('')
  // Formulario "+ NUEVA COSECHA" (también usado para editar) — cascada Campaña → Finca →
  // Parcela, igual que en "+ NUEVA TAREA". El cultivo/variedad no se escriben a mano: se
  // derivan de la parcela elegida para esa campaña (actual o histórico).
  const [cosFormVisible, setCosFormVisible] = useState(false)
  const [cosEditar, setCosEditar] = useState<Cosecha | null>(null)
  const [cosCampanaId, setCosCampanaId] = useState('')
  const [cosFincaId, setCosFincaId] = useState('')
  const [cosParcelaId, setCosParcelaId] = useState('')
  const [cosFecha, setCosFecha] = useState('')
  const [cosCantidad, setCosCantidad] = useState('')
  const [cosUnidad, setCosUnidad] = useState<string>('kg')
  const [cosObservaciones, setCosObservaciones] = useState('')
  const [cosError, setCosError] = useState('')
  // Mini-formulario de comercialización, anidado dentro de la cosecha expandida
  const [comFormCosechaId, setComFormCosechaId] = useState<string | null>(null)
  // Alta de comercialización a nivel de "parte" (cultivo+campaña+finca agrupados) en la
  // pestaña COSECHAS: se reparte automáticamente entre las cosechas más antiguas (FIFO).
  const [comFormParteKey, setComFormParteKey] = useState<string | null>(null)
  const [comEditarId, setComEditarId] = useState<string | null>(null)
  const [comFecha, setComFecha] = useState('')
  const [comCantidad, setComCantidad] = useState('')
  const [comUnidad, setComUnidad] = useState<string>('kg')
  const [comComprador, setComComprador] = useState('')
  const [comCompradorDniNif, setComCompradorDniNif] = useState('')
  const [comCompradorDireccion, setComCompradorDireccion] = useState('')
  const [comCompradorRgseaa, setComCompradorRgseaa] = useState('')
  const [comNumAlbaranFactura, setComNumAlbaranFactura] = useState('')
  const [comNumLote, setComNumLote] = useState('')
  const [comObservaciones, setComObservaciones] = useState('')
  ...
  const [datosExplotacionPorFinca, setDatosExplotacionPorFinca] = useState<Record<string, DatosExplotacion>>({})
  ...
  const [deFincaIds, setDeFincaIds] = useState<string[]>([])   // fincas a las que se aplicará al guardar
    ...
    try {
        ...
        if (tRes.data) {
          setTratamientos(tRes.data.map((t: any) => ({
            ...
            conAsesor: t.con_asesor || false,
            justificacionActuacion: t.justificacion_actuacion || undefined,
          })))
        }
        ...
        const ubicacionesRes = await supabase.from('ubicaciones').select('*').order('created_at', { ascending: false })
        if (ubicacionesRes.error) console.error('Error cargando ubicaciones (¿existe la tabla "ubicaciones"?):', ubicacionesRes.error)
        if (ubicacionesRes.data) {
          setUbicaciones(ubicacionesRes.data.map((u: any) => ({
            id: u.id, fincaId: u.finca_id, nombre: u.nombre || '',
            lat: Number(u.lat), lng: Number(u.lng),
            parcelaIds: u.parcela_ids || [], fechaGuardado: u.fecha_guardado || '',
          })))
        }
        ...
        if (tareasRes.data) {
          setTareas(tareasRes.data.map((t: any) => ({
            ...
            horasTrabajadas: t.horas_trabajadas || undefined,
            horasPersonalId: t.horas_personal_id || undefined,
            plaga: t.plaga || undefined,
            justificacionActuacion: t.justificacion_actuacion || undefined,
            tipoMedida: t.tipo_medida || undefined,
            intensidadMedida: t.intensidad_medida || undefined,
            eficacia: t.eficacia || undefined,
          })))
        }
        ...
        const cosechasRes = await supabase.from('cosechas').select('*').order('fecha_cosecha', { ascending: false })
        if (cosechasRes.error) console.error('Error cargando cosechas (¿existe la tabla "cosechas"?):', cosechasRes.error)
        if (cosechasRes.data) {
          setCosechas(cosechasRes.data.map((c: any) => ({
            id: c.id, parcelaId: c.parcela_id, campanaId: c.campana_id, cultivo: c.cultivo,
            variedad: c.variedad || undefined, fechaCosecha: c.fecha_cosecha,
            cantidad: c.cantidad != null ? String(c.cantidad) : undefined, unidad: c.unidad || 'kg',
            observaciones: c.observaciones || undefined, fechaRegistro: c.fecha_registro || '',
          })))
        }

        const comercializacionesRes = await supabase.from('comercializaciones').select('*').order('fecha', { ascending: false })
        if (comercializacionesRes.error) console.error('Error cargando comercializaciones (¿existe la tabla "comercializaciones"?):', comercializacionesRes.error)
        if (comercializacionesRes.data) {
          setComercializaciones(comercializacionesRes.data.map((c: any) => ({
            id: c.id, cosechaId: c.cosecha_id, fecha: c.fecha, cantidad: String(c.cantidad),
            unidad: c.unidad || 'kg', comprador: c.comprador || undefined, compradorDniNif: c.comprador_dni_nif || undefined,
            compradorDireccion: c.comprador_direccion || undefined, compradorRgseaa: c.comprador_rgseaa || undefined,
            numAlbaranFactura: c.num_albaran_factura || undefined, numLote: c.num_lote || undefined,
            observaciones: c.observaciones || undefined, fechaRegistro: c.fecha_registro || '', ventaId: c.venta_id || undefined,
          })))
        }
        ...
        const explotRes = await supabase.from('datos_explotacion').select('*').eq('user_id', session.user.id)
        if (explotRes.error) console.error('Error cargando datos_explotacion (¿tiene columna "finca_id"?):', explotRes.error)
        if (explotRes.data) {
          const mapa: Record<string, DatosExplotacion> = {}
          explotRes.data.forEach((fila: any) => {
            if (fila.finca_id && fila.datos) mapa[fila.finca_id] = { ...DATOS_EXPLOTACION_VACIO, ...fila.datos }
          })
          setDatosExplotacionPorFinca(mapa)
        }
  ...
  // - Ubicaciones (pozos, canales... — sección 2.2 del Cuaderno de Campo) -
  const cerrarFormUbicacion = () => {
    setFormUbicacionPaso('')
    setUbicacionEditar(null)
    setUbicacionFincaId('')
    setUbicacionPunto(null)
    setUbicacionNombre('')
    setUbicacionParcelaIds([])
    setUbicacionError('')
  }

  const iniciarNuevaUbicacion = () => {
    cerrarFormUbicacion()
    setFormUbicacionPaso('finca')
  }

  const editarUbicacion = (u: Ubicacion) => {
    setUbicacionEditar(u)
    setUbicacionFincaId(u.fincaId)
    setUbicacionPunto({ lat: u.lat, lng: u.lng })
    setUbicacionNombre(u.nombre)
    setUbicacionParcelaIds(u.parcelaIds)
    setUbicacionError('')
    setFormUbicacionPaso('detalles')
  }

  const continuarUbicacionAMapa = () => {
    if (!ubicacionFincaId) { setUbicacionError('Selecciona una finca'); return }
    setUbicacionError('')
    setFormUbicacionPaso('mapa')
  }

  const confirmarPuntoUbicacion = () => {
    if (!ubicacionPunto) { setUbicacionError('Toca el mapa para marcar el punto'); return }
    // Al entrar por primera vez (no editando), seleccionamos por defecto todas las parcelas de la finca
    if (!ubicacionEditar && ubicacionParcelaIds.length === 0) {
      setUbicacionParcelaIds(misParcelas.filter(p => p.fincaId === ubicacionFincaId).map(p => p.id))
    }
    setUbicacionError('')
    setFormUbicacionPaso('detalles')
  }

  const toggleUbicacionParcela = (id: string) => {
    setUbicacionParcelaIds(prev => prev.includes(id) ? prev.filter(x => x !== id) : [...prev, id])
  }

  const guardarUbicacion = async () => {
    if (!ubicacionNombre.trim()) { setUbicacionError('El nombre es obligatorio (ej: Pozo Norte, Canal de riego...)'); return }
    if (!ubicacionPunto) { setUbicacionError('Falta marcar el punto en el mapa'); return }
    if (ubicacionParcelaIds.length === 0) { setUbicacionError('Selecciona al menos una parcela'); return }
    if (!session) return
    setGuardandoUbicacion(true)
    setUbicacionError('')
    const nueva: Ubicacion = {
      id: ubicacionEditar?.id || String(Date.now()),
      fincaId: ubicacionFincaId,
      nombre: ubicacionNombre.trim(),
      lat: ubicacionPunto.lat,
      lng: ubicacionPunto.lng,
      parcelaIds: ubicacionParcelaIds,
      fechaGuardado: ubicacionEditar?.fechaGuardado || new Date().toLocaleDateString('es-ES'),
    }
    let error: any = null
    try {
      const resultado = await supabase.from('ubicaciones').upsert({
        id: nueva.id, user_id: session.user.id, finca_id: nueva.fincaId, nombre: nueva.nombre,
        lat: nueva.lat, lng: nueva.lng, parcela_ids: nueva.parcelaIds, fecha_guardado: nueva.fechaGuardado,
      })
    ...
    if (error) {
      ...
      setUbicacionError(`No se pudo guardar: ${error.message || 'revisa que la tabla "ubicaciones" exista en la base de datos'}`)
      return
    }
    const lista = ubicacionEditar
      ? ubicaciones.map(u => u.id === ubicacionEditar.id ? nueva : u)
      : [nueva, ...ubicaciones]
  ...
  const eliminarUbicacion = async (id: string) => {
    ...
    } catch (e) { console.error(e); alert('No se pudo eliminar la ubicación.'); return }
  ...
  // - Datos de la explotación (cuaderno de campo) -
  // Abre el editor de "datos de la explotación" para una o varias fincas a la vez. Si todas las
  // fincas indicadas comparten los mismos datos guardados, precarga ese valor; si no coinciden
  // (o alguna no tiene nada guardado todavía), abre el formulario en blanco para no mezclar datos.
  const abrirFormDatosExplotacion = (fincaIds: string[]) => {
    if (fincaIds.length === 0) return
    const datasets = fincaIds.map(id => datosExplotacionPorFinca[id]).filter(Boolean) as DatosExplotacion[]
    const todasIguales = datasets.length === fincaIds.length && datasets.every(d => JSON.stringify(d) === JSON.stringify(datasets[0]))
    setDeForm(todasIguales ? datasets[0] : DATOS_EXPLOTACION_VACIO)
    setDeFincaIds(fincaIds)
    setFormDatosExplotacion(true)
  }

  const guardarDatosExplotacion = async () => {
    if (!session || deFincaIds.length === 0) return
    setDeGuardando(true)
    try {
      const nuevoMapa = { ...datosExplotacionPorFinca }
      deFincaIds.forEach(id => { nuevoMapa[id] = deForm })
      const resultados = await Promise.all(deFincaIds.map(id =>
        supabase.from('datos_explotacion').upsert({ user_id: session.user.id, finca_id: id, datos: deForm })
      ))
      const conError = resultados.find(r => r.error)
      if (conError?.error) throw conError.error
      setDatosExplotacionPorFinca(nuevoMapa)
      setFormDatosExplotacion(false)
    } catch (e: any) {
      console.error('Error guardando datos de la explotación:', e)
      alert(`No se pudo guardar: ${e?.message || 'revisa que la tabla "datos_explotacion" tenga la columna "finca_id"'}`)
    } finally {
  ...
  // Formatea una cantidad de cosecha/comercialización al estilo español, sin decimales fijos
  // (p.ej. 8500 -> "8.500", 1234.5 -> "1.234,5"). Se usa en Historial de cosechas.
  const formatCantidadEs = (n: number): string => {
    if (!isFinite(n)) return ''
  ...
  // app puede completar: 1.2, 1.3, 1.4 (parcial), 2.1, 3.1 y 3.2, y 3.1 bis cuando existan
  // tratamientos marcados "Con asesor" o tareas de tipo "Alternativa no química". El resto
  // (2.2 sin ubicaciones, 3.3, 3.4, 3.5, 4 y 5) se genera en blanco, tal como el modelo
  // original, para rellenar a mano si procede.
  const generarCuaderno = async () => {
    if (!misPermisos['tratamientos.descargar_cuaderno']) { alert('No tienes permiso para descargar el Cuaderno de Campo.'); return }
    ...
    if (fincas.length > 0 && cuadernoFincaIds.length === 0) { alert('Selecciona al menos una finca — los datos de la explotación (1.1, Titular, 1.5) se guardan y se cargan por finca.'); return }
    ...
    try {
      ...
      const nombreEquipo = (id: string) => { const e = equipos.find(x => x.id === id); return e ? `${e.tipo}${e.subtipo ? ' ' + e.subtipo : ''} — ${e.nombre}${e.nroRoma ? ` (ROMA: ${e.nroRoma})` : ''}` : '' }
      ...
      const filas31 = tratamientosFoliares.map(t => {
        ...
        // La plantilla oficial del cuaderno solo admite la dosis en Kg/ha o L/ha, así que
        // convertimos la unidad real del tratamiento (que puede depender del volumen de
        // caldo, p.ej. mL/hL, %(v/v)...) a su equivalente canónico por hectárea.
        const dosisCanon = dosisACanonico(
          Number(t.dosis || '0'),
          t.unidadDosis as UnidadDosis,
          { litrosCaldoHa: t.litrosCaldoHa ? Number(t.litrosCaldoHa) : undefined }
        )
        const dosisTexto = dosisCanon.ok
          ? `${formatSupEs(dosisCanon.valor, 3)} ${dosisCanon.fase === 'masa' ? 'Kg/ha' : 'L/ha'}`
          : `${t.dosis || ''} ${t.unidadDosis || ''}`.trim()
        // Observaciones: las que haya escrito el usuario + el volumen de caldo aplicado (si la
        // unidad de dosis lo requiere), el % de superficie aplicada (si es distinto del 100%)
        // y un aviso si no se ha podido convertir la dosis por falta de datos.
        ...
        if (!dosisCanon.ok && t.dosis) notasExtra.push(`⚠ Dosis sin convertir a Kg/ha o L/ha (${dosisCanon.motivo || 'faltan datos'})`)
      ...
      // --- 3.1 bis Registro de tratamientos fitosanitarios de la parcela (solo cultivos/superficies
      // objeto de asesoramiento GIP). Sigue la plantilla oficial: CULTIVO y DATOS DE LA PARCELA se
      // rellenan siempre; luego la fila solo ocupa UNO de los dos bloques de intervención, dejando
      // el otro en blanco — "Alternativas no químicas" si viene de una tarea de tipo "Alternativa no
      // química", o "Alternativas químicas" si viene de un tratamiento marcado como "Con asesor".
      const supParcelas = (ids: string[]): number => ids.reduce((acc, id) => acc + (misParcelas.find(p => p.id === id)?.supHa || 0), 0)

      const tratamientosAsesorados = tratamientosFoliares.filter(t => t.conAsesor)
      const registrosAsesoradosTrat = tratamientosAsesorados.map(t => {
        const { especie, variedad } = cultivoDeParcelas(t.parcelaIds)
        const supCult = supParcelas(t.parcelaIds)
        const st = supTratada(t)
        const dosisCanon = dosisACanonico(
          Number(t.dosis || '0'),
          t.unidadDosis as UnidadDosis,
        ...
        return {
          ...
          fila: [
            ...
            especie, variedad,
            // DATOS DE LA PARCELA
            idsParcelasTexto(t.parcelaIds), supCult > 0 ? formatSupEs(supCult, 2) : '', st > 0 ? formatSupEs(st, 2) : '',
            // PLAGA A CONTROLAR
            t.problemaFitosanitario || '', t.justificacionActuacion || '',
            // ALTERNATIVAS NO QUÍMICAS DE INTERVENCIÓN — en blanco (viene de un tratamiento fitosanitario)
            '', '', '',
            // ALTERNATIVAS QUÍMICAS DE INTERVENCIÓN
            t.producto || '', t.numRegistro || '', dosisTexto, formatFechaEs(t.fecha),
            // Eficacia / Observaciones
            t.eficacia || '', t.observaciones || '',
          ],
        }
      })

      const tareasAlternativaSel = tareas.filter(x =>
        x.tipo === 'Alternativa no química' && x.estado === 'realizada' && x.campanaId === campanaSel.id &&
        (!idsFincaSel || (x.fincaId ? idsFincaSel.has(x.fincaId) : x.parcelaIds.some(pid => { const p = misParcelas.find(pp => pp.id === pid); return !!p?.fincaId && idsFincaSel!.has(p.fincaId) })))
      )
      const registrosAlternativa = tareasAlternativaSel.map(t => {
        const { especie, variedad } = cultivoDeParcelas(t.parcelaIds)
        const supCult = supParcelas(t.parcelaIds)
        return {
          fechaISO: t.fechaPrevista,
          fila: [
            // CULTIVO
            especie, variedad,
            // DATOS DE LA PARCELA (sin % de aplicación en tareas: se asume el 100% de lo cultivado)
            idsParcelasTexto(t.parcelaIds), supCult > 0 ? formatSupEs(supCult, 2) : '', supCult > 0 ? formatSupEs(supCult, 2) : '',
            // PLAGA A CONTROLAR
            t.plaga || '', t.justificacionActuacion || '',
            // ALTERNATIVAS NO QUÍMICAS DE INTERVENCIÓN
            t.tipoMedida || '', t.intensidadMedida || '', formatFechaEs(t.fechaPrevista),
            // ALTERNATIVAS QUÍMICAS DE INTERVENCIÓN — en blanco (viene de una tarea no química)
            '', '', '', '',
            // Eficacia / Observaciones
            t.eficacia || '', t.comentarios || '',
          ],
        }
      })

      const filas31bis = [...registrosAsesoradosTrat, ...registrosAlternativa]
        .sort((a, b) => (a.fechaISO || '').localeCompare(b.fechaISO || ''))
        .map(r => r.fila)

      // --- 2.2 Datos identificativos medioambientales de las parcelas (puntos de captación de agua) ---
      // Se construye a partir de las Ubicaciones (pozos, canales...) marcadas en el mapa y asociadas
      // a parcelas de las fincas seleccionadas. No cubre la parte de "zonas específicas" (protección
      // de hábitats, art. 34 RD 1311/2012), que se deja en blanco por no gestionarse en la app.
      const idsParcelasPorFinca: Record<string, string[]> = {}
      parcelasSel.forEach(p => { if (p.fincaId) (idsParcelasPorFinca[p.fincaId] ||= []).push(p.id) })
      const ubicacionesSel = ubicaciones.filter(u => !idsFincaSel || idsFincaSel.has(u.fincaId))
      const filas22 = ubicacionesSel
        .map(u => {
          // Solo las parcelas de la ubicación que sigan formando parte de este cuaderno (misma
          // campaña/finca seleccionadas)
          const relevantes = u.parcelaIds.filter(id => ordenPorParcela[id])
          if (relevantes.length === 0) return null
          const totalFinca = idsParcelasPorFinca[u.fincaId] || []
          const idTexto = totalFinca.length > 0 && relevantes.length === totalFinca.length
            ? 'TODAS'
            : relevantes.map(id => ordenPorParcela[id]).sort((a, b) => a - b).join(', ')
          const { especie, variedad } = cultivoDeParcelas(relevantes)
          const geojsons = relevantes.map(id => misParcelas.find(p => p.id === id)?.geojson).filter(Boolean)
          const incluido = geojsons.some(gj => puntoEnPoligono(u.lat, u.lng, gj))
          const distanciaM = incluido ? null : Math.min(...geojsons.map(gj => distanciaAPoligonoM(u.lat, u.lng, gj)))
          const utm = latLngToUTM(u.lat, u.lng)
          return [
            idTexto, especie, variedad,
            incluido ? 'SI' : 'NO',
            !incluido && isFinite(distanciaM as number) ? formatSupEs(distanciaM as number, 0) : '',
            `Huso ${utm.zona}${utm.banda}  X: ${formatSupEs(utm.x, 0)}  Y: ${formatSupEs(utm.y, 0)}`,
            u.nombre,
            '', '',
          ]
        })
      ...
      // --- 1.3 / 1.4 Equipos de aplicación realmente usados en los tratamientos del año/fincas
      // elegidos, separados en propios y externos de la explotación ---
      ...
      const equiposPropiosUsados = equiposUsados.filter(e => e.titularidad !== 'Externa')
      const equiposExternosUsados = equiposUsados.filter(e => e.titularidad === 'Externa')
      const filas13 = equiposPropiosUsados.map((e, i) => [String(i + 1), `${e.tipo}${e.subtipo ? ' ' + e.subtipo : ''} — ${e.nombre}`, e.nroRoma || '', formatFechaEs(e.fechaAdquisicion), formatFechaEs(e.fechaUltimaInspeccion)])
      const filas14Externos = equiposExternosUsados.map((e, i) => [String(i + 1), `${e.tipo}${e.subtipo ? ' ' + e.subtipo : ''} — ${e.nombre}`, e.nroRoma || '', e.dniCif || ''])

      // --- 1.5 Agrupación o entidad de asesoramiento ---
      // Los datos de la explotación (1.1, Titular y 1.5) están guardados por finca. Si las fincas
      // seleccionadas tienen datos distintos entre sí, se usan los de la primera y se avisa (ya en
      // el propio modal, antes de llegar aquí); si ninguna tiene datos guardados, queda en blanco.
      const de = fincasSeleccionadas.map(f => datosExplotacionPorFinca[f.id]).find(Boolean) || DATOS_EXPLOTACION_VACIO
      const filas15 = (de.agrupacionNombre || de.agrupacionNif)
        ? [[de.agrupacionNombre, de.agrupacionNif, de.agrupacionNroIdentificacion, de.agrupacionTipoExplotacion]]
        : []
      ...
      // Cada venta puede repartirse (FIFO) entre varias cosechas/parcelas cuando se registra
      // desde la vista agrupada por "parte"; esas filas comparten ventaId y aquí se vuelven a
      // juntar en una única línea, sumando cantidades y listando todas las parcelas de origen.
      const comercializacionesSel = comercializaciones.filter(com => {
        const cosecha = cosechas.find(c => c.id === com.cosechaId)
        return !!cosecha && cosecha.campanaId === campanaSel.id && !!ordenPorParcela[cosecha.parcelaId]
      })
      const gruposVenta = new Map<string, Comercializacion[]>()
      comercializacionesSel.forEach(com => {
        const clave = com.ventaId || com.id
        if (!gruposVenta.has(clave)) gruposVenta.set(clave, [])
        gruposVenta.get(clave)!.push(com)
      })
      const filas5 = Array.from(gruposVenta.values())
        .sort((a, b) => (a[0].fecha || '').localeCompare(b[0].fecha || ''))
        .map(grupo => {
          const ref = grupo[0]
          const cantidadTotal = grupo.reduce((acc, c) => acc + Number(c.cantidad || 0), 0)
          const ordenesParcelas = Array.from(new Set(grupo.map(c => {
            const cosecha = cosechas.find(x => x.id === c.cosechaId)
            return cosecha ? ordenPorParcela[cosecha.parcelaId] : undefined
          }).filter((n): n is number => n !== undefined))).sort((a, b) => a - b)
          const cosechaRef = cosechas.find(c => c.id === ref.cosechaId)
          return [
            formatFechaEs(ref.fecha),
            cosechaRef ? formatCultivoVariedad(cosechaRef.cultivo, cosechaRef.variedad) : '',
            formatCantidadEs(cantidadTotal),
            ordenesParcelas.join(', '),
            ref.numAlbaranFactura || '',
            ref.numLote || '',
            ref.comprador || '',
            ref.compradorDniNif || '',
            ref.compradorDireccion || '',
            ref.compradorRgseaa || '',
          ]
        })
      ...
      const secciones = [
        ...
        ...(filas14Externos.length > 0 ? [{
          numero: '1.4', titulo: 'EQUIPOS DE APLICACIÓN DE PRODUCTOS FITOSANITARIOS EXTERNOS DE LA EXPLOTACIÓN',
          headers: ['Nº orden', 'Descripción del equipo', 'Nº inscrip. ROMA', 'Titular (NIF/CIF)'],
          colWidths: [1, 3, 1.4, 1.6], rows: filas14Externos, filasVaciasMin: 2,
          notas: ['Se listan los equipos de terceros (empresas de servicios, alquiler...) empleados en los tratamientos registrados en el año/fincas seleccionados.'],
        }] : []),
        {
          numero: '1.5', titulo: 'AGRUPACIÓN O ENTIDAD DE ASESORAMIENTO A LA QUE PERTENECE LA EXPLOTACIÓN',
          headers: ['Nombre o razón social', 'NIF', 'Nº de identificación', 'Tipo de explotación'],
          colWidths: [3, 1.4, 1.6, 1.6], rows: filas15, filasVaciasMin: 1,
        },
        {
        ...
        },
        {
          ...
          colWidths: [1, 1.4, 1.4, 1.6, 1.2, 1.6, 1.8, 1.4, 1.4], rows: filas22, filasVaciasMin: 6,
          notas: filas22.length > 0
            ? ['Coordenadas UTM aproximadas, calculadas a partir del punto marcado en el mapa (MIS PARCELAS → Ubicaciones). Las columnas "Totalmente"/"Parcialmente" (zonas específicas, art. 34 RD 1311/2012) deben rellenarse a mano.']
            : [NOTA_SIN_DATOS],
          saltoPaginaAntes: true,
        },
        {
        ...
        },
        {
          ...
          gruposCabecera: [
            { titulo: 'CULTIVO', span: 2 },
            { titulo: 'DATOS DE LA PARCELA', span: 3 },
            { titulo: 'PLAGA A CONTROLAR', span: 2 },
            { titulo: 'ALTERNATIVAS NO QUÍMICAS DE INTERVENCIÓN', span: 3 },
            { titulo: 'ALTERNATIVAS QUÍMICAS DE INTERVENCIÓN', span: 4 },
            { titulo: 'Eficacia de la intervención', span: 1 },
            { titulo: 'Observaciones', span: 1 },
          ],
          headers: [
            'Especie', 'Variedad',
            'Id. parcelas', 'Sup. cultivada (ha)', 'Sup. tratada (ha)',
            'Plaga', 'Justificación de la actuación',
            'Tipo de medida', 'Intensidad de la medida', 'Fecha de actuación',
            'Nombre comercial', 'Nº registro', 'Dosis', 'Fecha de actuación',
            'Eficacia', 'Observaciones',
          ],
          colWidths: [1, 1, 0.8, 1, 1, 1, 1.5, 1, 1.2, 0.9, 1.2, 0.9, 0.9, 0.9, 1, 1.3], rows: filas31bis, filasVaciasMin: 5,
          notas: filas31bis.length > 0
            ? ['Cada fila ocupa solo uno de los dos bloques de intervención: "Alternativas químicas" si viene de un tratamiento fitosanitario marcado como "Con asesor", o "Alternativas no químicas" si viene de una tarea de tipo "Alternativa no química". Solo aplica a cultivos con asesoramiento de gestión integrada de plagas.']
            : ['Sección sin datos disponibles en la aplicación — solo aplica a cultivos con asesoramiento de gestión integrada de plagas.'],
          saltoPaginaAntes: true,
        },
        {
        ...
        },
        {
          ...
          colWidths: [1, 1.2, 1, 1.4, 1.2, 0.9, 1.8, 1, 1.6, 1], rows: filas5, filasVaciasMin: 5,
          notas: filas5.length > 0 ? undefined : [NOTA_SIN_DATOS], saltoPaginaAntes: true,
        },
      ]
  ...
  const PALETA_CULTIVOS = ['#1F4A28', '#0F4A70', '#ffb84d', '#6E2417', '#c792ea', '#4dd0e1', '#f06292', '#aed581', '#ba68c8', '#ffd54f', '#81c784', '#7986cb']
          ...
          <div>
            ...
            {predRecomendador && !tRecomendadoPor && (
              <div style={{ fontSize: 9, color: 'var(--muted)', fontFamily: 'var(--mono)', marginTop: 3 }}>
                ...
                <button onClick={() => setTRecomendadoPor(predRecomendador.id)} style={{ marginLeft: 6, color: 'var(--text)', background: 'transparent', border: 'none', cursor: 'pointer', fontSize: 9, fontFamily: 'var(--mono)', textDecoration: 'underline' }}>Usar</button>
              </div>
            )}
          </div>
        )}
          ...
          <div>
            ...
            {predAplicador && !tAplicadoPor && (
              <div style={{ fontSize: 9, color: 'var(--muted)', fontFamily: 'var(--mono)', marginTop: 3 }}>
                ...
                <button onClick={() => setTAplicadoPor(predAplicador.id)} style={{ marginLeft: 6, color: 'var(--text)', background: 'transparent', border: 'none', cursor: 'pointer', fontSize: 9, fontFamily: 'var(--mono)', textDecoration: 'underline' }}>Usar</button>
              </div>
            )}
          </div>
        )}
      ...
      <div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4, maxHeight: 180, overflowY: 'auto', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: 8 }}>
          {disponibles.map(e => {
            ...
            return (
              <label key={e.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 6px', borderRadius: 4, cursor: deshabilitado ? 'not-allowed' : 'pointer', opacity: deshabilitado ? 0.4 : 1, background: marcado ? 'rgba(31, 74, 40,0.08)' : 'transparent' }}>
        ...
        {dias.map((d, i) => {
          ...
          return (
            <div key={d.id} style={{ display: 'flex', flexWrap: 'wrap', gap: 6, alignItems: 'center', padding: '8px 10px', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6 }}>
              ...
              <span style={{ fontSize: 10, fontFamily: 'var(--mono)', color: completo ? 'var(--green)' : 'var(--muted)', textShadow: completo ? 'var(--green-outline)' : 'none', marginLeft: 'auto' }}>
                {completo ? `Total: ${formatDuracion(mins)}` : ''}
              </span>
              <button onClick={() => setDias(dias.filter(x => x.id !== d.id))} title="Eliminar día"
                style={{ background: 'transparent', border: 'none', color: 'var(--text)', cursor: 'pointer', fontSize: 14, padding: '0 2px' }}>✕</button>
            </div>
          )
        })}
        <button onClick={() => setDias([...dias, nuevoDiaHorario()])}
          style={{ alignSelf: 'flex-start', padding: '6px 12px', borderRadius: 6, background: 'var(--surface2)', border: '1px dashed var(--border)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 11, cursor: 'pointer' }}>
          + Añadir otro día
        </button>
        {dias.length > 0 && (
  ...
  const upsertTratamientoDB = async (t: Tratamiento): Promise<string | null> => {
    ...
    try {
      const { error } = await supabase.from('tratamientos').upsert({
        ...
        con_asesor: t.conAsesor || false, justificacion_actuacion: t.justificacionActuacion || null,
      })
      if (error) {
  ...
  const abrirFormTrat = (parcelaId?: string, trat?: Tratamiento, personalId?: string) => {
    ...
    setTConAsesor(trat?.conAsesor || false)
    setTJustificacionActuacion(trat?.justificacionActuacion || '')
  ...
  const guardarTratamiento = async () => {
    ...
    const datosComunes = {
      ...
      conAsesor: tConAsesor,
      justificacionActuacion: tConAsesor ? tJustificacionActuacion.trim() || undefined : undefined,
  ...
  const continuarNuevaTarea = () => {
    ...
    if (nuevaTareaTipo === 'Tratamiento fitosanitario') {
      ...
      setTarPlaga(''); setTarJustificacionActuacion(''); setTarTipoMedida(''); setTarIntensidadMedida(''); setTarEficacia('')
  ...
  const guardarTareaGeneral = async () => {
    ...
    const nueva: Tarea = {
      ...
      plaga: tarTipo === 'Alternativa no química' ? tarPlaga.trim() || undefined : undefined,
      justificacionActuacion: tarTipo === 'Alternativa no química' ? tarJustificacionActuacion.trim() || undefined : undefined,
      tipoMedida: tarTipo === 'Alternativa no química' ? tarTipoMedida.trim() || undefined : undefined,
      intensidadMedida: tarTipo === 'Alternativa no química' ? tarIntensidadMedida.trim() || undefined : undefined,
      eficacia: tarTipo === 'Alternativa no química' ? (tarEficacia || undefined) : undefined,
    }
    ...
    try {
      await supabase.from('tareas').upsert({
        ...
        plaga: nueva.plaga || null, justificacion_actuacion: nueva.justificacionActuacion || null,
        tipo_medida: nueva.tipoMedida || null, intensidad_medida: nueva.intensidadMedida || null,
        eficacia: nueva.eficacia || null,
      })
  ...
  const abrirEditarTareaGeneral = (t: Tarea) => {
    ...
    setTarPlaga(t.plaga || ''); setTarJustificacionActuacion(t.justificacionActuacion || '')
    setTarTipoMedida(t.tipoMedida || ''); setTarIntensidadMedida(t.intensidadMedida || '')
    setTarEficacia(t.eficacia || '')
  ...
  // Trazabilidad: Parcela → Cultivo → Campaña → Cosecha → Comercialización.
  const totalComercializado = (cosechaId: string): number =>
    comercializaciones.filter(c => c.cosechaId === cosechaId).reduce((acc, c) => acc + (Number(c.cantidad) || 0), 0)

  // Pendiente de comercializar: solo se puede calcular si se registró la materia cosechada.
  const pendienteComercializar = (c: Cosecha): number | null =>
    c.cantidad ? Math.max(0, Number(c.cantidad) - totalComercializado(c.id)) : null

  // "Parte" de cosecha: agrupa todas las cosechas de un mismo cultivo, campaña y finca (aunque
  // vengan de parcelas distintas) en un único registro de cara al usuario. La clave identifica
  // el grupo; las cosechas concretas (una por parcela/fecha) quedan como "entradas" del parte.
  const parteKeyDeCosecha = (c: Cosecha): string => {
    const p = misParcelas.find(x => x.id === c.parcelaId)
    return `${c.cultivo}|${c.campanaId}|${p?.fincaId || ''}`
  }

  // Resumen de cosecha/comercializado para una parcela+campaña, usado junto al histórico de
  // cultivos (PARCELA → CULTIVO → CAMPAÑA → COSECHA). Si no hay cosecha registrada para esa
  // campaña, no se muestra nada (nunca "0 kg").
  const cosechaResumen = (parcelaId: string, campanaId?: string): string | null => {
    if (!campanaId) return null
    const c = cosechas.find(x => x.parcelaId === parcelaId && x.campanaId === campanaId)
    if (!c) return null
    const partes: string[] = []
    if (c.cantidad) partes.push(`Cosecha: ${formatCantidadEs(Number(c.cantidad))} ${c.unidad}`)
    const com = totalComercializado(c.id)
    if (com > 0) partes.push(`Comercializado: ${formatCantidadEs(com)} ${c.unidad}`)
    return partes.length > 0 ? partes.join(' · ') : null
  }

  // Cultivo (y variedad) de una parcela para una campaña concreta: el actual si coincide con
  // esa campaña (o la parcela no tiene campaña asignada), si no el histórico guardado para esa
  // campaña. Igual criterio que en "+ NUEVA TAREA" — así el desplegable de parcela de "+ NUEVA
  // COSECHA" siempre muestra el cultivo correcto entre paréntesis, aunque la parcela haya
  // rotado de cultivo desde entonces.
  const cultivoVariedadEnCampana = (p: ParcelaGuardada, campanaId: string): { cultivo: string; variedad?: string } => {
    if (!campanaId || !p.campanaId || p.campanaId === campanaId) return { cultivo: p.cultivo, variedad: p.variedad }
    const hist = historicoCultivos.find(h => h.parcelaId === p.id && h.campanaId === campanaId)
    return hist ? { cultivo: hist.cultivo, variedad: hist.variedad } : { cultivo: p.cultivo, variedad: p.variedad }
  }

  const abrirNuevaCosecha = (p?: ParcelaGuardada) => {
    setCosEditar(null)
    setCosCampanaId(p?.campanaId || '')
    setCosFincaId(p?.fincaId || '')
    setCosParcelaId(p?.id || '')
    setCosFecha(new Date().toISOString().slice(0, 10))
    setCosCantidad('')
    setCosUnidad('kg')
    setCosObservaciones('')
    setCosError('')
    setCosFormVisible(true)
  }

  const abrirEditarCosecha = (c: Cosecha) => {
    const p = misParcelas.find(x => x.id === c.parcelaId)
    setCosEditar(c)
    setCosCampanaId(c.campanaId)
    setCosFincaId(p?.fincaId || '')
    setCosParcelaId(c.parcelaId)
    setCosFecha(c.fechaCosecha)
    setCosCantidad(c.cantidad || '')
    setCosUnidad(c.unidad || 'kg')
    setCosObservaciones(c.observaciones || '')
    setCosError('')
    setCosFormVisible(true)
  }
  ...
  const guardarCosecha = async () => {
    if (!session) return
    if (!cosCampanaId) { setCosError('La campaña es obligatoria'); return }
    if (!cosFincaId) { setCosError('La finca es obligatoria'); return }
    if (!cosParcelaId) { setCosError('La parcela es obligatoria'); return }
    if (!cosFecha) { setCosError('La fecha de cosecha es obligatoria'); return }
    if (cosCantidad && Number(cosCantidad) < 0) { setCosError('La cantidad no puede ser negativa'); return }
    const parcela = misParcelas.find(p => p.id === cosParcelaId)
    if (!parcela) { setCosError('La parcela seleccionada no es válida'); return }
    const { cultivo, variedad } = cultivoVariedadEnCampana(parcela, cosCampanaId)

    const nueva: Cosecha = {
      id: cosEditar?.id || String(Date.now()),
      parcelaId: cosParcelaId, campanaId: cosCampanaId, cultivo,
      variedad, fechaCosecha: cosFecha,
      cantidad: cosCantidad.trim() || undefined, unidad: cosUnidad,
      observaciones: cosObservaciones.trim() || undefined,
      fechaRegistro: cosEditar?.fechaRegistro || new Date().toLocaleDateString('es-ES'),
    }
    setCosechas(cosEditar ? cosechas.map(c => c.id === nueva.id ? nueva : c) : [nueva, ...cosechas])
    try {
      await supabase.from('cosechas').upsert({
        id: nueva.id, user_id: session.user.id, parcela_id: nueva.parcelaId, campana_id: nueva.campanaId,
        cultivo: nueva.cultivo, variedad: nueva.variedad || null, fecha_cosecha: nueva.fechaCosecha,
        cantidad: nueva.cantidad ? Number(nueva.cantidad) : null, unidad: nueva.unidad,
        observaciones: nueva.observaciones || null, fecha_registro: nueva.fechaRegistro,
      })
    } catch (e) { console.error('Error guardando cosecha:', e) }
    setCosFormVisible(false)
    setCosEditar(null)
  }

  const eliminarCosecha = async (id: string) => {
    if (!confirm('¿Eliminar esta cosecha? También se eliminarán sus comercializaciones registradas.')) return
    setCosechas(cosechas.filter(c => c.id !== id))
    setComercializaciones(comercializaciones.filter(c => c.cosechaId !== id))
    try {
      await supabase.from('comercializaciones').delete().eq('cosecha_id', id)
      await supabase.from('cosechas').delete().eq('id', id)
    } catch (e) { console.error('Error eliminando cosecha:', e) }
  }

  const abrirNuevaComercializacion = (cosechaId: string) => {
    setComFormCosechaId(cosechaId)
    setComFormParteKey(null)
    setComEditarId(null)
    setComFecha(new Date().toISOString().slice(0, 10))
    setComCantidad('')
    setComUnidad('kg')
    setComComprador('')
    setComCompradorDniNif('')
    setComCompradorDireccion('')
    setComCompradorRgseaa('')
    setComNumAlbaranFactura('')
    setComNumLote('')
    setComObservaciones('')
    setComError('')
  }

  // Igual que abrirNuevaComercializacion, pero para la pestaña COSECHAS (vista agrupada por
  // parte): al guardar, la cantidad se reparte automáticamente entre las cosechas del parte
  // empezando por la más antigua (ver guardarComercializacion).
  const abrirNuevaComercializacionParte = (parteKey: string) => {
    setComFormParteKey(parteKey)
    setComFormCosechaId(null)
    setComEditarId(null)
    setComFecha(new Date().toISOString().slice(0, 10))
    setComCantidad('')
    setComUnidad('kg')
  ...
  const abrirEditarComercializacion = (c: Comercializacion) => {
    setComFormCosechaId(c.cosechaId)
    setComFormParteKey(null)
    setComEditarId(c.id)
    setComFecha(c.fecha)
    setComCantidad(c.cantidad)
    setComUnidad(c.unidad || 'kg')
    setComComprador(c.comprador || '')
    setComCompradorDniNif(c.compradorDniNif || '')
    setComCompradorDireccion(c.compradorDireccion || '')
    setComCompradorRgseaa(c.compradorRgseaa || '')
    setComNumAlbaranFactura(c.numAlbaranFactura || '')
    setComNumLote(c.numLote || '')
    setComObservaciones(c.observaciones || '')
    setComError('')
  }

  const guardarComercializacion = async () => {
    if (!session) return
    if (!comFecha) { setComError('La fecha es obligatoria'); return }
    if (!comCantidad || Number(comCantidad) <= 0) { setComError('La cantidad comercializada es obligatoria'); return }

    // Caso 1: edición de una comercialización ya existente — sigue ligada a la misma cosecha.
    if (comEditarId) {
      if (!comFormCosechaId) return
      const cosecha = cosechas.find(c => c.id === comFormCosechaId)
      if (cosecha?.cantidad) {
        const yaComercializado = totalComercializado(comFormCosechaId) - Number(comercializaciones.find(c => c.id === comEditarId)?.cantidad || 0)
        if (yaComercializado + Number(comCantidad) > Number(cosecha.cantidad)) {
          setComError(`La cantidad supera lo pendiente de comercializar (${formatCantidadEs(Number(cosecha.cantidad) - yaComercializado)} ${cosecha.unidad})`)
          return
        }
      }
      const nueva: Comercializacion = {
        id: comEditarId, cosechaId: comFormCosechaId, fecha: comFecha, cantidad: comCantidad.trim(), unidad: comUnidad,
        comprador: comComprador.trim() || undefined, compradorDniNif: comCompradorDniNif.trim() || undefined,
        compradorDireccion: comCompradorDireccion.trim() || undefined, compradorRgseaa: comCompradorRgseaa.trim() || undefined,
        numAlbaranFactura: comNumAlbaranFactura.trim() || undefined, numLote: comNumLote.trim() || undefined,
        observaciones: comObservaciones.trim() || undefined,
        fechaRegistro: comercializaciones.find(c => c.id === comEditarId)?.fechaRegistro || new Date().toLocaleDateString('es-ES'),
        ventaId: comercializaciones.find(c => c.id === comEditarId)?.ventaId,
      }
      setComercializaciones(comercializaciones.map(c => c.id === comEditarId ? nueva : c))
      try {
        const { error } = await supabase.from('comercializaciones').upsert({
          id: nueva.id, user_id: session.user.id, cosecha_id: nueva.cosechaId, fecha: nueva.fecha,
          cantidad: Number(nueva.cantidad), unidad: nueva.unidad, comprador: nueva.comprador || null,
          comprador_dni_nif: nueva.compradorDniNif || null,
          comprador_direccion: nueva.compradorDireccion || null, comprador_rgseaa: nueva.compradorRgseaa || null,
          num_albaran_factura: nueva.numAlbaranFactura || null, num_lote: nueva.numLote || null,
          venta_id: nueva.ventaId || null,
          observaciones: nueva.observaciones || null, fecha_registro: nueva.fechaRegistro,
        })
        if (error) { setComError('No se pudo guardar: ' + error.message); return }
      } catch (e) { console.error('Error guardando comercialización:', e) }
      setComFormCosechaId(null); setComFormParteKey(null); setComEditarId(null)
      return
    }

    // Caso 2: alta nueva a nivel de PARTE (pestaña COSECHAS) — se reparte entre las cosechas
    // de ese cultivo+campaña+finca empezando por la más antigua (FIFO), pudiendo tocar varias
    // entradas/parcelas si la primera no tiene pendiente suficiente.
    if (comFormParteKey) {
      const entradas = cosechas
        .filter(c => c.cantidad && parteKeyDeCosecha(c) === comFormParteKey)
        .sort((a, b) => (a.fechaCosecha || '').localeCompare(b.fechaCosecha || '') || a.id.localeCompare(b.id))

      let restante = Number(comCantidad)
      const reparto: { cosechaId: string; cantidad: number }[] = []
      for (const c of entradas) {
        if (restante <= 1e-9) break
        const pend = pendienteComercializar(c)
        if (!pend) continue
        const usar = Math.min(pend, restante)
        reparto.push({ cosechaId: c.id, cantidad: usar })
        restante -= usar
      }
      if (restante > 1e-9) {
        const pendienteTotalParte = entradas.reduce((acc, c) => acc + (pendienteComercializar(c) || 0), 0)
        setComError(`La cantidad supera lo pendiente de comercializar (${formatCantidadEs(pendienteTotalParte)} ${comUnidad})`)
        return
      }

      const fechaRegistro = new Date().toLocaleDateString('es-ES')
      // Todas las filas de esta venta comparten ventaId, aunque el reparto FIFO toque varias
      // cosechas/parcelas — así el cuaderno de campo (sección 5) puede volver a mostrarlas como
      // una única línea de venta con el total y todas las parcelas de origen.
      const ventaId = String(Date.now())
      const nuevas: Comercializacion[] = reparto.map((r, i) => ({
        id: `${ventaId}-${i}`, cosechaId: r.cosechaId, fecha: comFecha, cantidad: String(r.cantidad), unidad: comUnidad,
        comprador: comComprador.trim() || undefined, compradorDniNif: comCompradorDniNif.trim() || undefined,
        compradorDireccion: comCompradorDireccion.trim() || undefined, compradorRgseaa: comCompradorRgseaa.trim() || undefined,
        numAlbaranFactura: comNumAlbaranFactura.trim() || undefined, numLote: comNumLote.trim() || undefined,
        observaciones: comObservaciones.trim() || undefined, fechaRegistro,
        ventaId: reparto.length > 1 ? ventaId : undefined,
      }))
      setComercializaciones([...nuevas, ...comercializaciones])
      try {
        for (const n of nuevas) {
          const { error } = await supabase.from('comercializaciones').upsert({
            id: n.id, user_id: session.user.id, cosecha_id: n.cosechaId, fecha: n.fecha,
            cantidad: Number(n.cantidad), unidad: n.unidad, comprador: n.comprador || null,
            comprador_dni_nif: n.compradorDniNif || null,
            comprador_direccion: n.compradorDireccion || null, comprador_rgseaa: n.compradorRgseaa || null,
            num_albaran_factura: n.numAlbaranFactura || null, num_lote: n.numLote || null,
            venta_id: n.ventaId || null,
            observaciones: n.observaciones || null, fecha_registro: n.fechaRegistro,
          })
          if (error) { setComError('No se pudo guardar: ' + error.message); return }
        }
      } catch (e) { console.error('Error guardando comercialización:', e) }
    ...
    // Caso 3: alta nueva ligada directamente a una única cosecha (historial por parcela).
    if (!comFormCosechaId) return
    const cosecha = cosechas.find(c => c.id === comFormCosechaId)
    if (cosecha?.cantidad) {
      const yaComercializado = totalComercializado(comFormCosechaId)
      if (yaComercializado + Number(comCantidad) > Number(cosecha.cantidad)) {
        setComError(`La cantidad supera lo pendiente de comercializar (${formatCantidadEs(Number(cosecha.cantidad) - yaComercializado)} ${cosecha.unidad})`)
        return
      }
    ...
    const nueva: Comercializacion = {
      id: String(Date.now()),
      cosechaId: comFormCosechaId, fecha: comFecha, cantidad: comCantidad.trim(), unidad: comUnidad,
      comprador: comComprador.trim() || undefined, compradorDniNif: comCompradorDniNif.trim() || undefined,
      compradorDireccion: comCompradorDireccion.trim() || undefined, compradorRgseaa: comCompradorRgseaa.trim() || undefined,
      numAlbaranFactura: comNumAlbaranFactura.trim() || undefined, numLote: comNumLote.trim() || undefined,
      observaciones: comObservaciones.trim() || undefined,
    ...
    setComercializaciones([nueva, ...comercializaciones])
    try {
      const { error } = await supabase.from('comercializaciones').upsert({
        id: nueva.id, user_id: session.user.id, cosecha_id: nueva.cosechaId, fecha: nueva.fecha,
        cantidad: Number(nueva.cantidad), unidad: nueva.unidad, comprador: nueva.comprador || null,
  ...
  const eliminarComercializacion = async (id: string) => {
    if (!confirm('¿Eliminar esta comercialización?')) return
    setComercializaciones(comercializaciones.filter(c => c.id !== id))
    try { await supabase.from('comercializaciones').delete().eq('id', id) } catch (e) { console.error(e) }
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100dvh', overflow: 'hidden' }} onClick={e => { if (menuAbierto && !(e.target as HTMLElement).closest('[data-menu]')) setMenuAbierto(false) }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', minHeight: isMobile ? 54 : 48 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: isMobile ? '0 10px' : '0 16px', flexShrink: 0 }}>
            ...
            <span style={{ fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 12, letterSpacing: '0.05em' }}>
              <span style={{ color: 'var(--green)', textShadow: 'var(--green-outline)' }}>K</span>
              <span style={{ color: '#16220F' }}>ULT</span>
              <span style={{ color: 'var(--green)', textShadow: 'var(--green-outline)' }}>A</span>
              <span style={{ color: '#16220F' }}>GRO</span>
            </span>
          </div>
            ...
            <div data-menu style={{ position: 'relative', flexShrink: 0 }}>
              ...
              {menuAbierto && (
                <div style={{ position: 'absolute', top: '100%', left: 0, zIndex: 6000, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '0 0 10px 10px', boxShadow: '0 8px 24px rgba(0,0,0,0.5)', minWidth: 220 }}>
                  {(['mapa', 'mis_parcelas', 'tratamientos', 'tareas', 'riego', 'cosechas', 'personal', 'equipos', 'permisos'] as Pestaña[]).filter(tab => puedeVerPestana(tab, misPermisos)).map(tab => (
                    <button key={tab} onClick={() => { setPestana(tab); setMenuAbierto(false) }}
                      style={{ display: 'block', width: '100%', padding: '15px 20px', border: 'none', background: pestana === tab ? 'rgba(31, 74, 40, 1)' : 'transparent', fontFamily: 'var(--mono)', fontSize: 14, fontWeight: pestana === tab ? 700 : 400, cursor: 'pointer', color: 'var(--text)', textAlign: 'left', borderLeft: `3px solid ${pestana === tab ? 'var(--green)' : 'transparent'}`, borderBottom: '1px solid var(--border)' }}
                    >
                      {tab === 'mapa' ? '🗺  MAPA' : tab === 'mis_parcelas' ? '📁  MIS PARCELAS' : tab === 'tratamientos' ? '🧪  TRATAMIENTOS' : tab === 'tareas' ? '📋  TAREAS' : tab === 'riego' ? '💧  RIEGO' : tab === 'cosechas' ? '🌾  COSECHAS' : tab === 'personal' ? '👷  PERSONAL' : tab === 'equipos' ? '🚜  EQUIPOS' : '🔐  PERMISOS'}
                    </button>
                  ))}
                </div>
              )}
          ...
          {isMobile ? (
            <span style={{ flex: 1, fontFamily: 'var(--mono)', fontSize: 13, fontWeight: 700, color: 'var(--green)', textShadow: 'var(--green-outline)', paddingLeft: 4 }}>
              {pestana === 'mapa' ? '🗺 MAPA' : pestana === 'mis_parcelas' ? '📁 MIS PARCELAS' : pestana === 'tratamientos' ? '🧪 TRATAMIENTOS' : pestana === 'tareas' ? '📋 TAREAS' : pestana === 'riego' ? '💧 RIEGO' : pestana === 'cosechas' ? '🌾 COSECHAS' : pestana === 'personal' ? '👷 PERSONAL' : pestana === 'equipos' ? '🚜 EQUIPOS' : '🔐 PERMISOS'}
            </span>
          ) : (
            <div style={{ display: 'flex', flex: 1 }}>
              {(['mapa', 'mis_parcelas', 'tratamientos', 'tareas', 'riego', 'cosechas', 'personal', 'equipos', 'permisos'] as Pestaña[]).filter(tab => puedeVerPestana(tab, misPermisos)).map(tab => (
                <button key={tab} onClick={() => setPestana(tab)}
                  style={{ padding: '12px 20px', border: 'none', background: 'transparent', fontFamily: 'var(--mono)', fontSize: 12, fontWeight: 700, cursor: 'pointer', color: 'var(--text)', borderBottom: `2px solid ${pestana === tab ? 'var(--green)' : 'transparent'}`, letterSpacing: '0.06em', transition: 'all 0.15s', whiteSpace: 'nowrap' }}
                >
                  {tab === 'mapa' ? '🗺 MAPA' : tab === 'mis_parcelas' ? '📁 MIS PARCELAS' : tab === 'tratamientos' ? '🧪 TRATAMIENTOS' : tab === 'tareas' ? '📋 TAREAS' : tab === 'riego' ? '💧 RIEGO' : tab === 'cosechas' ? '🌾 COSECHAS' : tab === 'personal' ? '👷 PERSONAL' : tab === 'equipos' ? '🚜 EQUIPOS' : '🔐 PERMISOS'}
                </button>
              ))}
              ...
              <>
                ...
                <div style={{ position: 'absolute', top: '100%', right: 0, marginTop: 6, zIndex: 6000, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, boxShadow: '0 8px 24px rgba(0,0,0,0.5)', minWidth: 220, overflow: 'hidden' }}>
                  ...
                  <button onClick={() => { setMenuUsuarioAbierto(false); cerrarSesion() }}
                    style={{ display: 'block', width: '100%', padding: '12px 14px', border: 'none', background: 'transparent', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 12, fontWeight: 700, cursor: 'pointer', textAlign: 'left' }}>
                    ⏻ CERRAR SESIÓN
                  </button>
                </div>
              </>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 3000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, width: '100%', maxWidth: 460, maxHeight: '90vh', overflow: 'auto', display: 'flex', flexDirection: 'column' }}>
            <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontFamily: 'var(--mono)', fontSize: 13, fontWeight: 700, color: 'var(--green)', textShadow: 'var(--green-outline)' }}>COMPROBAR ROMA / ROPO</span>
              <button onClick={() => setShowVerifRomaRopo(false)} style={{ background: 'transparent', border: 'none', color: 'var(--text)', cursor: 'pointer', fontSize: 18 }}>X</button>
            </div>
            <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 12px', borderRadius: 6, background: 'var(--green-dim)', border: '1px solid rgba(31, 74, 40,0.3)' }}>
                <span style={{ fontFamily: 'var(--mono)', fontSize: 11, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Registro:</span>
                <span style={{ fontFamily: 'var(--mono)', fontSize: 13, fontWeight: 700, color: 'var(--green)', textShadow: 'var(--green-outline)' }}>{verifRegistro.toUpperCase()}</span>
              </div>
              <div>
              ...
              {verifError && <div style={{ padding: '8px 12px', borderRadius: 6, background: 'rgba(110, 36, 23,0.08)', border: '1px solid rgba(110, 36, 23,0.2)', color: '#6E2417', fontSize: 12, fontFamily: 'var(--mono)' }}>{verifError}</div>}
              {verifResultado && (
                <div style={{ padding: '10px 12px', borderRadius: 6, background: 'var(--surface2)', border: '1px solid var(--border)', fontSize: 12, fontFamily: 'var(--mono)', color: 'var(--text)' }}>
                  {verifResultado.encontrados === 0 ? (
                    ...
                    <div>
                      <div style={{ marginBottom: 8, color: 'var(--green)', textShadow: 'var(--green-outline)' }}>
                      ...
                      {verifResultado.total_bruto_mapa != null && verifResultado.total_bruto_mapa > verifResultado.encontrados && (
                        <div style={{ marginBottom: 8, color: '#5C3F08', fontSize: 11 }}>
                      ...
                      {verifResultado.registros?.map((r: any, i: number) => (
                        <div key={i} style={{ padding: '8px 0', borderTop: i > 0 ? '1px solid var(--border)' : 'none' }}>
                          ...
                          {r.requiere_actualizacion_ccaa && (
                            <div style={{ marginTop: 4, color: '#6E2417' }}>⚠ Requiere actualizar datos en su Comunidad Autónoma</div>
                          )}
                          {verifRegistro === 'ropo' && (
                            <button type="button" onClick={() => aplicarResultadoROPO(r)} style={{ marginTop: 8, width: '100%', padding: '8px', borderRadius: 6, background: 'var(--green)', border: 'none', color: 'var(--text)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 11, cursor: 'pointer' }}>
                              ✅ USAR ESTOS DATOS
                            </button>
                          )}
                          {verifRegistro === 'roma' && (
                            <button type="button" onClick={() => aplicarResultadoROMA(r)} style={{ marginTop: 8, width: '100%', padding: '8px', borderRadius: 6, background: 'var(--green)', border: 'none', color: 'var(--text)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 11, cursor: 'pointer' }}>
                              ✅ USAR ESTOS DATOS
                            </button>
                          )}
                        </div>
            ...
            <div style={{ padding: '14px 20px', borderTop: '1px solid var(--border)', display: 'flex', gap: 10 }}>
              <button onClick={comprobarRomaRopo} disabled={verifLoading} style={{ flex: 1, padding: '11px', borderRadius: 8, background: 'var(--green)', border: 'none', color: 'var(--text)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 13, cursor: verifLoading ? 'default' : 'pointer', opacity: verifLoading ? 0.6 : 1 }}>
                {verifLoading ? 'CONSULTANDO...' : 'COMPROBAR'}
              </button>
              <button onClick={() => setShowVerifRomaRopo(false)} style={{ flex: 1, padding: '11px', borderRadius: 8, background: 'var(--surface2)', border: '1px solid var(--border)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 13, cursor: 'pointer' }}>CERRAR</button>
            </div>
          </div>
        </div>
      )}
      ...
      {/* - CONTENIDO - */}
      <div style={{ flex: (pestana === 'tratamientos' || pestana === 'cosechas') ? '0 0 0px' : 1, overflow: 'hidden', display: 'flex' }}>
          ...
          <div style={{ display: 'flex', width: '100%', height: '100%', position: 'relative' }}>
            ...
            {isMobile && (
              ...
              >
                ...
                {parcGeojson && (
                  <span style={{ fontFamily: 'var(--mono)', fontSize: 11, fontWeight: 700, color: 'var(--green)', textShadow: 'var(--green-outline)' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: isMobile ? '0 16px 16px' : '0', flexDirection: 'column', gap: 14, display: panelAbierto || !isMobile ? 'flex' : 'none' }}>
              ...
              <section>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 10 }}>
                  ...
                  <span style={{ fontSize: 11, fontFamily: 'var(--mono)', color: 'var(--green)', textShadow: 'var(--green-outline)', letterSpacing: '0.08em' }}>SELECCIONAR PARCELA</span>
                </div>

                <button onClick={() => { const next = !seleccionando; setSeleccionando(next); setMododibujo(false); setAnadiendoRecinto(false); if (isMobile && next) setPanelAbierto(false) }} style={{ width: '100%', padding: '10px', borderRadius: 8, background: seleccionando ? 'var(--green)' : 'var(--kampo-surface-deep)', border: `1px solid ${seleccionando ? 'var(--green)' : 'var(--border)'}`, color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 12, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, transition: 'all 0.2s' }}>
                  {estado === 'cargando_parcela' ? <><span className="spinner"/> BUSCANDO...</> : seleccionando ? '✕ CANCELAR' : '⊕ CLIC EN EL MAPA'}
                </button>

                {seleccionando && <div style={{ marginTop: 8, padding: '8px 10px', borderRadius: 6, background: 'rgba(15, 74, 112,0.06)', border: '1px solid rgba(15, 74, 112,0.2)', fontSize: 11, color: 'var(--blue)', fontFamily: 'var(--mono)' }}>👆 Haz clic sobre una parcela en el mapa</div>}
                ...
                <button onClick={() => { setMododibujo(m => !m); setSeleccionando(false); setAnadiendoRecinto(false) }} style={{ width: '100%', padding: '9px', borderRadius: 8, background: mododibujo ? 'rgba(15, 74, 112, 1)' : 'var(--kampo-surface-deep)', border: `1px solid ${mododibujo ? 'var(--blue)' : 'var(--border)'}`, color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 11, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, transition: 'all 0.2s' }}>
                  {mododibujo ? '✕ CANCELAR DIBUJO' : '✏ DIBUJAR PARCELA A MANO'}
                </button>
                {mododibujo && <div style={{ marginTop: 6, padding: '8px 10px', borderRadius: 6, background: 'rgba(15, 74, 112,0.06)', border: '1px solid rgba(15, 74, 112,0.2)', fontSize: 10, color: 'var(--blue)', fontFamily: 'var(--mono)', lineHeight: 1.6 }}>✏ Haz clic para dibujar . Doble clic para cerrar</div>}
                ...
                <div style={{ display: 'flex', gap: 6 }}>
                  <input
                    ...
                    style={{ flex: 1, minWidth: 0, boxSizing: 'border-box', background: 'var(--kampo-surface-deep)', border: '1px solid var(--border)', borderRadius: 8, padding: '9px 10px', color: 'var(--text)', fontSize: 12, fontFamily: 'var(--mono)', outline: 'none' }}
                  />
                  <button onClick={buscarPorReferenciaSigpac} disabled={estado === 'cargando_parcela'} style={{ flexShrink: 0, padding: '9px 14px', borderRadius: 8, background: 'var(--kampo-surface-deep)', border: '1px solid var(--border)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 12, fontWeight: 700, cursor: estado === 'cargando_parcela' ? 'wait' : 'pointer' }}>
                    🔎
                  </button>
                </div>
                {errorSigpacRef && <div style={{ marginTop: 6, padding: '8px 10px', borderRadius: 6, background: 'rgba(110, 36, 23,0.1)', border: '1px solid rgba(110, 36, 23,0.3)', fontSize: 10, color: '#6E2417', fontFamily: 'var(--mono)' }}>{errorSigpacRef}</div>}

                {fincaVistaEnMapa && (
                  <div style={{ marginTop: 8, padding: '10px', borderRadius: 6, background: 'rgba(110, 36, 23,0.08)', border: '1px solid rgba(110, 36, 23,0.25)', fontSize: 11, fontFamily: 'var(--mono)' }}>
                    <div style={{ color: '#6E2417', fontWeight: 700, marginBottom: 6 }}>🏡 {fincaVistaEnMapa.nombre}</div>
                    <div style={{ color: 'var(--muted)', lineHeight: 1.8 }}>
                      ...
                      {misPermisos['parcelas.ubicaciones.ver'] && (
                        <div>Ubicaciones: <span style={{ color: 'var(--blue)' }}>💧 {ubicaciones.filter(u => u.fincaId === fincaVistaEnMapa.id).length}</span></div>
                      )}
                    </div>
                  </div>
                )}
                ...
                {parcelaInfo && (
                  <div style={{ marginTop: 8, padding: '10px', borderRadius: 6, background: 'var(--green-dim)', border: '1px solid rgba(31, 74, 40,0.2)', fontSize: 11, fontFamily: 'var(--mono)' }}>
                    <div style={{ color: 'var(--green)', textShadow: 'var(--green-outline)', fontWeight: 700, marginBottom: 6 }}>
                ...
                {parcelaInfo && parcelaInfo.origen !== 'dibujado_mano' && !mododibujo && (
                  <button
                    ...
                    style={{ width: '100%', marginTop: 8, padding: '9px', borderRadius: 8, background: anadiendoRecinto ? 'rgba(15, 74, 112, 1)' : 'var(--kampo-surface-deep)', border: `1px solid ${anadiendoRecinto ? 'var(--blue)' : 'var(--border)'}`, color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 11, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, transition: 'all 0.2s' }}
                  >
                    {anadiendoRecinto ? '✕ TERMINAR DE AÑADIR' : '+ AÑADIR RECINTO'}
                  </button>
                )}
                {anadiendoRecinto && <div style={{ marginTop: 6, padding: '8px 10px', borderRadius: 6, background: 'rgba(15, 74, 112,0.06)', border: '1px solid rgba(15, 74, 112,0.2)', fontSize: 10, color: 'var(--blue)', fontFamily: 'var(--mono)', lineHeight: 1.6 }}>👆 Pulsa otro recinto en el mapa. Puedes seguir pulsando para añadir varios</div>}

                {parcGeojson && (
                  <div style={{ display: 'flex', gap: 6, marginTop: 6 }}>
                    <button onClick={deseleccionar} style={{ flex: 1, padding: '7px', borderRadius: 6, background: 'rgba(110, 36, 23, 1)', border: '1px solid rgba(110, 36, 23, 1)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 10, fontWeight: 700, cursor: 'pointer' }}>
                      ✕ DESELECCIONAR
                    </button>
                    <button onClick={abrirFormularioGuardar} style={{ flex: 1, padding: '7px', borderRadius: 6, background: 'rgba(31, 74, 40, 1)', border: '1px solid rgba(31, 74, 40, 1)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 10, fontWeight: 700, cursor: 'pointer' }}>
                      💾 GUARDAR
                    </button>
                  </div>
                )}
                ...
                <>
                  ...
                  <section>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 10 }}>
                      ...
                      <span style={{ fontSize: 11, fontFamily: 'var(--mono)', color: 'var(--green)', textShadow: 'var(--green-outline)', letterSpacing: '0.08em' }}>PERIODO</span>
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6, marginBottom: 8 }}>
                      {[{ label: 'Desde', val: fechaInicio, set: setFechaInicio }, { label: 'Hasta', val: fechaFin, set: setFechaFin }].map(f => (
                        <div key={f.label}>
                          ...
                          <input type="date" value={f.val} onChange={e => f.set(e.target.value)} style={{ width: '100%', background: 'var(--kampo-surface-deep)', border: '1px solid var(--border)', borderRadius: 5, padding: '5px 6px', color: 'var(--text)', fontSize: 11, fontFamily: 'var(--mono)', outline: 'none' }}/>
                        </div>
                      ))}
                    </div>
                    <button onClick={buscarImagenes} disabled={cargando} style={{ width: '100%', padding: '8px', borderRadius: 6, background: 'rgba(15, 74, 112, 1)', border: '1px solid var(--blue)', color: 'var(--text)', fontWeight: 700, fontSize: 11, fontFamily: 'var(--mono)', cursor: cargando ? 'wait' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
                    ...
                    {productos.length > 0 && (
                      <div style={{ marginTop: 8 }}>
                        ...
                        <select value={productoSel} onChange={e => { setProductoSel(e.target.value); resetear() }} style={{ width: '100%', background: 'var(--kampo-surface-deep)', border: '1px solid var(--border)', borderRadius: 5, padding: '5px 6px', color: 'var(--text)', fontSize: 10, fontFamily: 'var(--mono)', outline: 'none' }}>
                        ...
                        <button onClick={verImagenRGB} disabled={cargando} style={{ width: '100%', marginTop: 8, padding: '9px', borderRadius: 6, background: modoVista === 'rgb' ? 'rgba(15, 74, 112, 1)' : 'var(--kampo-surface-deep)', border: `1px solid var(--blue)`, color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 11, fontWeight: 700, cursor: cargando ? 'wait' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, transition: 'all 0.2s' }}>
                          {estado === 'cargando_rgb' ? <><span className="spinner" style={{ borderTopColor: 'var(--blue)' }}/> CARGANDO...</> : modoVista === 'rgb' ? '🛰 IMAGEN CARGADA' : '🛰 VER IMAGEN REAL'}
                        </button>
                      </div>
                    )}
                  </section>
                ...
                <>
                  ...
                  <section>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 10 }}>
                      ...
                      <span style={{ fontSize: 11, fontFamily: 'var(--mono)', color: 'var(--green)', textShadow: 'var(--green-outline)', letterSpacing: '0.08em' }}>MAPA NDVI</span>
                    </div>
                    <button onClick={calcularZonasNDVI} disabled={cargando} style={{ width: '100%', padding: '11px', borderRadius: 8, background: cargando ? 'var(--kampo-surface-deep)' : 'rgba(15, 74, 112, 1)', border: '1px solid var(--blue)', color: 'var(--text)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 13, cursor: cargando ? 'wait' : 'pointer', letterSpacing: '0.08em', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, transition: 'all 0.2s' }}>
                    ...
                    {zonasConSup.length > 0 && (
                      <div style={{ marginTop: 12 }}>
                        <div style={{ fontSize: 9, color: 'var(--green)', textShadow: 'var(--green-outline)', fontFamily: 'var(--mono)', marginBottom: 8, letterSpacing: '0.08em', fontWeight: 700 }}>ZONAS NDVI . SUPERFICIE</div>
                        {zonasConSup.map(z => {
                        ...
                        <div style={{ fontSize: 9, color: 'var(--muted)', fontFamily: 'var(--mono)', marginTop: 6, textAlign: 'right' }}>
                          Total: <span style={{ color: 'var(--green)', textShadow: 'var(--green-outline)', fontWeight: 700 }}>{parcelaSupHa.toFixed(4)} ha</span>
                        </div>
                      </div>
                    )}
                  </section>
                ...
                <>
                  ...
                  <section>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 10 }}>
                      ...
                      <span style={{ fontSize: 11, fontFamily: 'var(--mono)', color: 'var(--green)', textShadow: 'var(--green-outline)', letterSpacing: '0.08em' }}>CÁLCULO DE PRODUCCIÓN</span>
                    </div>
                    ...
                    <div style={{ display: 'flex', gap: 6, marginBottom: 10 }}>
                      <select id="zona-select" defaultValue="" style={{ flex: 1, background: 'var(--kampo-surface-deep)', border: '1px solid var(--border)', borderRadius: 5, padding: '7px 8px', color: 'var(--text)', fontSize: 11, fontFamily: 'var(--mono)', outline: 'none' }}>
                      ...
                      <input id="kg-input" type="number" min="0" placeholder="kg/ha" style={{ width: 90, background: 'var(--kampo-surface-deep)', border: '1px solid var(--border)', borderRadius: 5, padding: '7px 8px', color: 'var(--text)', fontSize: 11, fontFamily: 'var(--mono)', outline: 'none' }}/>
                      <button onClick={() => {
                      ...
                      }} style={{ padding: '7px 10px', borderRadius: 5, background: 'var(--green)', border: 'none', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 11, fontWeight: 700, cursor: 'pointer' }}>✓</button>
                    </div>
                    {Object.keys(kgHaCalculado).length > 0 && (
                      <div style={{ background: 'var(--kampo-surface-deep)', border: '1px solid var(--border)', borderRadius: 6, padding: '8px 10px', marginBottom: 8 }}>
                          ...
                          {zonasConSup.map(z => {
                            ...
                            return (
                              <div key={z.zona} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                                ...
                                <span style={{ fontSize: 9, fontFamily: 'var(--mono)', color: esRef ? 'var(--green)' : 'var(--muted)', textShadow: esRef ? 'var(--green-outline)' : 'none' }}>
                                  Z{z.zona}: <span style={{ color: esRef ? 'var(--green)' : 'var(--text)', textShadow: esRef ? 'var(--green-outline)' : 'none', fontWeight: esRef ? 700 : 400 }}>{val}</span>
                                </span>
                              </div>
                            )
                          })}
                        </div>
                        <button onClick={() => setKgPorHa({})} style={{ marginTop: 6, fontSize: 9, color: 'var(--text)', fontFamily: 'var(--mono)', background: 'transparent', border: 'none', cursor: 'pointer', textDecoration: 'underline' }}>Borrar valores</button>
                      </div>
                    )}
                    <button onClick={() => calcularProduccion(kgHaCalculado)} disabled={Object.keys(kgHaCalculado).length === 0} style={{ width: '100%', padding: '10px', borderRadius: 8, background: Object.keys(kgHaCalculado).length > 0 ? 'var(--green)' : 'var(--kampo-surface-deep)', border: 'none', color: 'var(--text)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 12, cursor: Object.keys(kgHaCalculado).length > 0 ? 'pointer' : 'not-allowed', letterSpacing: '0.06em', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
                      🌾 CALCULAR PRODUCCIÓN
                    </button>
                  </section>
                </>
              ...
              {produccion && (
                <>
                  <hr style={{ borderColor: 'var(--border)', borderWidth: '0 0 1px 0' }} />
                  <section>
                    <div style={{ fontSize: 11, fontFamily: 'var(--mono)', color: 'var(--green)', textShadow: 'var(--green-outline)', letterSpacing: '0.08em', marginBottom: 10 }}>ESTIMACIÓN DE COSECHA</div>
                    ...
                    <div style={{ marginTop: 10, padding: '12px', borderRadius: 8, background: 'var(--green-dim)', border: '1px solid rgba(31, 74, 40,0.3)' }}>
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5, fontSize: 11, fontFamily: 'var(--mono)' }}>
                        ...
                        <span style={{ color: 'var(--green)', textShadow: 'var(--green-outline)', fontWeight: 700 }}>{Math.round(produccion.total_kg).toLocaleString()} kg</span>
                      </div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, fontFamily: 'var(--mono)', marginBottom: 5 }}>
                        ...
                        <span style={{ color: 'var(--green)', textShadow: 'var(--green-outline)', fontWeight: 700 }}>{produccion.total_ha > 0 ? Math.round(produccion.total_kg / produccion.total_ha).toLocaleString() : 0} kg/ha</span>
                      </div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 14, fontFamily: 'var(--mono)', fontWeight: 700, marginTop: 8, paddingTop: 8, borderTop: '1px solid rgba(31, 74, 40,0.2)' }}>
                        <span style={{ color: 'var(--muted)' }}>TOTAL TONELADAS</span>
                        <span style={{ color: 'var(--green)', textShadow: 'var(--green-outline)', fontSize: 18 }}>{produccion.total_toneladas.toFixed(3)} t</span>
                      </div>
                    </div>
                  </section>
                </>
              ...
              {parcGeojson && (
                <>
                  ...
                  <section>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 10 }}>
                      <span style={{ fontSize: 16 }}>🗺</span>
                      <span style={{ fontSize: 11, fontFamily: 'var(--mono)', color: 'var(--green)', textShadow: 'var(--green-outline)', letterSpacing: '0.08em', fontWeight: 700 }}>CURVAS DE NIVEL</span>
                    </div>
                    ...
                    <button onClick={calcularContornos} disabled={calculandoContornos} style={{ width: '100%', padding: '10px', borderRadius: 8, background: calculandoContornos ? 'var(--kampo-surface-deep)' : 'rgba(15, 74, 112, 1)', border: '1px solid var(--blue)', color: 'var(--text)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 12, cursor: calculandoContornos ? 'wait' : 'pointer', letterSpacing: '0.06em', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, transition: 'all 0.2s' }}>
                      {calculandoContornos ? <><span className="spinner" style={{ borderTopColor: 'var(--blue)' }}/> CALCULANDO MDT...</> : '🗺 GENERAR CURVAS DE NIVEL'}
                    </button>
                    <button onClick={calcularMdtColor} disabled={calculandoMdtColor} style={{ width: '100%', marginTop: 8, padding: '10px', borderRadius: 8, background: calculandoMdtColor ? 'var(--kampo-surface-deep)' : 'rgba(15, 74, 112, 1)', border: '1px solid var(--blue)', color: 'var(--text)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 12, cursor: calculandoMdtColor ? 'wait' : 'pointer', letterSpacing: '0.06em', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, transition: 'all 0.2s' }}>
                        ...
                        <div style={{ marginTop: 10 }}>
                          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 5 }}>
                            {[
                              ...
                              <div key={s.k} style={{ background: 'var(--kampo-surface-deep)', border: '1px solid var(--border)', borderRadius: 6, padding: '7px 10px' }}>
                          ...
                          {mapaActivo === 'contornos' && Array.isArray(statsActivas.leyenda) && statsActivas.leyenda.length > 0 && (
                            <div style={{ marginTop: 8, background: 'var(--kampo-surface-deep)', border: '1px solid var(--border)', borderRadius: 6, padding: '8px 10px' }}>
                          ...
                          <button onClick={toggleMapaMdt} style={{ width: '100%', marginTop: 8, padding: '8px', borderRadius: 6, background: mapaVisible ? 'rgba(15, 74, 112, 1)' : 'var(--kampo-surface-deep)', border: `1px solid ${mapaVisible ? 'var(--blue)' : 'var(--border)'}`, color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 10, cursor: 'pointer', fontWeight: 600 }}>
              ...
              <hr style={{ borderColor: 'var(--border)', borderWidth: '0 0 1px 0' }} />
              <section>
                <button onClick={() => setMostrarHistorico(h => !h)} style={{ width: '100%', padding: '8px', borderRadius: 6, background: 'transparent', border: '1px solid var(--border)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 10, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between', letterSpacing: '0.06em' }}>
                ...
                {mostrarHistorico && (
                  <div style={{ marginTop: 8 }}>
                    {historico.length === 0
                      ...
                      : <>
                        <button onClick={async () => { if (confirm('Borrar todo el histórico?')) { setHistorico([]); if (session) { try { await supabase.from('historico').delete().eq('user_id', session.user.id) } catch (e) { console.error(e) } } } }} style={{ fontSize: 9, color: 'var(--text)', fontFamily: 'var(--mono)', background: 'transparent', border: 'none', cursor: 'pointer', marginBottom: 8, textDecoration: 'underline' }}>Borrar histórico</button>
                        {historico.map((entrada: any) => (
                          <div key={entrada.id} style={{ background: 'var(--kampo-surface-deep)', border: '1px solid var(--border)', borderRadius: 6, padding: '8px 10px', marginBottom: 6 }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                              <span style={{ fontSize: 10, color: 'var(--green)', textShadow: 'var(--green-outline)', fontFamily: 'var(--mono)', fontWeight: 700 }}>{entrada.fecha}</span>
                            ...
                            {entrada.geojson && (
                              ...
                              }} style={{ width: '100%', padding: '5px', borderRadius: 5, background: 'rgba(31, 74, 40, 1)', border: '1px solid rgba(31, 74, 40, 1)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 10, cursor: 'pointer', letterSpacing: '0.04em' }}>
                                ▶ CARGAR ANÁLISIS
                              </button>
                            )}
                          </div>
              ...
              {error && <div style={{ padding: '8px 10px', borderRadius: 6, background: 'rgba(110, 36, 23,0.08)', border: '1px solid rgba(110, 36, 23,0.2)', color: '#6E2417', fontSize: 11, fontFamily: 'var(--mono)' }}>⚠ {error}</div>}
            ...
            <div style={{ flex: 1, position: 'relative' }}>
              <MapView
                ...
                indiceColor={(parcelaVistaEnMapa || fincaVistaEnMapa) ? 'transparent' : modoVista === 'rgb' ? '#5C3F08' : '#1F4A28'}
                parcelaVistaColor={fincaVistaEnMapa ? '#6E2417' : parcelaVistaEnMapa ? '#3F5233' : undefined}
                seleccionando={seleccionando || anadiendoRecinto}
                mododibujo={mododibujo}
                onMododibujoCambiado={setMododibujo}
                puntosMarcador={(fincaVistaEnMapa && misPermisos['parcelas.ubicaciones.ver']) ? ubicaciones.filter(u => u.fincaId === fincaVistaEnMapa.id).map(u => ({ id: u.id, nombre: u.nombre, lat: u.lat, lng: u.lng })) : undefined}
              />
              <div style={{ position: 'absolute', top: 12, right: 12, zIndex: 1000, fontFamily: 'var(--mono)', fontSize: 11, background: 'rgba(225, 231, 211,0.92)', border: '1px solid var(--border)', backdropFilter: 'blur(8px)', borderRadius: 6, padding: '6px 12px', display: 'flex', alignItems: 'center', gap: 8 }}>
                {fincaVistaEnMapa && <span style={{ color: '#6E2417', fontWeight: 700 }}>🏡 FINCA: {fincaVistaEnMapa.nombre}</span>}
                ...
                {!parcelaVistaEnMapa && !fincaVistaEnMapa && modoVista === 'zonas' && <span style={{ color: 'var(--green)', textShadow: 'var(--green-outline)', fontWeight: 700 }}>🌿 MAPA NDVI</span>}
              </div>
              ...
              {(seleccionando || anadiendoRecinto) && (
                <div style={{ position: 'absolute', bottom: 40, left: '50%', transform: 'translateX(-50%)', zIndex: 1000, pointerEvents: 'none', background: 'rgba(15, 74, 112,0.1)', border: '1px solid var(--blue)', backdropFilter: 'blur(8px)', borderRadius: 8, padding: '10px 20px', fontFamily: 'var(--mono)', fontSize: 12, color: 'var(--blue)', letterSpacing: '0.06em' }}>
        ...
        {pestana === 'mis_parcelas' && (
          <div style={{ display: 'flex', width: '100%', height: '100%', overflow: 'hidden' }}>
            ...
            {formularioVisible && (
              <div style={{ width: isMobile ? '100%' : 480, height: '100%', background: 'var(--surface)', borderRight: isMobile ? 'none' : '1px solid var(--border)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)', background: 'var(--surface2)' }}>
                  <div style={{ fontFamily: 'var(--mono)', fontSize: 13, fontWeight: 700, color: 'var(--green)', textShadow: 'var(--green-outline)', letterSpacing: '0.06em' }}>
                ...
                <div style={{ flex: 1, overflow: 'auto', padding: 20, display: 'flex', flexDirection: 'column', gap: 16 }}>
                  ...
                  {/* Info parcela */}
                  {parcelaInfo && (
                    <div style={{ padding: '8px 12px', borderRadius: 6, background: 'var(--green-dim)', border: '1px solid rgba(31, 74, 40,0.2)', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)' }}>
                      {parcelaInfo.origen === 'dibujado_mano'
                        ? parcelaInfo.detectandoSigpac
                  ...
                  {[
                    ...
                    <div key={f.key}>
                      <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.08em' }}>
                        {f.label} {f.obligatorio && <span style={{ color: '#6E2417' }}>*</span>}
                      </label>
                      <input
                        type="text"
                        value={f.value}
                        ...
                        style={{ width: '100%', background: 'var(--surface2)', border: `1px solid ${f.obligatorio && !f.value.trim() && formError ? '#6E2417' : 'var(--border)'}`, borderRadius: 6, padding: '9px 12px', color: 'var(--text)', fontSize: 13, fontFamily: 'var(--sans)', outline: 'none' }}
                      />
                    </div>
                  ))}
                  ...
                  {[
                    ...
                    <div key={f.label}>
                      ...
                      <select value={f.value} onChange={e => f.set(e.target.value as any)} style={{ width: '100%', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '9px 12px', color: 'var(--text)', fontSize: 13, fontFamily: 'var(--sans)', outline: 'none' }}>
                  ...
                  <div>
                    ...
                    {fincas.length === 0 ? (
                      ...
                      <select value={formFincaId} onChange={e => setFormFincaId(e.target.value)} style={{ width: '100%', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '9px 12px', color: 'var(--text)', fontSize: 13, fontFamily: 'var(--sans)', outline: 'none' }}>
                  ...
                  <div>
                    ...
                    {campanas.length === 0 ? (
                      ...
                      <select value={formCampanaId} onChange={e => setFormCampanaId(e.target.value)} style={{ width: '100%', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '9px 12px', color: 'var(--text)', fontSize: 13, fontFamily: 'var(--sans)', outline: 'none' }}>
                    ...
                    <div style={{ padding: '10px 12px', borderRadius: 8, background: 'rgba(255,180,84,0.08)', border: '1px solid rgba(255,180,84,0.3)' }}>
                      {!cultivoAnteriorGuardado ? (
                        <>
                          ...
                          <button onClick={abrirGuardarCultivoAnterior} style={{ padding: '7px 12px', borderRadius: 6, background: 'transparent', border: '1px solid #ffb454', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 11, fontWeight: 700, cursor: 'pointer' }}>
                            💾 Guardar cultivo anterior con campaña
                          </button>
                        </>
                      ) : (
                        <div style={{ fontSize: 11, fontFamily: 'var(--mono)', color: 'var(--green)', textShadow: 'var(--green-outline)' }}>
                  ...
                  {formError && <div style={{ padding: '8px 12px', borderRadius: 6, background: 'rgba(110, 36, 23,0.08)', border: '1px solid rgba(110, 36, 23,0.2)', color: '#6E2417', fontSize: 12, fontFamily: 'var(--mono)' }}>⚠ {formError}</div>}
                </div>

                {/* Botones */}
                <div style={{ padding: '16px 20px', borderTop: '1px solid var(--border)', display: 'flex', gap: 10 }}>
                  <button onClick={guardarParcela} style={{ flex: 1, padding: '12px', borderRadius: 8, background: 'var(--green)', border: 'none', color: 'var(--text)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 13, cursor: 'pointer', letterSpacing: '0.06em' }}>
                    💾 GUARDAR
                  </button>
                  <button onClick={cancelarFormulario} style={{ flex: 1, padding: '12px', borderRadius: 8, background: 'var(--surface2)', border: '1px solid var(--border)', color: 'var(--text)', fontFamily: 'var(--mono)', fontWeight: 600, fontSize: 13, cursor: 'pointer' }}>
                    CANCELAR
                  </button>
                </div>
              </div>
            ...
            <div style={{ flex: 1, height: '100%', overflow: 'auto', padding: 24, background: 'var(--bg)' }}>
                ...
                <div>
                  ...
                  {misPermisos['fincas.crear_editar'] && (
                    <button onClick={() => { setFincaEditar(null); setFNombre(''); setFDescripcion(''); setFError(''); setFormFinca(true) }}
                      style={{ padding: '5px 12px', borderRadius: 6, background: 'rgba(31, 74, 40, 1)', border: '1px solid rgba(31, 74, 40, 1)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 11, cursor: 'pointer', fontWeight: 700, marginRight: 8 }}>
                      + Crear finca
                    </button>
                  )}
                  {misPermisos['parcelas.crear_editar'] && (
                    <button onClick={() => setImportarExcelVisible(true)}
                      style={{ padding: '5px 12px', borderRadius: 6, background: 'rgba(15, 74, 112, 1)', border: '1px solid rgba(15, 74, 112, 1)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 11, cursor: 'pointer', fontWeight: 700 }}>
                      📥 Importar Excel
                    </button>
                  )}
                  <div style={{ marginTop: 8, display: 'flex', gap: 8 }}>
                    <button onClick={() => { setCampanaEditar(null); setCampNombre(''); setCampFechaInicio(''); setCampFechaFin(''); setCampError(''); setFormCampanaVisible(true) }}
                      style={{ padding: '5px 12px', borderRadius: 6, background: 'rgba(92, 63, 8, 1)', border: '1px solid rgba(92, 63, 8, 1)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 11, cursor: 'pointer', fontWeight: 700 }}>
                      + Crear campaña
                    </button>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                  ...
                  {filtroFinca && (
                    <button onClick={() => verFincaEnMapa(filtroFinca)}
                      style={{ padding: '5px 12px', borderRadius: 6, background: 'rgba(110, 36, 23, 1)', border: '1px solid rgba(110, 36, 23, 1)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 11, cursor: 'pointer', fontWeight: 700 }}>
                      🗺 Ver finca en mapa
                    </button>
                  )}
                </div>
                {fincas.length > 0 && (
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    <button onClick={() => setFiltroFinca('')}
                      style={{ padding: '4px 10px', borderRadius: 20, fontFamily: 'var(--mono)', fontSize: 10, cursor: 'pointer', background: filtroFinca === '' ? 'var(--green)' : 'var(--surface2)', color: 'var(--text)', border: '1px solid var(--border)', fontWeight: filtroFinca === '' ? 700 : 400 }}>
                      Todas
                    </button>
                    {fincas.map(f => (
                      <div key={f.id} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                        <button onClick={() => setFiltroFinca(filtroFinca === f.id ? '' : f.id)}
                          style={{ padding: '4px 10px', borderRadius: 20, fontFamily: 'var(--mono)', fontSize: 10, cursor: 'pointer', background: filtroFinca === f.id ? 'var(--green)' : 'var(--surface2)', color: 'var(--text)', border: '1px solid var(--border)', fontWeight: filtroFinca === f.id ? 700 : 400 }}>
                          {f.nombre}
                        </button>
                        {misPermisos['fincas.crear_editar'] && (
                          <button onClick={() => { setFincaEditar(f); setFNombre(f.nombre); setFDescripcion(f.descripcion); setFError(''); setFormFinca(true) }}
                            style={{ fontSize: 9, color: 'var(--text)', background: 'transparent', border: 'none', cursor: 'pointer' }}>✏</button>
                        )}
              ...
              {/* Gestión de ubicaciones (pozos, canales... — sección 2.2 del Cuaderno de Campo) */}
              {misPermisos['parcelas.ubicaciones.ver'] && (
                <div style={{ marginBottom: 20 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                    <span style={{ fontFamily: 'var(--mono)', fontSize: 11, fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Ubicaciones — pozos, canales... ({ubicaciones.length})</span>
                    {misPermisos['parcelas.ubicaciones.gestionar'] && (
                      <button onClick={iniciarNuevaUbicacion} disabled={fincas.length === 0}
                        style={{ padding: '5px 12px', borderRadius: 6, background: fincas.length === 0 ? 'var(--surface2)' : 'rgba(15, 74, 112, 1)', border: `1px solid ${fincas.length === 0 ? 'var(--border)' : 'rgba(15, 74, 112, 1)'}`, color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 11, cursor: fincas.length === 0 ? 'default' : 'pointer', fontWeight: 700 }}>
                        + Añadir ubicación
                      </button>
                    )}
                  </div>
                  {fincas.length === 0 ? (
                    <div style={{ fontSize: 11, color: 'var(--muted)', fontFamily: 'var(--mono)' }}>Crea una finca primero para poder añadir ubicaciones.</div>
                  ) : ubicaciones.length === 0 ? (
                    <div style={{ fontSize: 11, color: 'var(--muted)', fontFamily: 'var(--mono)' }}>Sin ubicaciones todavía. Sirven para rellenar la sección 2.2 del Cuaderno de Campo (puntos de captación de agua).</div>
                  ) : (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                      {ubicaciones.map(u => (
                        <div key={u.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, padding: '7px 10px', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6 }}>
                          <div style={{ minWidth: 0 }}>
                            <span style={{ fontFamily: 'var(--mono)', fontSize: 12, fontWeight: 700, color: 'var(--text)' }}>💧 {u.nombre}</span>
                            <span style={{ fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--muted)', marginLeft: 8 }}>
                              {fincas.find(f => f.id === u.fincaId)?.nombre || '—'} · {u.parcelaIds.length} parcela{u.parcelaIds.length !== 1 ? 's' : ''}
                            </span>
                          </div>
                          {misPermisos['parcelas.ubicaciones.gestionar'] && (
                            <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
                              <button onClick={() => editarUbicacion(u)} style={{ fontSize: 11, color: 'var(--text)', background: 'transparent', border: 'none', cursor: 'pointer' }}>✏</button>
                              <button onClick={() => eliminarUbicacion(u.id)} style={{ fontSize: 11, color: 'var(--text)', background: 'transparent', border: 'none', cursor: 'pointer' }}>✕</button>
                            </div>
                          )}
                        </div>
                      ))}
                ...
                <div style={{ marginBottom: 20 }}>
                  ...
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    <button onClick={() => setFiltroCampana('')}
                      style={{ padding: '4px 10px', borderRadius: 20, fontFamily: 'var(--mono)', fontSize: 10, cursor: 'pointer', background: filtroCampana === '' ? 'var(--amber)' : 'var(--surface2)', color: 'var(--text)', border: '1px solid var(--border)', fontWeight: filtroCampana === '' ? 700 : 400 }}>
                      Todas
                    </button>
                    {campanas.map(c => (
                      <div key={c.id} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                        <button onClick={() => setFiltroCampana(filtroCampana === c.id ? '' : c.id)}
                          style={{ padding: '4px 10px', borderRadius: 20, fontFamily: 'var(--mono)', fontSize: 10, cursor: 'pointer', background: filtroCampana === c.id ? 'var(--amber)' : 'var(--surface2)', color: 'var(--text)', border: '1px solid var(--border)', fontWeight: filtroCampana === c.id ? 700 : 400 }}>
                ...
                <div style={{ marginBottom: 20 }}>
                  ...
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    <button onClick={() => setFiltroCultivoParcelas('')}
                      style={{ padding: '4px 10px', borderRadius: 20, fontFamily: 'var(--mono)', fontSize: 10, cursor: 'pointer', background: filtroCultivoParcelas === '' ? 'var(--blue)' : 'var(--surface2)', color: 'var(--text)', border: '1px solid var(--border)', fontWeight: filtroCultivoParcelas === '' ? 700 : 400 }}>
                      Todos
                    </button>
                    {Array.from(new Set(misParcelas.map(p => p.cultivo))).sort().map(c => (
                      <button key={c} onClick={() => setFiltroCultivoParcelas(filtroCultivoParcelas === c ? '' : c)}
                        style={{ padding: '4px 10px', borderRadius: 20, fontFamily: 'var(--mono)', fontSize: 10, cursor: 'pointer', background: filtroCultivoParcelas === c ? 'var(--blue)' : 'var(--surface2)', color: 'var(--text)', border: '1px solid var(--border)', fontWeight: filtroCultivoParcelas === c ? 700 : 400 }}>
                        {c}
                      </button>
                    ))}
                  </div>
              ...
              {formFinca && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
                  <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, width: '100%', maxWidth: 400, padding: 24 }}>
                    <div style={{ fontFamily: 'var(--mono)', fontSize: 13, fontWeight: 700, color: 'var(--green)', textShadow: 'var(--green-outline)', marginBottom: 16 }}>{fincaEditar ? 'EDITAR FINCA' : 'CREAR FINCA'}</div>
                    ...
                    {fError && <div style={{ padding: '8px 12px', borderRadius: 6, background: 'rgba(110, 36, 23,0.08)', border: '1px solid rgba(110, 36, 23,0.2)', color: '#6E2417', fontSize: 12, fontFamily: 'var(--mono)', marginBottom: 12 }}>{fError}</div>}
                    <div style={{ display: 'flex', gap: 10 }}>
                      <button onClick={guardarFinca} style={{ flex: 1, padding: 11, borderRadius: 8, background: 'var(--green)', border: 'none', color: 'var(--text)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>GUARDAR</button>
                      <button onClick={() => { setFormFinca(false); setFincaEditar(null); setFNombre(''); setFDescripcion('') }} style={{ flex: 1, padding: 11, borderRadius: 8, background: 'var(--surface2)', border: '1px solid var(--border)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 13, cursor: 'pointer' }}>CANCELAR</button>
                    </div>
                  </div>
                </div>
              )}

              {/* Modal Añadir/Editar UBICACIÓN — paso 1: elegir finca */}
              {formUbicacionPaso === 'finca' && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
                  <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, width: '100%', maxWidth: 400, padding: 24 }}>
                    <div style={{ fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--muted)', marginBottom: 4, letterSpacing: '0.06em' }}>PASO 1 DE 3</div>
                    <div style={{ fontFamily: 'var(--mono)', fontSize: 13, fontWeight: 700, color: 'var(--blue)', marginBottom: 16 }}>💧 AÑADIR UBICACIÓN — FINCA</div>
                    <div style={{ marginBottom: 16 }}>
                      <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase' }}>¿A qué finca pertenece? *</label>
                      <select value={ubicacionFincaId} onChange={e => { setUbicacionFincaId(e.target.value); setUbicacionParcelaIds([]) }}
                        style={{ width: '100%', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '9px 12px', color: 'var(--text)', fontSize: 13, fontFamily: 'var(--sans)', outline: 'none' }}>
                        <option value="">Selecciona una finca</option>
                        {fincas.map(f => <option key={f.id} value={f.id}>{f.nombre}</option>)}
                      </select>
                      <div style={{ fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginTop: 6 }}>A continuación se abrirá el mapa sobre esta finca para marcar el punto (pozo, canal, balsa...).</div>
                    </div>
                    {ubicacionError && <div style={{ padding: '8px 12px', borderRadius: 6, background: 'rgba(110, 36, 23,0.08)', border: '1px solid rgba(110, 36, 23,0.2)', color: '#6E2417', fontSize: 12, fontFamily: 'var(--mono)', marginBottom: 12 }}>{ubicacionError}</div>}
                    <div style={{ display: 'flex', gap: 10 }}>
                      <button onClick={continuarUbicacionAMapa} style={{ flex: 1, padding: 11, borderRadius: 8, background: 'var(--blue)', border: 'none', color: 'var(--text)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>CONTINUAR →</button>
                      <button onClick={cerrarFormUbicacion} style={{ flex: 1, padding: 11, borderRadius: 8, background: 'var(--surface2)', border: '1px solid var(--border)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 13, cursor: 'pointer' }}>CANCELAR</button>
                    </div>
                  </div>
                </div>
              )}

              {/* Modal Añadir/Editar UBICACIÓN — paso 2: marcar el punto en el mapa */}
              {formUbicacionPaso === 'mapa' && (
                <div style={{ position: 'fixed', inset: 0, background: 'var(--bg)', zIndex: 2000 }}>
                  <div style={{ position: 'absolute', top: 12, left: 12, right: 12, zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, flexWrap: 'wrap' }}>
                    <button onClick={() => setFormUbicacionPaso('finca')}
                      style={{ padding: '8px 14px', borderRadius: 8, background: 'rgba(225, 231, 211, 1)', border: '1px solid var(--border)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 12, cursor: 'pointer', fontWeight: 700 }}>
                      ← Atrás
                    </button>
                    <div style={{ padding: '8px 14px', borderRadius: 8, background: 'rgba(15, 74, 112,0.12)', border: '1px solid var(--blue)', color: 'var(--blue)', fontFamily: 'var(--mono)', fontSize: 12, fontWeight: 700 }}>
                      👆 Toca el mapa para marcar el punto — {fincas.find(f => f.id === ubicacionFincaId)?.nombre}
                    </div>
                  </div>
                  <UbicacionMapModal
                    parcelas={misParcelas.filter(p => p.fincaId === ubicacionFincaId).map(p => ({ id: p.id, nombre: formatCultivoVariedad(p.cultivo, p.variedad), geojson: p.geojson }))}
                    punto={ubicacionPunto}
                    onPuntoSeleccionado={(lat, lng) => setUbicacionPunto({ lat, lng })}
                  />
                  <div style={{ position: 'absolute', bottom: 20, left: '50%', transform: 'translateX(-50%)', zIndex: 1000 }}>
                    <button onClick={confirmarPuntoUbicacion} disabled={!ubicacionPunto}
                      style={{ padding: '12px 28px', borderRadius: 10, background: ubicacionPunto ? 'var(--green)' : 'var(--surface2)', border: 'none', color: 'var(--text)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 13, cursor: ubicacionPunto ? 'pointer' : 'default', letterSpacing: '0.06em' }}>
                      {ubicacionPunto ? '✓ CONFIRMAR PUNTO →' : 'MARCA UN PUNTO EN EL MAPA'}
                    </button>
                  </div>
                </div>
              )}

              {/* Modal Añadir/Editar UBICACIÓN — paso 3: nombre y parcelas asociadas */}
              {formUbicacionPaso === 'detalles' && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
                  <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, width: '100%', maxWidth: 440, padding: 24, maxHeight: '90vh', overflow: 'auto' }}>
                    <div style={{ fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--muted)', marginBottom: 4, letterSpacing: '0.06em' }}>PASO 3 DE 3</div>
                    <div style={{ fontFamily: 'var(--mono)', fontSize: 13, fontWeight: 700, color: 'var(--blue)', marginBottom: 16 }}>💧 {ubicacionEditar ? 'EDITAR UBICACIÓN' : 'AÑADIR UBICACIÓN'} — DETALLES</div>
                    <div style={{ marginBottom: 14 }}>
                      <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase' }}>Nombre *</label>
                      <input type="text" value={ubicacionNombre} onChange={e => setUbicacionNombre(e.target.value)} placeholder="Ej: Pozo Norte, Canal de riego..."
                        style={{ width: '100%', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '9px 12px', color: 'var(--text)', fontSize: 13, fontFamily: 'var(--sans)', outline: 'none' }}/>
                    </div>
                    <div style={{ marginBottom: 16 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                        <label style={{ fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', textTransform: 'uppercase' }}>Parcelas asociadas *</label>
                        <button onClick={() => setUbicacionParcelaIds(misParcelas.filter(p => p.fincaId === ubicacionFincaId).map(p => p.id))}
                          style={{ fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--text)', background: 'transparent', border: 'none', cursor: 'pointer' }}>
                          Seleccionar todas
                        </button>
                      </div>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 4, maxHeight: 180, overflow: 'auto', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: 8 }}>
                        {misParcelas.filter(p => p.fincaId === ubicacionFincaId).map(p => (
                          <label key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 8, fontFamily: 'var(--sans)', fontSize: 12, color: 'var(--text)', cursor: 'pointer', padding: '3px 4px' }}>
                            <input type="checkbox" checked={ubicacionParcelaIds.includes(p.id)} onChange={() => toggleUbicacionParcela(p.id)} />
                            {formatCultivoVariedad(p.cultivo, p.variedad)}{p.nombre ? ` — ${p.nombre}` : ''}
                          </label>
                        ))}
                        {misParcelas.filter(p => p.fincaId === ubicacionFincaId).length === 0 && (
                          <div style={{ fontSize: 11, color: 'var(--muted)', fontFamily: 'var(--mono)' }}>Esta finca no tiene parcelas todavía.</div>
                        )}
                      </div>
                    </div>
                    {ubicacionError && <div style={{ padding: '8px 12px', borderRadius: 6, background: 'rgba(110, 36, 23,0.08)', border: '1px solid rgba(110, 36, 23,0.2)', color: '#6E2417', fontSize: 12, fontFamily: 'var(--mono)', marginBottom: 12 }}>{ubicacionError}</div>}
                    <div style={{ display: 'flex', gap: 10 }}>
                      <button onClick={guardarUbicacion} disabled={guardandoUbicacion}
                        style={{ flex: 1, padding: 11, borderRadius: 8, background: 'var(--green)', border: 'none', color: 'var(--text)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 13, cursor: guardandoUbicacion ? 'default' : 'pointer', opacity: guardandoUbicacion ? 0.6 : 1 }}>
              ...
              {formCampanaVisible && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
                  <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, width: '100%', maxWidth: 400, padding: 24 }}>
                    ...
                    {campError && <div style={{ padding: '8px 12px', borderRadius: 6, background: 'rgba(110, 36, 23,0.08)', border: '1px solid rgba(110, 36, 23,0.2)', color: '#6E2417', fontSize: 12, fontFamily: 'var(--mono)', marginBottom: 12 }}>{campError}</div>}
                    <div style={{ display: 'flex', gap: 10 }}>
                      <button onClick={async () => {
                        const eraNueva = !campanaEditar
                        const guardada = await guardarCampana()
                        if (guardada && eraNueva) abrirPopupCambioCampanas(guardada.id)
                      }} style={{ flex: 1, padding: 11, borderRadius: 8, background: 'var(--amber)', border: 'none', color: 'var(--text)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>GUARDAR</button>
                      <button onClick={() => { setFormCampanaVisible(false); setCampanaEditar(null); setCampError('') }} style={{ flex: 1, padding: 11, borderRadius: 8, background: 'var(--surface2)', border: '1px solid var(--border)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 13, cursor: 'pointer' }}>CANCELAR</button>
                    </div>
                  </div>
                </div>
              )}
              ...
              {popupCampanaHistorico && parcelaEditar && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 2500, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
                  <div style={{ background: 'var(--surface)', border: '1px solid #ffb454', borderRadius: 12, width: '100%', maxWidth: 400, padding: 24 }}>
                    ...
                    <div style={{ display: 'flex', gap: 10 }}>
                      <button onClick={() => confirmarGuardarCultivoAnterior(popupCampanaSeleccionada)} style={{ flex: 1, padding: 11, borderRadius: 8, background: '#ffb454', border: 'none', color: 'var(--text)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>GUARDAR</button>
                      <button onClick={() => setPopupCampanaHistorico(false)} style={{ flex: 1, padding: 11, borderRadius: 8, background: 'var(--surface2)', border: '1px solid var(--border)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 13, cursor: 'pointer' }}>CANCELAR</button>
                    </div>
                  </div>
                </div>
              )}
                  ...
                  <div style={{ background: 'var(--surface)', border: '1px solid #ffb454', borderRadius: 12, width: '100%', maxWidth: 900, maxHeight: '90vh', display: 'flex', flexDirection: 'column' }}>
                            ...
                            <div key={fila.parcelaId} style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 8, padding: 10 }}>
                              ...
                              <div>
                                ...
                                <select value={fila.campanaNuevaId} onChange={e => actualizarFilaCambioCampana(fila.parcelaId, 'campanaNuevaId', e.target.value)}
                                  style={{ width: '100%', background: 'var(--surface)', border: `1px solid ${fila.campanaNuevaId ? 'var(--green)' : 'var(--border)'}`, borderRadius: 5, padding: '7px 8px', color: 'var(--text)', fontSize: 12, fontFamily: 'var(--mono)', outline: 'none' }}>
                          ...
                          <tbody>
                            {filasCambioCampana.map(fila => (
                              <tr key={fila.parcelaId} style={{ borderBottom: '1px solid var(--border)' }}>
                                ...
                                <td style={{ padding: '6px' }}>
                                  <select value={fila.campanaNuevaId} onChange={e => actualizarFilaCambioCampana(fila.parcelaId, 'campanaNuevaId', e.target.value)}
                                    style={{ width: '100%', minWidth: 130, background: 'var(--surface2)', border: `1px solid ${fila.campanaNuevaId ? 'var(--green)' : 'var(--border)'}`, borderRadius: 5, padding: '5px 7px', color: 'var(--text)', fontSize: 11, fontFamily: 'var(--mono)', outline: 'none' }}>
                                    <option value="">— Sin cambio —</option>
                                    {campanas.map(c => <option key={c.id} value={c.id}>{c.nombre}</option>)}
                                  </select>
                                </td>
                    ...
                    <div style={{ padding: isMobile ? '12px' : '16px 24px', display: 'flex', gap: 10 }}>
                      <button onClick={guardarCambiosCampana} disabled={guardandoCambioCampanas}
                        style={{ flex: 1, padding: 11, borderRadius: 8, background: '#ffb454', border: 'none', color: 'var(--text)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 13, cursor: guardandoCambioCampanas ? 'default' : 'pointer', opacity: guardandoCambioCampanas ? 0.6 : 1 }}>
                        {guardandoCambioCampanas ? 'GUARDANDO...' : 'GUARDAR CAMBIOS'}
                      </button>
                      <button onClick={() => setPopupCambioCampanas(false)} style={{ flex: 1, padding: 11, borderRadius: 8, background: 'var(--surface2)', border: '1px solid var(--border)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 13, cursor: 'pointer' }}>CERRAR</button>
                    </div>
                  </div>
                </div>
              )}
            ...
            {/* Resumen total parcelas (respeta el filtro de finca activo) */}
              <div style={{ display: 'flex', gap: 12, marginBottom: 20 }}>
                <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, padding: '12px 20px', flex: 1 }}>
                  ...
                  <div style={{ fontSize: 24, fontFamily: 'var(--mono)', fontWeight: 700, color: 'var(--green)', textShadow: 'var(--green-outline)' }}>{misParcelasVista.length}</div>
                </div>
                <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, padding: '12px 20px', flex: 1 }}>
                ...
                <div ref={parcelaDetalleRef} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, marginBottom: 20, overflow: 'hidden' }}>
                  <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--surface2)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <button onClick={() => setParcelaDetalleId(null)} style={{ background: 'transparent', border: 'none', color: 'var(--text)', cursor: 'pointer', fontSize: 16 }}>← volver</button>
                      <span style={{ fontFamily: 'var(--mono)', fontSize: 13, fontWeight: 700, color: 'var(--green)', textShadow: 'var(--green-outline)' }}>{parcelaDetalle.nombre || parcelaDetalle.cultivo}</span>
                    </div>
                    <button onClick={() => abrirFormTrat(parcelaDetalle.id)} style={{ padding: '6px 12px', borderRadius: 6, background: 'var(--green)', border: 'none', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 11, fontWeight: 700, cursor: 'pointer' }}>+ AÑADIR TRATAMIENTO</button>
                  </div>
                  <div style={{ padding: 16 }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px,1fr))', gap: 8, marginBottom: 16 }}>
                      {[
                        ...
                        <div key={s.k} style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '8px 10px' }}>
                          ...
                          <div style={{ fontSize: 13, fontFamily: 'var(--mono)', fontWeight: 700, color: 'var(--text)', marginTop: 2 }}>{s.v}</div>
                        </div>
                      ))}
                    </div>
                    ...
                    {historicoCultivos.filter(h => h.parcelaId === parcelaDetalle.id).length > 0 && (
                      <div style={{ marginBottom: 16 }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                          <div style={{ background: 'rgba(31, 74, 40,0.08)', border: '1px solid var(--green)', borderRadius: 6, padding: '8px 10px', fontSize: 11, fontFamily: 'var(--mono)' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                              <span style={{ color: 'var(--green)', textShadow: 'var(--green-outline)' }}>● Actual: {formatCultivoVariedad(parcelaDetalle.cultivo, parcelaDetalle.variedad)}</span>
                              <span style={{ color: 'var(--muted)' }}>{campanas.find(c => c.id === parcelaDetalle.campanaId)?.nombre || 'Sin campaña'}</span>
                            </div>
                            {cosechaResumen(parcelaDetalle.id, parcelaDetalle.campanaId) && (
                              <div style={{ color: 'var(--muted)', marginTop: 4, fontSize: 10 }}>{cosechaResumen(parcelaDetalle.id, parcelaDetalle.campanaId)}</div>
                            )}
                          </div>
                          {historicoCultivos
                            ...
                            .map(h => (
                              historicoEditarId === h.id ? (
                                <div key={h.id} style={{ background: 'var(--surface2)', border: '1px solid #ffb454', borderRadius: 6, padding: '8px 10px' }}>
                                  ...
                                  <div style={{ display: 'flex', gap: 6 }}>
                                    <button onClick={guardarEdicionHistorico} style={{ padding: '5px 10px', borderRadius: 5, background: '#ffb454', border: 'none', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 10, fontWeight: 700, cursor: 'pointer' }}>Guardar</button>
                                    <button onClick={() => setHistoricoEditarId(null)} style={{ padding: '5px 10px', borderRadius: 5, background: 'transparent', border: '1px solid var(--border)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 10, cursor: 'pointer' }}>Cancelar</button>
                                  </div>
                                </div>
                              ) : (
                                <div key={h.id} style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '8px 10px', fontSize: 11, fontFamily: 'var(--mono)' }}>
                                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                      ...
                                      <button onClick={() => abrirEditarHistorico(h)} style={{ background: 'transparent', border: 'none', color: 'var(--text)', cursor: 'pointer', fontSize: 11 }} title="Editar">✏</button>
                                      <button onClick={() => eliminarHistoricoCultivo(h.id)} style={{ background: 'transparent', border: 'none', color: 'var(--text)', cursor: 'pointer', fontSize: 11 }} title="Eliminar">✕</button>
                                    </div>
                                  </div>
                                  {cosechaResumen(h.parcelaId, h.campanaId) && (
                                    <div style={{ color: 'var(--muted)', marginTop: 4, fontSize: 10 }}>{cosechaResumen(h.parcelaId, h.campanaId)}</div>
                                  )}
                                </div>
                              )
                            ))}
                    ...
                    {/* HISTORIAL DE COSECHAS — registro de cosecha comercializada, asociado a esta
                        parcela (ver PARCELA → CULTIVO → CAMPAÑA → COSECHA → COMERCIALIZACIÓN). */}
                    {misPermisos['cosechas.ver'] && (
                    <div style={{ marginBottom: 16 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                        <div style={{ fontSize: 11, fontFamily: 'var(--mono)', color: 'var(--text)', letterSpacing: '0.06em', fontWeight: 700 }}>HISTORIAL DE COSECHAS</div>
                        {misPermisos['cosechas.crear_editar_eliminar'] && (
                          <button onClick={() => abrirNuevaCosecha(parcelaDetalle)} style={{ padding: '5px 10px', borderRadius: 6, background: 'transparent', border: '1px solid var(--green)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 10, fontWeight: 700, cursor: 'pointer' }}>+ NUEVA COSECHA</button>
                        )}
                      </div>
                      {cosechas.filter(c => c.parcelaId === parcelaDetalle.id).length === 0 ? (
                        <div style={{ fontSize: 11, color: 'var(--muted)', fontFamily: 'var(--mono)', padding: '6px 0' }}>Todavía no hay cosechas registradas en esta parcela.</div>
                      ) : (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                          {cosechas
                            .filter(c => c.parcelaId === parcelaDetalle.id)
                            .sort((a, b) => (b.fechaCosecha || '').localeCompare(a.fechaCosecha || ''))
                            .map(c => {
                              const expandida = cosechaExpandidaId === c.id
                              const pendiente = pendienteComercializar(c)
                              const comercial = totalComercializado(c.id)
                              const comercializacionesCosecha = comercializaciones.filter(x => x.cosechaId === c.id).sort((a, b) => (b.fecha || '').localeCompare(a.fecha || ''))
                              return (
                                <div key={c.id} style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, overflow: 'hidden' }}>
                                  <div onClick={() => setCosechaExpandidaId(expandida ? null : c.id)} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '9px 10px', cursor: 'pointer' }}>
                                    <div>
                                      <span style={{ fontSize: 11, fontFamily: 'var(--mono)', color: 'var(--text)', fontWeight: 700 }}>
                                        🌿 {formatCultivoVariedad(c.cultivo, c.variedad)}{c.cantidad ? ` — ${formatCantidadEs(Number(c.cantidad))} ${c.unidad}` : ' — Sin registrar'}
                                      </span>
                                      <div style={{ fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginTop: 2 }}>
                                        {campanas.find(camp => camp.id === c.campanaId)?.nombre || 'Sin campaña'} · {formatFechaEs(c.fechaCosecha)}
                                      </div>
                                    </div>
                                    <span style={{ color: 'var(--muted)', fontSize: 12 }}>{expandida ? '▲' : '▼'}</span>
                                  </div>
                                  {expandida && (
                                    <div style={{ padding: '0 10px 10px', borderTop: '1px solid var(--border)' }}>
                                      <div style={{ display: 'flex', flexDirection: 'column', gap: 3, padding: '8px 0', fontSize: 11, fontFamily: 'var(--mono)' }}>
                                        <div style={{ color: 'var(--muted)' }}>Materia cosechada: <span style={{ color: 'var(--text)' }}>{c.cantidad ? `${formatCantidadEs(Number(c.cantidad))} ${c.unidad}` : 'Sin registrar'}</span></div>
                                        <div style={{ color: 'var(--muted)' }}>Materia comercializada: <span style={{ color: 'var(--text)' }}>{comercial > 0 ? `${formatCantidadEs(comercial)} ${c.unidad}` : 'Sin registrar'}</span></div>
                                        <div style={{ color: 'var(--muted)' }}>Pendiente de comercializar: <span style={{ color: 'var(--text)' }}>{pendiente !== null ? `${formatCantidadEs(pendiente)} ${c.unidad}` : '—'}</span></div>
                                        {c.observaciones && <div style={{ color: 'var(--muted)' }}>Observaciones: <span style={{ color: 'var(--text)' }}>{c.observaciones}</span></div>}
                                      </div>
                                      <div style={{ display: 'flex', gap: 6, marginBottom: 8 }}>
                                        {misPermisos['cosechas.crear_editar_eliminar'] && (
                                          <button onClick={() => abrirEditarCosecha(c)} style={{ padding: '5px 10px', borderRadius: 5, background: 'transparent', border: '1px solid var(--border)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 10, cursor: 'pointer' }}>Editar</button>
                                        )}
                                        {misPermisos['cosechas.crear_editar_eliminar'] && (
                                          <button onClick={() => eliminarCosecha(c.id)} style={{ padding: '5px 10px', borderRadius: 5, background: 'transparent', border: '1px solid #6E2417', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 10, cursor: 'pointer' }}>Eliminar</button>
                                        )}
                                      </div>

                                      {misPermisos['cosechas.comercializacion.ver'] && (<>
                                      <div style={{ fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--blue)', letterSpacing: '0.06em', marginBottom: 6, fontWeight: 700 }}>COMERCIALIZACIONES</div>
                                      {comercializacionesCosecha.length === 0 && comFormCosechaId !== c.id && (
                                        <div style={{ fontSize: 10, color: 'var(--muted)', fontFamily: 'var(--mono)', marginBottom: 6 }}>Sin comercializaciones registradas.</div>
                                      )}
                                      <div style={{ display: 'flex', flexDirection: 'column', gap: 5, marginBottom: 8 }}>
                                        {comercializacionesCosecha.map(com => (
                                          <div key={com.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 5, padding: '6px 8px', fontSize: 10, fontFamily: 'var(--mono)' }}>
                                            <span style={{ color: 'var(--text)' }}>{formatFechaEs(com.fecha)} → {formatCantidadEs(Number(com.cantidad))} {com.unidad}{com.comprador ? ` → ${com.comprador}${com.compradorDniNif ? ` (${com.compradorDniNif})` : ''}` : ''}</span>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                                              {misPermisos['cosechas.comercializacion.crear_editar_eliminar'] && (
                                                <button onClick={() => abrirEditarComercializacion(com)} style={{ background: 'transparent', border: 'none', color: 'var(--text)', cursor: 'pointer', fontSize: 10 }} title="Editar">✏</button>
                                              )}
                                              {misPermisos['cosechas.comercializacion.crear_editar_eliminar'] && (
                                                <button onClick={() => eliminarComercializacion(com.id)} style={{ background: 'transparent', border: 'none', color: 'var(--text)', cursor: 'pointer', fontSize: 10 }} title="Eliminar">✕</button>
                                              )}
                                            </div>
                                          </div>
                                        ))}
                                      ...
                                      {comFormCosechaId === c.id && misPermisos['cosechas.comercializacion.crear_editar_eliminar'] ? (
                                        <div style={{ background: 'var(--surface)', border: '1px solid var(--blue)', borderRadius: 6, padding: 8, display: 'flex', flexDirection: 'column', gap: 6 }}>
                                          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
                                            <input type="date" value={comFecha} onChange={e => setComFecha(e.target.value)}
                                              style={{ width: '100%', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 5, padding: '6px 8px', color: 'var(--text)', fontSize: 11, fontFamily: 'var(--mono)', outline: 'none' }} />
                                            <div style={{ display: 'flex', gap: 4 }}>
                                              <input type="number" step="any" min="0" value={comCantidad} onChange={e => setComCantidad(e.target.value)} placeholder="Cantidad"
                                                style={{ flex: 1, width: 0, background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 5, padding: '6px 8px', color: 'var(--text)', fontSize: 11, fontFamily: 'var(--mono)', outline: 'none' }} />
                                              <select value={comUnidad} onChange={e => setComUnidad(e.target.value)}
                                                style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 5, padding: '6px 4px', color: 'var(--text)', fontSize: 11, fontFamily: 'var(--mono)', outline: 'none' }}>
                                                {UNIDADES_COSECHA.map(u => <option key={u} value={u}>{u}</option>)}
                                              </select>
                                            </div>
                                          </div>
                                          <div style={{ display: 'flex', gap: 4 }}>
                                            <input value={comComprador} onChange={e => setComComprador(e.target.value)} placeholder="Comprador / destinatario (opcional)"
                                              style={{ flex: 1, width: 0, background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 5, padding: '6px 8px', color: 'var(--text)', fontSize: 11, fontFamily: 'var(--mono)', outline: 'none' }} />
                                            <input value={comCompradorDniNif} onChange={e => setComCompradorDniNif(e.target.value)} placeholder="DNI/NIF (opcional)"
                                              style={{ width: 120, background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 5, padding: '6px 8px', color: 'var(--text)', fontSize: 11, fontFamily: 'var(--mono)', outline: 'none' }} />
                                          </div>
                                          <input value={comCompradorDireccion} onChange={e => setComCompradorDireccion(e.target.value)} placeholder="Dirección del cliente (opcional)"
                                            style={{ width: '100%', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 5, padding: '6px 8px', color: 'var(--text)', fontSize: 11, fontFamily: 'var(--mono)', outline: 'none' }} />
                                          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 6 }}>
                                            <input value={comCompradorRgseaa} onChange={e => setComCompradorRgseaa(e.target.value)} placeholder="Nº RGSEAA (opcional)"
                                              style={{ width: '100%', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 5, padding: '6px 8px', color: 'var(--text)', fontSize: 11, fontFamily: 'var(--mono)', outline: 'none' }} />
                                            <input value={comNumAlbaranFactura} onChange={e => setComNumAlbaranFactura(e.target.value)} placeholder="Nº albarán/factura"
                                              style={{ width: '100%', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 5, padding: '6px 8px', color: 'var(--text)', fontSize: 11, fontFamily: 'var(--mono)', outline: 'none' }} />
                                            <input value={comNumLote} onChange={e => setComNumLote(e.target.value)} placeholder="Nº lote"
                                              style={{ width: '100%', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 5, padding: '6px 8px', color: 'var(--text)', fontSize: 11, fontFamily: 'var(--mono)', outline: 'none' }} />
                                          </div>
                                          <input value={comObservaciones} onChange={e => setComObservaciones(e.target.value)} placeholder="Observaciones (opcional)"
                                            style={{ width: '100%', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 5, padding: '6px 8px', color: 'var(--text)', fontSize: 11, fontFamily: 'var(--mono)', outline: 'none' }} />
                                          {comError && <div style={{ color: '#6E2417', fontSize: 10, fontFamily: 'var(--mono)' }}>{comError}</div>}
                                          <div style={{ display: 'flex', gap: 6 }}>
                                            <button onClick={guardarComercializacion} style={{ padding: '6px 10px', borderRadius: 5, background: 'var(--blue)', border: 'none', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 10, fontWeight: 700, cursor: 'pointer' }}>Guardar</button>
                                            <button onClick={() => { setComFormCosechaId(null); setComFormParteKey(null); setComEditarId(null) }} style={{ padding: '6px 10px', borderRadius: 5, background: 'transparent', border: '1px solid var(--border)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 10, cursor: 'pointer' }}>Cancelar</button>
                                          </div>
                                        </div>
                                      ) : misPermisos['cosechas.comercializacion.crear_editar_eliminar'] ? (
                                        <button onClick={() => abrirNuevaComercializacion(c.id)} style={{ padding: '6px 10px', borderRadius: 5, background: 'transparent', border: '1px solid var(--blue)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 10, fontWeight: 700, cursor: 'pointer' }}>+ Comercialización</button>
                                      ) : null}
                                      </>)}
                                    </div>
                                  )}
                    ...
                    {parcelaDetalle.parcelaInfo?.recintos?.length > 0 && (
                      <div style={{ marginBottom: 16 }}>
                        <div style={{ fontSize: 11, fontFamily: 'var(--mono)', color: 'var(--text)', letterSpacing: '0.06em', marginBottom: 8, fontWeight: 700 }}>REFERENCIA SIGPAC</div>
                    ...
                    <div style={{ fontSize: 11, fontFamily: 'var(--mono)', color: 'var(--text)', letterSpacing: '0.06em', marginBottom: 10, fontWeight: 700 }}>HISTORIAL DE TRATAMIENTOS</div>
                          ...
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                            <div style={{ flex: 1 }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                                <span style={{ fontSize: 10, fontFamily: 'var(--mono)', fontWeight: 700, color: 'var(--green)', textShadow: 'var(--green-outline)' }}>{t.fecha}</span>
                                <span style={{ fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--blue)', background: 'rgba(15, 74, 112,0.1)', border: '1px solid rgba(15, 74, 112,0.2)', borderRadius: 4, padding: '1px 6px' }}>{t.tipo}</span>
                              </div>
                            ...
                            <div style={{ display: 'flex', gap: 6, flexShrink: 0, marginLeft: 8 }}>
                              <button onClick={() => abrirFormTrat(parcelaDetalle.id, t)} style={{ padding: '4px 8px', borderRadius: 4, background: 'var(--surface)', border: '1px solid var(--border)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 10, cursor: 'pointer' }}>✏</button>
                              {misPermisos['tratamientos.eliminar'] && (
                                <button onClick={() => eliminarTratamiento(t.id)} style={{ padding: '4px 8px', borderRadius: 4, background: 'transparent', border: '1px solid rgba(110, 36, 23, 1)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 10, cursor: 'pointer' }}>✕</button>
                              )}
                            </div>
                          </div>
                        </div>
                  ...
                  <div style={{ marginTop: 16, border: '1px solid var(--border)', borderRadius: 8, overflow: 'hidden' }}>
                    ...
                    {climaAbierto && (
                      <div style={{ padding: '12px 14px' }}>
                        ...
                        {climaError && <div style={{ fontSize: 11, color: '#6E2417', fontFamily: 'var(--mono)' }}>{climaError}</div>}
                          ...
                          <>
                            {climaDatos.estacion && (
                              <div style={{ fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 8, padding: '4px 0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                ...
                                {climaDatos.fuente && (
                                  <span style={{ fontSize: 9, fontFamily: 'var(--mono)', padding: '2px 6px', borderRadius: 4, background: climaDatos.fuente === 'SIAR' ? 'rgba(31, 74, 40,0.1)' : 'rgba(15, 74, 112,0.1)', color: climaDatos.fuente === 'SIAR' ? 'var(--green)' : 'var(--blue)', textShadow: climaDatos.fuente === 'SIAR' ? 'var(--green-outline)' : 'none', border: `1px solid ${climaDatos.fuente === 'SIAR' ? 'rgba(31, 74, 40,0.3)' : 'rgba(15, 74, 112,0.3)'}` }}>
                                    {climaDatos.fuente}
                                  </span>
                                )}
                              </div>
              ...
              {misParcelasVista.length === 0 ? (
                ...
                <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(auto-fill, minmax(260px, 1fr))', gap: isMobile ? 10 : 16 }}>
                  {misParcelasVista.map(p => {
                    ...
                    <div
                      key={p.id}
                      style={{ background: 'var(--surface)', border: campanaTerminada ? '1px solid #6E2417' : '1px solid var(--border)', borderRadius: 10, overflow: 'hidden', cursor: 'pointer', transition: 'border-color 0.2s' }}
                      onMouseEnter={e => { if (!campanaTerminada) e.currentTarget.style.borderColor = '#1F4A28' }}
                    ...
                    >
                      ...
                      <div style={{ height: 140, background: 'var(--surface2)', position: 'relative', overflow: 'hidden' }}>
                        ...
                        <button
                          ...
                          style={{ position: 'absolute', top: 8, right: 8, width: 32, height: 32, borderRadius: '50%', background: 'rgba(225, 231, 211, 1)', border: '1px solid var(--border)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 15, backdropFilter: 'blur(4px)', transition: 'background 0.2s' }}
                          onMouseEnter={e => (e.currentTarget.style.background = 'rgba(31, 74, 40, 1)')}
                          onMouseLeave={e => (e.currentTarget.style.background = 'rgba(225, 231, 211, 1)')}
                        >
                          👁
                        </button>
                      </div>
                      ...
                      <div style={{ padding: '12px 14px', position: 'relative' }}>
                        ...
                        <div style={{ fontFamily: 'var(--mono)', fontSize: 12, fontWeight: 700, color: 'var(--green)', textShadow: 'var(--green-outline)', marginBottom: 4, letterSpacing: '0.04em' }}>
                        ...
                        <div style={{ fontSize: 10, color: 'var(--muted)', fontFamily: 'var(--mono)', lineHeight: 1.6 }}>
                          ...
                          {campanaDeP && (
                            <div style={{ color: campanaTerminada ? '#6E2417' : 'var(--muted)' }}>
                      ...
                      <div style={{ padding: '8px 14px', borderTop: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        {misPermisos['parcelas.crear_editar'] && (
                          <button onClick={e => { e.stopPropagation(); abrirEditar(p) }} style={{ fontSize: 9, color: 'var(--text)', fontFamily: 'var(--mono)', background: 'transparent', border: 'none', cursor: 'pointer', textDecoration: 'underline' }}>✏ Editar info</button>
                        )}
                        {misPermisos['parcelas.eliminar'] && (
                          <button onClick={e => { e.stopPropagation(); eliminarParcela(p.id) }} style={{ fontSize: 9, color: 'var(--text)', fontFamily: 'var(--mono)', background: 'transparent', border: 'none', cursor: 'pointer', textDecoration: 'underline' }}>Eliminar</button>
                        )}
                      </div>
                    </div>
                  )})}
        ...
        {pestana === 'tratamientos' && (
          <div style={{ flex: 1, height: '100%', overflow: 'auto', padding: isMobile ? 12 : 24, background: 'var(--bg)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px,1fr))', gap: 12, marginBottom: 24 }}>
              {[
                { k: 'Total tratamientos', v: String(tratamientos.length), color: 'var(--green)', textShadow: 'var(--green-outline)' },
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, flexWrap: 'wrap', gap: 10 }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                {misPermisos['tratamientos.descargar_cuaderno'] && (
                  <button onClick={() => { setCuadernoFincaIds([]); setCuadernoCampanaId(prev => prev && campanas.some(c => c.id === prev) ? prev : ((campanas.find(c => !campanaFinalizada(c)) || campanas[0])?.id || '')); setCuadernoModalVisible(true) }}
                    style={{ padding: '8px 16px', borderRadius: 8, background: 'var(--surface2)', border: '1px solid var(--border)', color: 'var(--text)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 12, cursor: 'pointer', letterSpacing: '0.06em' }}>📄 CUADERNO DE CAMPO</button>
                )}
                {misPermisos['tratamientos.crear_editar'] && (
                  <button onClick={() => abrirFormTrat()} style={{ padding: '8px 16px', borderRadius: 8, background: 'var(--green)', border: 'none', color: 'var(--text)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 12, cursor: 'pointer', letterSpacing: '0.06em' }}>+ NUEVO TRATAMIENTO</button>
                )}
              </div>
            </div>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
              {campanas.length > 0 && (
                <select value={filtroCampanaTrat} onChange={e => setFiltroCampanaTrat(e.target.value)} style={{ background: 'var(--surface)', border: `1px solid ${filtroCampanaTrat ? 'var(--amber)' : 'var(--border)'}`, borderRadius: 6, padding: '6px 10px', color: 'var(--text)', fontSize: 11, fontFamily: 'var(--mono)', outline: 'none' }}>
                  <option value="">Todas las campañas</option>
                  {campanas.map(c => <option key={c.id} value={c.id}>{c.nombre}{campanaFinalizada(c) ? ' (finalizada)' : ''}</option>)}
                </select>
              )}
              {filtroCampanaTrat && (
                <button onClick={() => setFiltroCampanaTrat('')} style={{ padding: '6px 12px', borderRadius: 6, background: 'transparent', border: '1px solid rgba(92, 63, 8, 1)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 11, cursor: 'pointer' }}>✕ Quitar filtro de campaña</button>
              )}
              <select value={filtroTipo} onChange={e => setFiltroTipo(e.target.value)} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 6, padding: '6px 10px', color: 'var(--text)', fontSize: 11, fontFamily: 'var(--mono)', outline: 'none' }}>
                <option value="">Todos los tipos</option>
                {TIPOS_TRATAMIENTO.map(t => <option key={t} value={t}>{t}</option>)}
              </select>
              {fincas.length > 0 && (
                <select value={filtroFincaTrat} onChange={e => setFiltroFincaTrat(e.target.value)} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 6, padding: '6px 10px', color: 'var(--text)', fontSize: 11, fontFamily: 'var(--mono)', outline: 'none' }}>
              ...
              <select value={filtroCultivoTrat} onChange={e => setFiltroCultivoTrat(e.target.value)} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 6, padding: '6px 10px', color: 'var(--text)', fontSize: 11, fontFamily: 'var(--mono)', outline: 'none' }}>
              ...
              {(filtroTipo || filtroProducto || filtroCultivoTrat) && (
                <button onClick={() => { setFiltroTipo(''); setFiltroProducto(''); setFiltroCultivoTrat('') }} style={{ padding: '6px 12px', borderRadius: 6, background: 'transparent', border: '1px solid rgba(110, 36, 23, 1)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 11, cursor: 'pointer' }}>✕ Limpiar</button>
              )}
            </div>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', cursor: 'pointer' }} onClick={() => setTratExpandido(expandido ? null : t.id)}>
                        ...
                        <div style={{ flex: 1 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 3 }}>
                            ...
                            <span style={{ fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--blue)', background: 'rgba(15, 74, 112,0.1)', border: '1px solid rgba(15, 74, 112,0.2)', borderRadius: 4, padding: '1px 7px' }}>{t.tipo}</span>
                          </div>
                        ...
                        <div style={{ display: 'flex', gap: 6 }}>
                          <button onClick={e => { e.stopPropagation(); generarPdfTratamiento(t) }} disabled={generandoPdfId === t.id} style={{ padding: '5px 10px', borderRadius: 5, background: 'var(--surface2)', border: '1px solid var(--border)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 11, cursor: generandoPdfId === t.id ? 'default' : 'pointer', opacity: generandoPdfId === t.id ? 0.5 : 1 }} title="Descargar PDF">
                            {generandoPdfId === t.id ? '⏳' : '📄'}
                          </button>
                          <button onClick={e => { e.stopPropagation(); abrirFormTrat(undefined, t) }} style={{ padding: '5px 10px', borderRadius: 5, background: 'var(--surface2)', border: '1px solid var(--border)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 11, cursor: 'pointer' }}>✏</button>
                          {misPermisos['tratamientos.eliminar'] && (
                            <button onClick={e => { e.stopPropagation(); eliminarTratamiento(t.id) }} style={{ padding: '5px 10px', borderRadius: 5, background: 'transparent', border: '1px solid rgba(110, 36, 23, 1)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 11, cursor: 'pointer' }}>✕</button>
                          )}
                        </div>
                      </div>
                        ...
                        <div style={{ borderTop: '1px solid var(--border)', padding: '12px 16px', background: 'var(--surface2)' }}>
                          ...
                          <div style={{ marginBottom: t.observaciones ? 12 : 0 }}>
                            ...
                            {parcelasAfectadas.map(p => {
                              ...
                              return (
                                <div key={p.id} style={{ display: 'grid', gridTemplateColumns: '1fr auto auto auto', gap: 6, padding: '6px 0', borderBottom: '1px solid var(--border)' }}>
                                  ...
                                  <div style={{ fontSize: 11, fontFamily: 'var(--mono)', color: 'var(--green)', textShadow: 'var(--green-outline)', textAlign: 'right' }}>{pct}%</div>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr auto auto auto', gap: 6, padding: '6px 0', marginTop: 2 }}>
                              ...
                              <div style={{ fontSize: 11, fontFamily: 'var(--mono)', color: 'var(--green)', textShadow: 'var(--green-outline)', fontWeight: 700, textAlign: 'right' }}>{supTotal.toFixed(4)}</div>
                              <div style={{ fontSize: 11, fontFamily: 'var(--mono)', color: 'var(--green)', textShadow: 'var(--green-outline)', textAlign: 'right' }}>100%</div>
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: 8, marginBottom: 20, flexWrap: 'wrap' }}>
                  <select value={filtroCampanaEstadistica} onChange={e => setFiltroCampanaEstadistica(e.target.value)}
                    style={{ background: 'var(--surface)', border: `1px solid ${filtroCampanaEstadistica ? 'var(--green)' : 'var(--border)'}`, borderRadius: 6, padding: '7px 12px', color: 'var(--text)', fontSize: 12, fontFamily: 'var(--mono)', outline: 'none' }}>
                  ...
                  {filtroCampanaEstadistica && fincas.length > 0 && (
                    <select value={filtroFincaEstadistica} onChange={e => setFiltroFincaEstadistica(e.target.value)}
                      style={{ background: 'var(--surface)', border: `1px solid ${filtroFincaEstadistica ? 'var(--blue)' : 'var(--border)'}`, borderRadius: 6, padding: '7px 12px', color: 'var(--text)', fontSize: 12, fontFamily: 'var(--mono)', outline: 'none' }}>
                      <option value="">Todas las fincas</option>
                      {fincas.map(f => <option key={f.id} value={f.id}>{f.nombre}</option>)}
                    </select>
                  )}
                  {(filtroCampanaEstadistica || filtroFincaEstadistica) && (
                    <button onClick={() => { setFiltroCampanaEstadistica(''); setFiltroFincaEstadistica('') }}
                      style={{ padding: '7px 12px', borderRadius: 6, background: 'transparent', border: '1px solid rgba(110, 36, 23, 1)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 11, cursor: 'pointer' }}>✕ Quitar filtros</button>
                  )}
                </div>
                ...
                {!filtroCampanaEstadistica ? (
                  ...
                  Object.entries(estadisticasPorCultivo()).map(([cultivo, productos]) => (
                    <div key={cultivo} style={{ marginBottom: 28 }}>
                      <div style={{ fontFamily: 'var(--mono)', fontSize: 13, fontWeight: 700, color: 'var(--green)', textShadow: 'var(--green-outline)', letterSpacing: '0.06em', marginBottom: 12, paddingBottom: 6, borderBottom: '1px solid var(--border)' }}>
                        🌾 {cultivo.toUpperCase()}
                      </div>
                            ...
                            <div key={key} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, overflow: 'hidden' }}>
                              ...
                              >
                                ...
                                {tieneDosisMax && !noComparable ? (
                                  <div style={{ position: 'relative', width: 130, height: 130 }}>
                                    <DonutChart
                                      slices={[
                                        { value: pieData[0]?.value || 0, color: excedido ? '#6E2417' : '#1F4A28' },
                                        { value: pieData[1]?.value || 0, color: '#CDD6B9' },
                                      ]}
                                      innerRadius={42}
                                      outerRadius={60}
                                    />
                                    <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                                      <div style={{ fontSize: 16, fontFamily: 'var(--mono)', fontWeight: 700, color: excedido ? '#6E2417' : 'var(--text)' }}>
                                  ...
                                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: 130 }}>
                                    <div style={{ fontSize: 22, fontFamily: 'var(--mono)', fontWeight: 700, color: 'var(--green)', textShadow: 'var(--green-outline)' }}>
                                      {dosisPonderadaMostrar.toFixed(2)}
                                    </div>
                                    <div style={{ fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--green)', textShadow: 'var(--green-outline)', fontWeight: 700, marginTop: 2 }}>
                                      {unidadMostrada}
                                    </div>
                                ...
                                {tieneMax && (
                                  <div style={{ fontSize: 9, fontFamily: 'var(--mono)', color: 'var(--muted)', marginTop: 8, textAlign: 'center' }}>
                                    Aplicaciones: <span style={{ color: p.numAplicaciones > (p.aplicMaxima as number) ? '#6E2417' : 'var(--text)', fontWeight: 700 }}>{p.numAplicaciones}</span> / {Number.isInteger(p.aplicMaxima) ? p.aplicMaxima : (p.aplicMaxima as number).toFixed(1)}
                                  </div>
                                )}
                              ...
                              {expandido && (
                                <div style={{ borderTop: '1px solid var(--border)', padding: '10px 14px', background: 'var(--surface2)' }}>
                                  ...
                                  {tieneDosisMax && !noComparable && (
                                    <div style={{ marginTop: 6, fontSize: 9, fontFamily: 'var(--mono)', color: 'var(--muted)' }}>
                                      Restante hasta máx: <span style={{ color: dosisPonderadaMostrar > (dosisMaximaValor as number) ? '#6E2417' : 'var(--green)', textShadow: dosisPonderadaMostrar > (dosisMaximaValor as number) ? 'none' : 'var(--green-outline)', fontWeight: 700 }}>
            ...
            {subTabTrat === 'stock' && (
              <div>
                <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 14 }}>
                  {misPermisos['tratamientos.crear_editar'] && (
                    <button onClick={() => setFormStock(true)} style={{ padding: '8px 16px', borderRadius: 8, background: 'var(--green)', border: 'none', color: 'var(--text)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 12, cursor: 'pointer', letterSpacing: '0.06em' }}>+ AÑADIR PRODUCTO</button>
                  )}
                </div>
                    ...
                    {stock.map(s => {
                      ...
                      return (
                        <div key={s.id} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr auto', gap: 8, padding: '10px 16px', borderBottom: '1px solid var(--border)', alignItems: 'center' }}>
                          ...
                          <div style={{ fontSize: 12, fontFamily: 'var(--mono)', fontWeight: 700, textAlign: 'right', color: existencias < 0 ? '#6E2417' : 'var(--green)', textShadow: existencias < 0 ? 'none' : 'var(--green-outline)' }}>
                            {existencias.toFixed(2)} {s.unidad}
                          </div>
                          {misPermisos['tratamientos.crear_editar'] && (
                            <button onClick={() => eliminarStockItem(s.id)} style={{ padding: '4px 8px', borderRadius: 4, background: 'transparent', border: '1px solid rgba(110, 36, 23, 1)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 10, cursor: 'pointer', justifySelf: 'end' }}>✕</button>
                          )}
                        </div>
                      )
                    })}
                ...
                {formStock && (
                  <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
                    <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, width: '100%', maxWidth: 380, overflow: 'auto' }}>
                      <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <span style={{ fontFamily: 'var(--mono)', fontSize: 13, fontWeight: 700, color: 'var(--green)', textShadow: 'var(--green-outline)' }}>📦 AÑADIR PRODUCTO A STOCK</span>
                        <button onClick={() => setFormStock(false)} style={{ background: 'transparent', border: 'none', color: 'var(--text)', cursor: 'pointer', fontSize: 18 }}>✕</button>
                      </div>
                      <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 14 }}>
                            ...
                            <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 3000, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 6, boxShadow: '0 8px 24px rgba(0,0,0,0.4)', maxHeight: 200, overflowY: 'auto', marginTop: 2 }}>
                              {fitoResultados.map(p => (
                                <div
                                  ...
                                  style={{ padding: '8px 14px', cursor: 'pointer', borderBottom: '1px solid var(--border)', borderLeft: p.eliminado ? '3px solid #6E2417' : '3px solid transparent' }}
                                  onMouseEnter={e => (e.currentTarget.style.background = 'var(--surface2)')}
                                  onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}
                                >
                                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                                    <span style={{ fontSize: 12, fontWeight: 600, color: p.eliminado ? '#6E2417' : 'var(--text)' }}>{p.nombre}</span>
                                    {p.eliminado && <span style={{ fontSize: 8, fontFamily: 'var(--mono)', color: '#6E2417', background: 'rgba(110, 36, 23,0.1)', border: '1px solid rgba(110, 36, 23,0.3)', borderRadius: 3, padding: '1px 4px' }}>NO AUTORIZADO</span>}
                                  </div>
                      ...
                      <div style={{ padding: '14px 20px', borderTop: '1px solid var(--border)', display: 'flex', gap: 10 }}>
                        <button onClick={guardarStockItem} style={{ flex: 1, padding: '11px', borderRadius: 8, background: 'var(--green)', border: 'none', color: 'var(--text)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>💾 GUARDAR</button>
                        <button onClick={() => setFormStock(false)} style={{ flex: 1, padding: '11px', borderRadius: 8, background: 'var(--surface2)', border: '1px solid var(--border)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 13, cursor: 'pointer' }}>CANCELAR</button>
                      </div>
                    </div>
                  </div>
                )}
          ...
          <div style={{ width: '100%', height: '100%', overflow: 'auto', padding: isMobile ? 12 : 24, background: 'var(--bg)' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              ...
              {misPermisos['personal.crear_editar'] && (
                <button onClick={() => abrirFormPersonal()} style={{ padding: '8px 16px', borderRadius: 8, background: 'var(--green)', border: 'none', color: 'var(--text)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 12, cursor: 'pointer', letterSpacing: '0.06em' }}>+ AÑADIR PERSONAL</button>
              )}
            </div>

            {/* Filtros */}
            <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
              <select value={filtroFuncion} onChange={e => setFiltroFuncion(e.target.value)} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 6, padding: '6px 10px', color: 'var(--text)', fontSize: 11, fontFamily: 'var(--mono)', outline: 'none' }}>
                <option value="">Todas las funciones</option>
                {FUNCIONES_PERSONAL.map(f => <option key={f} value={f}>{ICONO_FUNCION[f]} {f}</option>)}
              </select>
              <select value={filtroContrato} onChange={e => setFiltroContrato(e.target.value)} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 6, padding: '6px 10px', color: 'var(--text)', fontSize: 11, fontFamily: 'var(--mono)', outline: 'none' }}>
                <option value="">Todos los contratos</option>
                {TIPOS_CONTRATO.map(c => <option key={c} value={c}>{c}</option>)}
              </select>
              {fincas.length > 0 && (
                <select value={filtroFincaPersonal} onChange={e => setFiltroFincaPersonal(e.target.value)} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 6, padding: '6px 10px', color: 'var(--text)', fontSize: 11, fontFamily: 'var(--mono)', outline: 'none' }}>
                  <option value="">Todas las fincas</option>
                  {fincas.map(f => <option key={f.id} value={f.id}>{f.nombre}</option>)}
                </select>
              )}
              {(filtroFuncion || filtroContrato || filtroFincaPersonal) && (
                <button onClick={() => { setFiltroFuncion(''); setFiltroContrato(''); setFiltroFincaPersonal('') }} style={{ padding: '6px 12px', borderRadius: 6, background: 'transparent', border: '1px solid rgba(110, 36, 23, 1)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 11, cursor: 'pointer' }}>Limpiar</button>
              )}
            </div>
            ...
            {personalDetalle && (
              <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, marginBottom: 20, overflow: 'hidden' }}>
                <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--surface2)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <button onClick={() => setPersonalDetalleId(null)} style={{ background: 'transparent', border: 'none', color: 'var(--text)', cursor: 'pointer', fontSize: 14, fontFamily: 'var(--mono)' }}>Volver</button>
                    ...
                    {misPermisos['personal.descargar_excel'] && (
                      <>
                        ...
                        <button onClick={() => descargarExcelPersonal(personalDetalle, excelAnioPersonal)}
                          style={{ padding: '6px 12px', borderRadius: 6, background: 'var(--surface2)', border: '1px solid var(--border)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 11, cursor: 'pointer' }}>📊 Descargar Excel</button>
                      </>
                    )}
                    {misPermisos['personal.crear_editar'] && (
                      <button onClick={() => abrirFormPersonal(personalDetalle)} style={{ padding: '6px 12px', borderRadius: 6, background: 'var(--surface2)', border: '1px solid var(--border)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 11, cursor: 'pointer' }}>Editar</button>
                    )}
                  </div>
                </div>
                <div style={{ padding: 16 }}>
                    ...
                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                      {esRecomendador(personalDetalle) && (
                        <button
                          ...
                          style={{ padding: '6px 12px', borderRadius: 6, background: personalDetalle.predeterminadoRecomendador ? 'rgba(15, 74, 112, 1)' : 'var(--surface)', border: `1px solid ${personalDetalle.predeterminadoRecomendador ? 'var(--blue)' : 'var(--border)'}`, color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 11, cursor: 'pointer', fontWeight: personalDetalle.predeterminadoRecomendador ? 700 : 400 }}
                        >
                      ...
                      {esAplicador(personalDetalle) && (
                        <button
                          ...
                          style={{ padding: '6px 12px', borderRadius: 6, background: personalDetalle.predeterminadoAplicador ? 'rgba(31, 74, 40, 1)' : 'var(--surface)', border: `1px solid ${personalDetalle.predeterminadoAplicador ? 'var(--green)' : 'var(--border)'}`, color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 11, cursor: 'pointer', fontWeight: personalDetalle.predeterminadoAplicador ? 700 : 400 }}
                        >
                  ...
                  {esAplicador(personalDetalle) && tratamientosDePersonal(personalDetalle.id, 'aplicado').length > 0 && (
                    <div>
                      <div style={{ fontSize: 11, fontFamily: 'var(--mono)', color: 'var(--green)', textShadow: 'var(--green-outline)', fontWeight: 700, marginBottom: 8 }}>TRATAMIENTOS APLICADOS ({tratamientosDePersonal(personalDetalle.id, 'aplicado').length})</div>
                      {tratamientosDePersonal(personalDetalle.id, 'aplicado').map(t => {
                        ...
                        return (
                          <div key={t.id} style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '8px 12px', marginBottom: 6 }}>
                            <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 3 }}>
                              <span style={{ fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--text)', fontWeight: 700 }}>{t.fecha}</span>
                              <span style={{ fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--green)', textShadow: 'var(--green-outline)', background: 'rgba(31, 74, 40,0.08)', border: '1px solid rgba(31, 74, 40,0.2)', borderRadius: 4, padding: '1px 6px' }}>{t.tipo}</span>
                            </div>
                  ...
                  .map(p => {
                    ...
                    return (
                      <div
                        ...
                        style={{ background: 'var(--surface)', border: `1px solid ${p.activo ? 'var(--border)' : 'rgba(110, 36, 23,0.2)'}`, borderRadius: 10, padding: 16, cursor: 'pointer', opacity: p.activo ? 1 : 0.6, transition: 'border-color 0.2s' }}
                        onMouseEnter={e => (e.currentTarget.style.borderColor = 'var(--green)')}
                        onMouseLeave={e => (e.currentTarget.style.borderColor = p.activo ? 'var(--border)' : 'rgba(110, 36, 23,0.2)')}
                      >
                        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 10 }}>
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 3, flexShrink: 0 }}>
                            {p.predeterminadoRecomendador && <span style={{ fontSize: 8, fontFamily: 'var(--mono)', color: 'var(--blue)', background: 'rgba(15, 74, 112,0.1)', borderRadius: 3, padding: '1px 4px' }}>⭐ REC</span>}
                            {p.predeterminadoAplicador && <span style={{ fontSize: 8, fontFamily: 'var(--mono)', color: 'var(--green)', textShadow: 'var(--green-outline)', background: 'rgba(31, 74, 40,0.08)', borderRadius: 3, padding: '1px 4px' }}>⭐ APL</span>}
                            {!p.activo && <span style={{ fontSize: 8, fontFamily: 'var(--mono)', color: '#6E2417', background: 'rgba(110, 36, 23,0.08)', borderRadius: 3, padding: '1px 4px' }}>INAC.</span>}
                          </div>
                        </div>
                        <div style={{ display: 'flex', gap: 6, marginBottom: 10, flexWrap: 'wrap' }}>
                          <span style={{ fontSize: 9, fontFamily: 'var(--mono)', color: 'var(--blue)', background: 'rgba(15, 74, 112,0.08)', border: '1px solid rgba(15, 74, 112,0.15)', borderRadius: 4, padding: '2px 7px' }}>{p.tipoContrato}</span>
                          {p.nroRopo && <span style={{ fontSize: 9, fontFamily: 'var(--mono)', color: 'var(--green)', textShadow: 'var(--green-outline)', background: 'rgba(31, 74, 40,0.06)', border: '1px solid rgba(31, 74, 40,0.15)', borderRadius: 4, padding: '2px 7px' }}>ROPO: {p.nroRopo}</span>}
                          {p.fincaId && fincas.find(f => f.id === p.fincaId) && (
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: nRec !== null && nApl !== null ? '1fr 1fr' : '1fr', gap: 6, marginBottom: 10 }}>
                          ...
                          {nApl !== null && (
                            <div style={{ background: 'var(--surface2)', borderRadius: 5, padding: '5px 8px', textAlign: 'center' }}>
                              <div style={{ fontSize: 16, fontFamily: 'var(--mono)', fontWeight: 700, color: 'var(--green)', textShadow: 'var(--green-outline)' }}>{nApl}</div>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 8 }}>
                          {misPermisos['personal.crear_editar'] && (
                            <button onClick={e => { e.stopPropagation(); desactivarPersonal(p.id) }} style={{ fontSize: 9, color: 'var(--text)', fontFamily: 'var(--mono)', background: 'transparent', border: 'none', cursor: 'pointer', textDecoration: 'underline' }}>
            ...
            {formPersonal && (
              <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
                <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, width: '100%', maxWidth: 480, maxHeight: '90vh', overflow: 'auto', display: 'flex', flexDirection: 'column' }}>
                  <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontFamily: 'var(--mono)', fontSize: 13, fontWeight: 700, color: 'var(--green)', textShadow: 'var(--green-outline)' }}>{personalEditar ? 'EDITAR PERSONAL' : 'AÑADIR PERSONAL'}</span>
                    <button onClick={() => setFormPersonal(false)} style={{ background: 'transparent', border: 'none', color: 'var(--text)', cursor: 'pointer', fontSize: 18 }}>X</button>
                  </div>
                  <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 14 }}>
                    {[
                    ...
                    <div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 5 }}>
                        ...
                        <button type="button" onClick={() => { setVerifRegistro('ropo'); setVerifNif(pDni); setVerifResultado(null); setVerifError(''); setShowVerifRomaRopo(true) }} style={{ display: 'flex', alignItems: 'center', gap: 4, background: 'transparent', border: 'none', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 10, fontWeight: 700, cursor: 'pointer', padding: 0 }}>
                          🔍 ROPO
                        </button>
                      </div>
                    ...
                    <div>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                        {FUNCIONES_PERSONAL.map(f => (
                          <label key={f} style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', padding: '7px 10px', borderRadius: 6, background: pFunciones.includes(f) ? 'var(--green-dim)' : 'var(--surface2)', border: `1px solid ${pFunciones.includes(f) ? 'rgba(31, 74, 40,0.3)' : 'var(--border)'}`, transition: 'all 0.15s' }}>
                            <input
                              type="checkbox"
                    ...
                    <div>
                      ...
                      <select value={pContrato} onChange={e => setPContrato(e.target.value)} style={{ width: '100%', background: 'var(--surface2)', border: `1px solid ${!pContrato && pError ? '#6E2417' : 'var(--border)'}`, borderRadius: 6, padding: '9px 12px', color: 'var(--text)', fontSize: 13, fontFamily: 'var(--sans)', outline: 'none' }}>
                          ...
                          {fincas.map(f => {
                            ...
                            return (
                              <button key={f.id} type="button"
                                ...
                                style={{ padding: '6px 12px', borderRadius: 14, background: sel ? 'rgba(15, 74, 112, 1)' : 'var(--surface2)', border: `1px solid ${sel ? 'var(--blue)' : 'var(--border)'}`, color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 11, cursor: 'pointer' }}>
                                {f.nombre}
                              </button>
                            )
                          })}
                    ...
                    {pError && <div style={{ padding: '8px 12px', borderRadius: 6, background: 'rgba(110, 36, 23,0.08)', border: '1px solid rgba(110, 36, 23,0.2)', color: '#6E2417', fontSize: 12, fontFamily: 'var(--mono)' }}>{pError}</div>}
                  </div>
                  <div style={{ padding: '14px 20px', borderTop: '1px solid var(--border)', display: 'flex', gap: 10 }}>
                    <button onClick={guardarPersonal} style={{ flex: 1, padding: '11px', borderRadius: 8, background: 'var(--green)', border: 'none', color: 'var(--text)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>GUARDAR</button>
                    <button onClick={() => setFormPersonal(false)} style={{ flex: 1, padding: '11px', borderRadius: 8, background: 'var(--surface2)', border: '1px solid var(--border)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 13, cursor: 'pointer' }}>CANCELAR</button>
                  </div>
                </div>
              </div>
            )}
...
{/* - Pestaña EQUIPOS - */}
          ...
          <div style={{ width: '100%', height: '100%', overflow: 'auto', padding: isMobile ? 12 : 24, background: 'var(--bg)' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              ...
              {misPermisos['maquinaria.crear_editar'] && (
                <button onClick={() => abrirFormEquipo()} style={{ padding: '8px 16px', borderRadius: 8, background: 'var(--green)', border: 'none', color: 'var(--text)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 12, cursor: 'pointer', letterSpacing: '0.06em' }}>+ AÑADIR EQUIPO</button>
              )}
            </div>

            {/* Filtros */}
            <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
              <select value={filtroTipoEquipo} onChange={e => setFiltroTipoEquipo(e.target.value)} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 6, padding: '6px 10px', color: 'var(--text)', fontSize: 11, fontFamily: 'var(--mono)', outline: 'none' }}>
                <option value="">Todos los tipos</option>
                {TIPOS_MAQUINARIA.map(t => <option key={t} value={t}>{t}</option>)}
              </select>
              {fincas.length > 0 && (
                <select value={filtroFincaEquipo} onChange={e => setFiltroFincaEquipo(e.target.value)} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 6, padding: '6px 10px', color: 'var(--text)', fontSize: 11, fontFamily: 'var(--mono)', outline: 'none' }}>
                  <option value="">Todas las fincas</option>
                  {fincas.map(f => <option key={f.id} value={f.id}>{f.nombre}</option>)}
                </select>
              )}
              {(filtroTipoEquipo || filtroFincaEquipo) && (
                <button onClick={() => { setFiltroTipoEquipo(''); setFiltroFincaEquipo('') }} style={{ padding: '6px 12px', borderRadius: 6, background: 'transparent', border: '1px solid rgba(110, 36, 23, 1)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 11, cursor: 'pointer' }}>Limpiar</button>
              )}
            </div>
            ...
            {equipoDetalle && (
              <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, marginBottom: 20, overflow: 'hidden' }}>
                <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--surface2)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <button onClick={() => setEquipoDetalleId(null)} style={{ background: 'transparent', border: 'none', color: 'var(--text)', cursor: 'pointer', fontSize: 14, fontFamily: 'var(--mono)' }}>Volver</button>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    {misPermisos['maquinaria.crear_editar'] && (
                      <button onClick={() => abrirFormEquipo(equipoDetalle)} style={{ padding: '6px 12px', borderRadius: 6, background: 'var(--surface2)', border: '1px solid var(--border)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 11, cursor: 'pointer' }}>Editar</button>
                    )}
                    {misPermisos['maquinaria.eliminar'] && (
                      <button onClick={() => eliminarEquipo(equipoDetalle.id)} style={{ padding: '6px 12px', borderRadius: 6, background: 'transparent', border: '1px solid rgba(110, 36, 23, 1)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 11, cursor: 'pointer' }}>Eliminar</button>
                    )}
                  </div>
                </div>
                <div style={{ padding: 16 }}>
                  ...
                  {tratamientosDeEquipo(equipoDetalle.id).length > 0 && (
                    <div>
                      <div style={{ fontSize: 11, fontFamily: 'var(--mono)', color: 'var(--green)', textShadow: 'var(--green-outline)', fontWeight: 700, marginBottom: 8 }}>HISTORIAL DE TRATAMIENTOS ({tratamientosDeEquipo(equipoDetalle.id).length})</div>
                  ...
                  .map(e => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div style={{ display: 'flex', gap: 6, marginBottom: 10, flexWrap: 'wrap' }}>
                          {e.titularidad && <span style={{ fontSize: 9, fontFamily: 'var(--mono)', color: 'var(--blue)', background: 'rgba(15, 74, 112,0.08)', border: '1px solid rgba(15, 74, 112,0.15)', borderRadius: 4, padding: '2px 7px' }}>{e.titularidad}</span>}
                        ...
                        {nTrat > 0 && (
                          <div style={{ background: 'var(--surface2)', borderRadius: 5, padding: '5px 8px', textAlign: 'center', marginBottom: 8 }}>
                            <div style={{ fontSize: 16, fontFamily: 'var(--mono)', fontWeight: 700, color: 'var(--green)', textShadow: 'var(--green-outline)' }}>{nTrat}</div>
                            <div style={{ fontSize: 8, fontFamily: 'var(--mono)', color: 'var(--muted)' }}>TRATAMIENTOS</div>
                          </div>
                        )}
                        <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                          {misPermisos['maquinaria.eliminar'] && (
                            <button onClick={ev => { ev.stopPropagation(); eliminarEquipo(e.id) }} style={{ fontSize: 9, color: 'var(--text)', fontFamily: 'var(--mono)', background: 'transparent', border: 'none', cursor: 'pointer', textDecoration: 'underline' }}>
                              Eliminar
                            </button>
                          )}
                        </div>
            ...
            {formEquipo && (
              <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
                <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, width: '100%', maxWidth: 480, maxHeight: '90vh', overflow: 'auto', display: 'flex', flexDirection: 'column' }}>
                  <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontFamily: 'var(--mono)', fontSize: 13, fontWeight: 700, color: 'var(--green)', textShadow: 'var(--green-outline)' }}>{equipoEditar ? 'EDITAR EQUIPO' : 'AÑADIR EQUIPO'}</span>
                    <button onClick={() => setFormEquipo(false)} style={{ background: 'transparent', border: 'none', color: 'var(--text)', cursor: 'pointer', fontSize: 18 }}>X</button>
                  </div>
                  <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 14 }}>
                    <div>
                      ...
                      <select value={eTipo} onChange={e => { setETipo(e.target.value); setESubtipo('') }} style={{ width: '100%', background: 'var(--surface2)', border: `1px solid ${!eTipo && eError ? '#6E2417' : 'var(--border)'}`, borderRadius: 6, padding: '9px 12px', color: 'var(--text)', fontSize: 13, fontFamily: 'var(--sans)', outline: 'none' }}>
                        <option value="" disabled>Selecciona tipo...</option>
                        {TIPOS_MAQUINARIA.map(t => <option key={t} value={t}>{t}</option>)}
                      </select>
                    </div>
                    {subtiposPorTipo(eTipo).length > 0 && (
                      <div>
                        ...
                        <select value={eSubtipo} onChange={e => setESubtipo(e.target.value)} style={{ width: '100%', background: 'var(--surface2)', border: `1px solid ${!eSubtipo && eError ? '#6E2417' : 'var(--border)'}`, borderRadius: 6, padding: '9px 12px', color: 'var(--text)', fontSize: 13, fontFamily: 'var(--sans)', outline: 'none' }}>
                    ...
                    <div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 5 }}>
                        ...
                        <button type="button" onClick={() => { setVerifRegistro('roma'); setVerifNif(eDniCif); setVerifResultado(null); setVerifError(''); setShowVerifRomaRopo(true) }} style={{ display: 'flex', alignItems: 'center', gap: 4, background: 'transparent', border: 'none', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 10, fontWeight: 700, cursor: 'pointer', padding: 0 }}>
                          🔍 ROMA
                        </button>
                      </div>
                    ...
                    <div>
                      ...
                      <select value={eTitularidad} onChange={e => setETitularidad(e.target.value as 'Propia' | 'Externa' | '')} style={{ width: '100%', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '9px 12px', color: 'var(--text)', fontSize: 13, fontFamily: 'var(--sans)', outline: 'none' }}>
                        <option value="">Sin especificar</option>
                    ...
                    {eTitularidad === 'Propia' && (
                      <div>
                        ...
                        {fincas.length === 0 ? (
                          ...
                          <select value={eFincaId} onChange={e => setEFincaId(e.target.value)} style={{ width: '100%', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '9px 12px', color: 'var(--text)', fontSize: 13, fontFamily: 'var(--sans)', outline: 'none' }}>
                            <option value="">Sin finca asignada</option>
                            {fincas.map(f => <option key={f.id} value={f.id}>{f.nombre}</option>)}
                          </select>
                        )}
                    ...
                    {eError && <div style={{ padding: '8px 12px', borderRadius: 6, background: 'rgba(110, 36, 23,0.08)', border: '1px solid rgba(110, 36, 23,0.2)', color: '#6E2417', fontSize: 12, fontFamily: 'var(--mono)' }}>{eError}</div>}
                  </div>
                  <div style={{ padding: '14px 20px', borderTop: '1px solid var(--border)', display: 'flex', gap: 10 }}>
                    <button onClick={guardarEquipo} style={{ flex: 1, padding: '11px', borderRadius: 8, background: 'var(--green)', border: 'none', color: 'var(--text)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>GUARDAR</button>
                    <button onClick={() => setFormEquipo(false)} style={{ flex: 1, padding: '11px', borderRadius: 8, background: 'var(--surface2)', border: '1px solid var(--border)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 13, cursor: 'pointer' }}>CANCELAR</button>
                  </div>
                </div>
              </div>
            )}
            ...
            <div style={{ display: 'flex', height: 'calc(100vh - 140px)', gap: isMobile ? 0 : 16, flexDirection: isMobile ? 'column' : 'row' }}>
              ...
              <div style={{ display: isMobile && tareaSel ? 'none' : 'flex', width: isMobile ? '100%' : '25%', minWidth: isMobile ? undefined : 260, flex: isMobile ? 1 : undefined, height: isMobile ? undefined : '100%', minHeight: 0, flexDirection: 'column', borderRight: isMobile ? 'none' : '1px solid var(--border)', paddingRight: isMobile ? 0 : 12, overflow: 'hidden' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                  ...
                  <button onClick={abrirNuevaTarea} style={{ padding: '6px 12px', borderRadius: 6, background: 'var(--green)', border: 'none', color: 'var(--text)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 11, cursor: 'pointer' }}>+ NUEVA TAREA</button>
                </div>
              ...
              <div style={{ display: isMobile && !tareaSel ? 'none' : 'block', flex: 1, minHeight: 0, overflow: 'auto', padding: isMobile ? '12px 0 24px' : '0 4px' }}>
                {isMobile && tareaSel && (
                  <button onClick={() => setTareaSeleccionadaId(null)} style={{ background: 'transparent', border: 'none', color: 'var(--text)', cursor: 'pointer', fontSize: 14, fontFamily: 'var(--mono)', marginBottom: 12, padding: 0 }}>← volver a la lista</button>
                )}
                {!tareaSel ? (
                  ...
                  return (
                    <div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16, flexWrap: 'wrap', gap: 10 }}>
                        ...
                        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                          {tareaSel.estado !== 'realizada' && (
                            <button onClick={() => iniciarMarcarTareaRealizada(tareaSel)} style={{ padding: '8px 14px', borderRadius: 6, background: 'var(--green)', border: 'none', color: 'var(--text)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 11, cursor: 'pointer' }}>✓ MARCAR COMO REALIZADA</button>
                          )}
                          <button onClick={() => tareaSel.tipo === 'Tratamiento fitosanitario' ? abrirEditarTareaTratamiento(tareaSel) : abrirEditarTareaGeneral(tareaSel)} style={{ padding: '8px 14px', borderRadius: 6, background: 'var(--surface2)', border: '1px solid var(--border)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 11, cursor: 'pointer' }}>EDITAR</button>
                          <button onClick={() => eliminarTarea(tareaSel.id)} style={{ padding: '8px 14px', borderRadius: 6, background: 'transparent', border: '1px solid rgba(110, 36, 23, 1)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 11, cursor: 'pointer' }}>ELIMINAR</button>
                        </div>
                      </div>
                      ...
                      {tareaSel.tratamientoId && (
                        <div style={{ fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--green)', textShadow: 'var(--green-outline)' }}>✓ Registrada en TRATAMIENTOS → REALIZADOS</div>
                      )}
                    </div>
                  )
                })()}
              ...
              {formNuevaTareaVisible && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 2500, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
                  <div style={{ background: 'var(--surface)', border: '1px solid var(--green)', borderRadius: 12, width: '100%', maxWidth: 380, padding: 24 }}>
                    <div style={{ fontFamily: 'var(--mono)', fontSize: 14, fontWeight: 700, color: 'var(--green)', textShadow: 'var(--green-outline)', marginBottom: 14 }}>+ NUEVA TAREA</div>
                    ...
                    <select value={nuevaTareaTipo} onChange={e => setNuevaTareaTipo(e.target.value)}
                      style={{ width: '100%', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '9px 12px', color: 'var(--text)', fontSize: 13, fontFamily: 'var(--sans)', outline: 'none', marginBottom: 16 }}>
                      <option value="" disabled>Selecciona tipo...</option>
                      {TIPOS_TAREA.map(t => <option key={t} value={t}>{t}</option>)}
                    </select>
                    <div style={{ display: 'flex', gap: 10 }}>
                      <button onClick={continuarNuevaTarea} disabled={!nuevaTareaTipo} style={{ flex: 1, padding: 11, borderRadius: 8, background: 'var(--green)', border: 'none', color: 'var(--text)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 13, cursor: nuevaTareaTipo ? 'pointer' : 'default', opacity: nuevaTareaTipo ? 1 : 0.5 }}>CONTINUAR</button>
                      <button onClick={() => setFormNuevaTareaVisible(false)} style={{ flex: 1, padding: 11, borderRadius: 8, background: 'var(--surface2)', border: '1px solid var(--border)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 13, cursor: 'pointer' }}>CANCELAR</button>
                    </div>
                  </div>
                </div>
              )}
              ...
              {tarFormVisible && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 2500, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
                  <div style={{ background: 'var(--surface)', border: '1px solid var(--green)', borderRadius: 12, width: '100%', maxWidth: 480, maxHeight: '90vh', overflow: 'auto' }}>
                    <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <span style={{ fontFamily: 'var(--mono)', fontSize: 13, fontWeight: 700, color: 'var(--green)', textShadow: 'var(--green-outline)' }}>{tarEditar ? '✏ EDITAR TAREA' : '+ NUEVA TAREA'}: {tarTipo}</span>
                      <button onClick={() => setTarFormVisible(false)} style={{ background: 'transparent', border: 'none', color: 'var(--text)', cursor: 'pointer', fontSize: 18 }}>✕</button>
                    </div>
                    <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 14 }}>
                      {tarTipo === 'Otros' && (
                        <div>
                          <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase' }}>Tipo de tarea <span style={{ color: '#6E2417' }}>*</span></label>
                            ...
                            {misParcelas.filter(p => p.fincaId === tarFincaId).map(p => {
                              ...
                              return (
                                <button key={p.id} onClick={() => setTarParcelaIds(sel ? tarParcelaIds.filter(id => id !== p.id) : [...tarParcelaIds, p.id])}
                                  style={{ padding: '5px 10px', borderRadius: 14, background: sel ? 'rgba(31, 74, 40, 1)' : 'var(--surface)', border: `1px solid ${sel ? 'var(--green)' : 'var(--border)'}`, color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 10, cursor: 'pointer' }}>
                      ...
                      <div>
                        <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase' }}>Fecha a realizar <span style={{ color: '#6E2417' }}>*</span></label>
                        <input type="date" value={tarFechaPrevista} onChange={e => setTarFechaPrevista(e.target.value)}
                          style={{ width: '100%', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '9px 12px', color: 'var(--text)', fontSize: 13, fontFamily: 'var(--sans)', outline: 'none' }} />
                      </div>
                      {tarTipo === 'Alternativa no química' && (
                        <div style={{ padding: '10px 12px', borderRadius: 8, background: 'var(--surface2)', border: '1px solid var(--border)', display: 'flex', flexDirection: 'column', gap: 10 }}>
                          <div style={{ fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--green)', textShadow: 'var(--green-outline)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Datos para el Cuaderno de Campo (3.1 bis — solo con asesoramiento)</div>
                          <div>
                            <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase' }}>Plaga</label>
                            <input type="text" value={tarPlaga} onChange={e => setTarPlaga(e.target.value)} placeholder="Plaga o enfermedad..." style={{ width: '100%', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 6, padding: '9px 12px', color: 'var(--text)', fontSize: 13, fontFamily: 'var(--sans)', outline: 'none' }}/>
                          </div>
                          <div>
                            <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase' }}>Justificación de la actuación</label>
                            <textarea value={tarJustificacionActuacion} onChange={e => setTarJustificacionActuacion(e.target.value)} rows={2} placeholder="Motivo de la actuación..." style={{ width: '100%', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 6, padding: '9px 12px', color: 'var(--text)', fontSize: 13, fontFamily: 'var(--sans)', outline: 'none', resize: 'vertical' }}/>
                          </div>
                          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                            <div>
                              <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase' }}>Tipo de medida</label>
                              <input type="text" value={tarTipoMedida} onChange={e => setTarTipoMedida(e.target.value)} placeholder="Ej: Biológica, Cultural..." style={{ width: '100%', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 6, padding: '9px 12px', color: 'var(--text)', fontSize: 13, fontFamily: 'var(--sans)', outline: 'none' }}/>
                            </div>
                            <div>
                              <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase' }}>Intensidad de la medida</label>
                              <input type="text" value={tarIntensidadMedida} onChange={e => setTarIntensidadMedida(e.target.value)} placeholder="Ej: Puntual, Total..." style={{ width: '100%', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 6, padding: '9px 12px', color: 'var(--text)', fontSize: 13, fontFamily: 'var(--sans)', outline: 'none' }}/>
                            </div>
                          </div>
                          <div>
                            <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase' }}>Eficacia</label>
                            <select value={tarEficacia} onChange={e => setTarEficacia(e.target.value as any)} style={{ width: '100%', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 6, padding: '9px 12px', color: 'var(--text)', fontSize: 13, fontFamily: 'var(--sans)', outline: 'none' }}>
                          ...
                          {equipos.map(e => {
                            ...
                            return (
                              <button key={e.id} onClick={() => setTarEquipoIds(sel ? tarEquipoIds.filter(id => id !== e.id) : [...tarEquipoIds, e.id])}
                                style={{ padding: '5px 10px', borderRadius: 14, background: sel ? 'rgba(15, 74, 112, 1)' : 'var(--surface)', border: `1px solid ${sel ? 'var(--blue)' : 'var(--border)'}`, color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 10, cursor: 'pointer' }}>
                                {e.nombre}
                              </button>
                            )
                          })}
                      ...
                      {tarEditar?.estado === 'realizada' && (
                        <div>
                          <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase' }}>
                            Tiempo dedicado {!tarEjecutorId && <span style={{ color: '#8A4B12', textTransform: 'none' }}>(selecciona quién ejecuta para poder imputar horas)</span>}
                          </label>
                          {renderEditorDias(tarHorasTrabajadas, setTarHorasTrabajadas)}
                        </div>
                      )}
                      {tarError && <div style={{ color: '#6E2417', fontSize: 11, fontFamily: 'var(--mono)' }}>{tarError}</div>}
                      <div style={{ display: 'flex', gap: 10 }}>
                        <button onClick={guardarTareaGeneral} style={{ flex: 1, padding: 11, borderRadius: 8, background: 'var(--green)', border: 'none', color: 'var(--text)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>GUARDAR</button>
                        <button onClick={() => setTarFormVisible(false)} style={{ flex: 1, padding: 11, borderRadius: 8, background: 'var(--surface2)', border: '1px solid var(--border)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 13, cursor: 'pointer' }}>CANCELAR</button>
                      </div>
                    </div>
                  </div>
                </div>
        ...
        {pestana === 'riego' && (
          <RiegoTab session={session} fincas={fincas} misParcelas={misParcelas} ubicaciones={ubicaciones} isMobile={isMobile}
            sectorInicial={riegoSectorFocus} onConsumirSectorInicial={() => setRiegoSectorFocus(null)} misPermisos={misPermisos} />
        )}

        {pestana === 'cosechas' && (() => {
          const cultivosDisponibles = Array.from(new Set(cosechas.map(c => c.cultivo))).sort((a, b) => a.localeCompare(b))
          const cosechasTab = cosechas
            .filter(c => !filtroCampanaCosechasTab || c.campanaId === filtroCampanaCosechasTab)
            .filter(c => !filtroFincaCosechasTab || misParcelas.find(p => p.id === c.parcelaId)?.fincaId === filtroFincaCosechasTab)
            .filter(c => !filtroCultivoCosechasTab || c.cultivo === filtroCultivoCosechasTab)

          // Agrupa las cosechas filtradas en "partes": una por cultivo+campaña+finca, aunque
          // vengan de parcelas distintas. Cada parte muestra el total sumado y, al expandir,
          // el desglose por parcela/fecha (entradas) y las comercializaciones conjuntas.
          const partesMap = new Map<string, Cosecha[]>()
          cosechasTab.forEach(c => {
            const key = parteKeyDeCosecha(c)
            if (!partesMap.has(key)) partesMap.set(key, [])
            partesMap.get(key)!.push(c)
          })
          const partes = Array.from(partesMap.entries()).map(([key, entradasSinOrdenar]) => {
            const entradas = [...entradasSinOrdenar].sort((a, b) => (b.fechaCosecha || '').localeCompare(a.fechaCosecha || ''))
            const ref = entradas[0]
            const finca = fincas.find(f => f.id === misParcelas.find(p => p.id === ref.parcelaId)?.fincaId)
            const cantidadConocida = entradas.reduce((acc, c) => acc + (c.cantidad ? Number(c.cantidad) : 0), 0)
            const haySinRegistrar = entradas.some(c => !c.cantidad)
            const comercializadoTotal = entradas.reduce((acc, c) => acc + totalComercializado(c.id), 0)
            const pendienteTotal = Math.max(0, cantidadConocida - comercializadoTotal)
            return { key, cultivo: ref.cultivo, variedad: ref.variedad, campanaId: ref.campanaId, unidad: ref.unidad || 'kg', finca, entradas, cantidadConocida, haySinRegistrar, comercializadoTotal, pendienteTotal }
          }).sort((a, b) => (b.entradas[0]?.fechaCosecha || '').localeCompare(a.entradas[0]?.fechaCosecha || ''))

          return (
            <div key="cosechas" style={{ flex: 1, height: '100%', overflow: 'auto', padding: isMobile ? 12 : 24, background: 'var(--bg)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10, marginBottom: 16 }}>
                <div>
                  <div style={{ fontFamily: 'var(--mono)', fontSize: 16, fontWeight: 700, color: 'var(--text)' }}>🌾 Cosecha comercializada</div>
                  <div style={{ fontSize: 11, color: 'var(--muted)', fontFamily: 'var(--mono)' }}>{cosechas.length} cosecha{cosechas.length !== 1 ? 's' : ''} registrada{cosechas.length !== 1 ? 's' : ''}</div>
                </div>
                {misPermisos['cosechas.crear_editar_eliminar'] && (
                  <button onClick={() => abrirNuevaCosecha()} style={{ padding: '7px 14px', borderRadius: 6, background: 'rgba(31, 74, 40, 1)', border: '1px solid var(--green)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>+ NUEVA COSECHA</button>
                )}
              </div>

              <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
                <select value={filtroCampanaCosechasTab} onChange={e => setFiltroCampanaCosechasTab(e.target.value)}
                  style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '8px 12px', color: 'var(--text)', fontSize: 12, fontFamily: 'var(--mono)', outline: 'none' }}>
                  <option value="">Campaña: Todas</option>
                  {campanas.map(c => <option key={c.id} value={c.id}>{c.nombre}</option>)}
                </select>
                <select value={filtroFincaCosechasTab} onChange={e => setFiltroFincaCosechasTab(e.target.value)}
                  style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '8px 12px', color: 'var(--text)', fontSize: 12, fontFamily: 'var(--mono)', outline: 'none' }}>
                  <option value="">Finca: Todas</option>
                  {fincas.map(f => <option key={f.id} value={f.id}>{f.nombre}</option>)}
                </select>
                <select value={filtroCultivoCosechasTab} onChange={e => setFiltroCultivoCosechasTab(e.target.value)}
                  style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '8px 12px', color: 'var(--text)', fontSize: 12, fontFamily: 'var(--mono)', outline: 'none' }}>
              ...
              {cosechas.length === 0 ? (
                <div style={{ fontSize: 12, color: 'var(--muted)', fontFamily: 'var(--mono)', padding: '20px 0' }}>Todavía no hay cosechas registradas. Empieza con "+ NUEVA COSECHA".</div>
              ) : partes.length === 0 ? (
                <div style={{ fontSize: 12, color: 'var(--muted)', fontFamily: 'var(--mono)', padding: '20px 0' }}>Ninguna cosecha coincide con estos filtros.</div>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {partes.map(parte => {
                    const expandida = cosechaExpandidaId === parte.key
                    const comercializacionesParte = comercializaciones
                      .filter(x => parte.entradas.some(e => e.id === x.cosechaId))
                      .sort((a, b) => (b.fecha || '').localeCompare(a.fecha || ''))
                    const cosechaComFormAbierta = comFormCosechaId ? cosechas.find(c => c.id === comFormCosechaId) : null
                    const formAqui = comFormParteKey === parte.key || (!!cosechaComFormAbierta && parteKeyDeCosecha(cosechaComFormAbierta) === parte.key)
                    return (
                      <div key={parte.key} style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 8, overflow: 'hidden' }}>
                        <div onClick={() => setCosechaExpandidaId(expandida ? null : parte.key)} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '11px 14px', cursor: 'pointer' }}>
                          <div>
                            <span style={{ fontSize: 12, fontFamily: 'var(--mono)', color: 'var(--text)', fontWeight: 700 }}>
                              🌿 {formatCultivoVariedad(parte.cultivo, parte.variedad)}{parte.cantidadConocida > 0 ? ` — ${formatCantidadEs(parte.cantidadConocida)} ${parte.unidad}${parte.haySinRegistrar ? '+' : ''}` : ' — Sin registrar'}
                            </span>
                            <div style={{ fontSize: 11, fontFamily: 'var(--mono)', color: 'var(--muted)', marginTop: 3 }}>
                              📁 {parte.finca?.nombre || 'Finca eliminada'} · {campanas.find(camp => camp.id === parte.campanaId)?.nombre || 'Sin campaña'} · {parte.entradas.length} entrada{parte.entradas.length !== 1 ? 's' : ''}
                            </div>
                          </div>
                          <span style={{ color: 'var(--muted)', fontSize: 12 }}>{expandida ? '▲' : '▼'}</span>
                        </div>
                        {expandida && (
                          <div style={{ padding: '0 14px 14px', borderTop: '1px solid var(--border)' }}>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 3, padding: '10px 0', fontSize: 11, fontFamily: 'var(--mono)' }}>
                              <div style={{ color: 'var(--muted)' }}>Materia cosechada: <span style={{ color: 'var(--text)' }}>{parte.cantidadConocida > 0 ? `${formatCantidadEs(parte.cantidadConocida)} ${parte.unidad}` : 'Sin registrar'}{parte.haySinRegistrar ? ' (hay entradas sin registrar)' : ''}</span></div>
                              <div style={{ color: 'var(--muted)' }}>Materia comercializada: <span style={{ color: 'var(--text)' }}>{parte.comercializadoTotal > 0 ? `${formatCantidadEs(parte.comercializadoTotal)} ${parte.unidad}` : 'Sin registrar'}</span></div>
                              <div style={{ color: 'var(--muted)' }}>Pendiente de comercializar: <span style={{ color: 'var(--text)' }}>{parte.cantidadConocida > 0 ? `${formatCantidadEs(parte.pendienteTotal)} ${parte.unidad}` : '—'}</span></div>
                            </div>

                            <div style={{ fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--green)', textShadow: 'var(--green-outline)', letterSpacing: '0.06em', marginBottom: 6, fontWeight: 700 }}>ENTRADAS ({parte.entradas.length})</div>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 5, marginBottom: 10 }}>
                              {parte.entradas.map(e => {
                                const p = misParcelas.find(x => x.id === e.parcelaId)
                                return (
                                  <div key={e.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 5, padding: '6px 8px', fontSize: 10, fontFamily: 'var(--mono)', gap: 8 }}>
                                    <span style={{ color: 'var(--text)' }}>📁 {p?.nombre || p?.cultivo || 'Parcela eliminada'} · {formatFechaEs(e.fechaCosecha)} → {e.cantidad ? `${formatCantidadEs(Number(e.cantidad))} ${e.unidad}` : 'Sin registrar'}</span>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
                                      {misPermisos['cosechas.crear_editar_eliminar'] && (
                                        <button onClick={() => abrirEditarCosecha(e)} style={{ background: 'transparent', border: 'none', color: 'var(--text)', cursor: 'pointer', fontSize: 10 }} title="Editar">✏</button>
                                      )}
                                      {misPermisos['cosechas.crear_editar_eliminar'] && (
                                        <button onClick={() => eliminarCosecha(e.id)} style={{ background: 'transparent', border: 'none', color: 'var(--text)', cursor: 'pointer', fontSize: 10 }} title="Eliminar">✕</button>
                                      )}
                                    </div>
                                  </div>
                                )
                            ...
                            {misPermisos['cosechas.comercializacion.ver'] && (<>
                            <div style={{ fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--blue)', letterSpacing: '0.06em', marginBottom: 6, fontWeight: 700 }}>COMERCIALIZACIONES</div>
                            {comercializacionesParte.length === 0 && !formAqui && (
                              <div style={{ fontSize: 10, color: 'var(--muted)', fontFamily: 'var(--mono)', marginBottom: 6 }}>Sin comercializaciones registradas.</div>
                            )}
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 5, marginBottom: 8 }}>
                              {comercializacionesParte.map(com => (
                                <div key={com.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 5, padding: '6px 8px', fontSize: 10, fontFamily: 'var(--mono)' }}>
                                  <span style={{ color: 'var(--text)' }}>{formatFechaEs(com.fecha)} → {formatCantidadEs(Number(com.cantidad))} {com.unidad}{com.comprador ? ` → ${com.comprador}${com.compradorDniNif ? ` (${com.compradorDniNif})` : ''}` : ''}</span>
                                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                                    {misPermisos['cosechas.comercializacion.crear_editar_eliminar'] && (
                            ...
                            {formAqui && misPermisos['cosechas.comercializacion.crear_editar_eliminar'] ? (
                              <div style={{ background: 'var(--surface)', border: '1px solid var(--blue)', borderRadius: 6, padding: 8, display: 'flex', flexDirection: 'column', gap: 6 }}>
                                {!comEditarId && (
                                  <div style={{ fontSize: 9, color: 'var(--muted)', fontFamily: 'var(--mono)' }}>Se descuenta primero de la cosecha más antigua de este parte.</div>
                                )}
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
                                  <input type="date" value={comFecha} onChange={e => setComFecha(e.target.value)}
                                    style={{ width: '100%', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 5, padding: '6px 8px', color: 'var(--text)', fontSize: 11, fontFamily: 'var(--mono)', outline: 'none' }} />
                              ...
                              <button onClick={() => abrirNuevaComercializacionParte(parte.key)} style={{ padding: '6px 10px', borderRadius: 5, background: 'transparent', border: '1px solid var(--blue)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 10, fontWeight: 700, cursor: 'pointer' }}>+ Comercialización</button>
                            ) : null}
                            </>)}
                          </div>
                        )}
        ...
        {cosFormVisible && (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 2500, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
            <div style={{ background: 'var(--surface)', border: '1px solid var(--green)', borderRadius: 12, width: '100%', maxWidth: 420, padding: 24, maxHeight: '90vh', overflowY: 'auto' }}>
              <div style={{ fontFamily: 'var(--mono)', fontSize: 13, fontWeight: 700, color: 'var(--green)', textShadow: 'var(--green-outline)', marginBottom: 16 }}>{cosEditar ? 'EDITAR COSECHA' : '+ NUEVA COSECHA'}</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                  <div>
                    <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase' }}>1. Campaña <span style={{ color: '#6E2417' }}>*</span></label>
                    <select value={cosCampanaId} onChange={e => { setCosCampanaId(e.target.value); setCosParcelaId('') }}
                      style={{ width: '100%', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '9px 12px', color: 'var(--text)', fontSize: 13, fontFamily: 'var(--sans)', outline: 'none' }}>
                  ...
                  <div>
                    <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase' }}>2. Finca <span style={{ color: '#6E2417' }}>*</span></label>
                    <select value={cosFincaId} onChange={e => { setCosFincaId(e.target.value); setCosParcelaId('') }}
                      style={{ width: '100%', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '9px 12px', color: 'var(--text)', fontSize: 13, fontFamily: 'var(--sans)', outline: 'none' }}>
                      <option value="">Selecciona</option>
                      {fincas.map(f => <option key={f.id} value={f.id}>{f.nombre}</option>)}
                    </select>
                ...
                <div>
                  <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase' }}>3. Parcela <span style={{ color: '#6E2417' }}>*</span></label>
                  {!cosCampanaId || !cosFincaId ? (
                    <div style={{ fontSize: 11, color: 'var(--muted)', fontFamily: 'var(--mono)', padding: '8px 10px', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6 }}>
                      Selecciona campaña y finca para ver las parcelas disponibles.
                    </div>
                  ) : (
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, maxHeight: 120, overflow: 'auto', padding: 8, background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6 }}>
                      {misParcelas.filter(p => p.fincaId === cosFincaId).map(p => {
                        const sel = cosParcelaId === p.id
                        const { cultivo } = cultivoVariedadEnCampana(p, cosCampanaId)
                        return (
                          <button key={p.id} onClick={() => setCosParcelaId(p.id)}
                            style={{ padding: '5px 10px', borderRadius: 14, background: sel ? 'rgba(31, 74, 40, 1)' : 'var(--surface)', border: `1px solid ${sel ? 'var(--green)' : 'var(--border)'}`, color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 10, cursor: 'pointer' }}>
                            {p.nombre ? `${p.nombre} (${cultivo})` : cultivo}
                          </button>
                        )
                      })}
                      {misParcelas.filter(p => p.fincaId === cosFincaId).length === 0 && <span style={{ fontSize: 10, color: 'var(--muted)', fontFamily: 'var(--mono)' }}>No hay parcelas guardadas en esta finca</span>}
                    </div>
                  )}
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase' }}>Fecha de cosecha <span style={{ color: '#6E2417' }}>*</span></label>
                  <input type="date" value={cosFecha} onChange={e => setCosFecha(e.target.value)}
                    style={{ width: '100%', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '9px 12px', color: 'var(--text)', fontSize: 13, fontFamily: 'var(--sans)', outline: 'none' }} />
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 10 }}>
                  <div>
                    <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase' }}>Producción cosechada</label>
                    <input type="number" step="any" min="0" value={cosCantidad} onChange={e => setCosCantidad(e.target.value)} placeholder="Opcional"
                      style={{ width: '100%', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '9px 12px', color: 'var(--text)', fontSize: 13, fontFamily: 'var(--sans)', outline: 'none' }} />
                  </div>
                  <div>
                    <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase' }}>Unidad</label>
                    <select value={cosUnidad} onChange={e => setCosUnidad(e.target.value)}
                      style={{ width: '100%', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '9px 12px', color: 'var(--text)', fontSize: 13, fontFamily: 'var(--sans)', outline: 'none' }}>
                      {UNIDADES_COSECHA.map(u => <option key={u} value={u}>{u}</option>)}
                    </select>
                  </div>
                ...
                <div>
                  <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase' }}>Observaciones</label>
                  <textarea value={cosObservaciones} onChange={e => setCosObservaciones(e.target.value)} rows={2} placeholder="Notas adicionales (opcional)..."
                    style={{ width: '100%', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '9px 12px', color: 'var(--text)', fontSize: 13, fontFamily: 'var(--sans)', outline: 'none', resize: 'vertical' }} />
                </div>
                {cosError && <div style={{ color: '#6E2417', fontSize: 11, fontFamily: 'var(--mono)' }}>{cosError}</div>}
                <div style={{ display: 'flex', gap: 10, marginTop: 4 }}>
                  <button onClick={guardarCosecha} style={{ flex: 1, padding: 11, borderRadius: 8, background: 'var(--green)', border: 'none', color: 'var(--text)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>GUARDAR</button>
                  <button onClick={() => { setCosFormVisible(false); setCosEditar(null) }} style={{ flex: 1, padding: 11, borderRadius: 8, background: 'var(--surface2)', border: '1px solid var(--border)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 13, cursor: 'pointer' }}>CANCELAR</button>
                </div>
              </div>
            </div>
          </div>
      ...
      {cuadernoModalVisible && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 3500, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: 'var(--surface)', border: '1px solid var(--green)', borderRadius: 12, width: '100%', maxWidth: 460, maxHeight: '90vh', overflow: 'auto' }}>
            <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontFamily: 'var(--mono)', fontSize: 13, fontWeight: 700, color: 'var(--green)', textShadow: 'var(--green-outline)' }}>📄 CUADERNO DE CAMPO</span>
            ...
            <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div style={{ fontSize: 11, color: 'var(--muted)', fontFamily: 'var(--mono)', lineHeight: 1.6 }}>
                Genera el Cuaderno de Campo en Word con los tratamientos y parcelas de la campaña y las fincas seleccionadas. Solo se incluyen los cultivos asociados a esa campaña (actuales o históricos).
              </div>
              <div>
                <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase' }}>Campaña</label>
                {campanas.length === 0 ? (
              ...
              <div>
                <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase' }}>
                  Fincas {fincas.length > 0 && <span style={{ color: '#6E2417', textTransform: 'none' }}>*</span>}
                </label>
                {fincas.length === 0 ? (
                ...
                {fincas.length > 0 && cuadernoFincaIds.length === 0 && (
              ...
              {(() => {
                if (cuadernoFincaIds.length === 0) return null
                const fincasSel = fincas.filter(f => cuadernoFincaIds.includes(f.id))
                const datasets = fincasSel.map(f => datosExplotacionPorFinca[f.id]).filter(Boolean) as DatosExplotacion[]
                ...
                if (!todasIguales) {
                  return (
                    <div style={{ padding: '8px 12px', borderRadius: 6, background: 'rgba(110, 36, 23,0.08)', border: '1px solid rgba(110, 36, 23,0.2)', color: '#6E2417', fontSize: 11, fontFamily: 'var(--mono)', lineHeight: 1.5 }}>
                ...
                return (
                  <div style={{ padding: '8px 12px', borderRadius: 6, background: 'rgba(31, 74, 40,0.08)', border: '1px solid rgba(31, 74, 40,0.25)', color: 'var(--green)', textShadow: 'var(--green-outline)', fontSize: 11, fontFamily: 'var(--mono)' }}>
                    ✓ Datos de la explotación cargados{fincasSel.length > 1 ? ' (coinciden en todas las fincas seleccionadas)' : ''}.
                  </div>
                )
              })()}
              <button onClick={() => abrirFormDatosExplotacion(cuadernoFincaIds)} disabled={cuadernoFincaIds.length === 0}
                style={{ fontSize: 11, fontFamily: 'var(--mono)', color: 'var(--text)', background: 'transparent', border: 'none', cursor: cuadernoFincaIds.length === 0 ? 'default' : 'pointer', textDecoration: cuadernoFincaIds.length === 0 ? 'none' : 'underline', alignSelf: 'flex-start', padding: 0 }}>
                🌾 {cuadernoFincaIds.length === 0 ? 'Elige una finca para editar sus datos de la explotación' : `Editar datos de la explotación (${cuadernoFincaIds.length === 1 ? fincas.find(f => f.id === cuadernoFincaIds[0])?.nombre : `${cuadernoFincaIds.length} fincas`})`}
              </button>
            </div>
            <div style={{ padding: '14px 20px', borderTop: '1px solid var(--border)', display: 'flex', gap: 10 }}>
              <button onClick={generarCuaderno} disabled={generandoCuaderno || !cuadernoCampanaId || (fincas.length > 0 && cuadernoFincaIds.length === 0)}
                style={{ flex: 1, padding: 11, borderRadius: 8, background: 'var(--green)', border: 'none', color: 'var(--text)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 13, cursor: (generandoCuaderno || !cuadernoCampanaId || (fincas.length > 0 && cuadernoFincaIds.length === 0)) ? 'default' : 'pointer', opacity: (generandoCuaderno || !cuadernoCampanaId || (fincas.length > 0 && cuadernoFincaIds.length === 0)) ? 0.6 : 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
      ...
      {formDatosExplotacion && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 3500, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: 'var(--surface)', border: '1px solid var(--green)', borderRadius: 12, width: '100%', maxWidth: 640, maxHeight: '90vh', overflow: 'auto' }}>
            <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontFamily: 'var(--mono)', fontSize: 13, fontWeight: 700, color: 'var(--green)', textShadow: 'var(--green-outline)' }}>🌾 DATOS DE LA EXPLOTACIÓN</span>
              <button onClick={() => setFormDatosExplotacion(false)} style={{ background: 'transparent', border: 'none', color: 'var(--text)', cursor: 'pointer', fontSize: 18, lineHeight: 1 }}>✕</button>
            </div>
            <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 18 }}>
              <div style={{ fontSize: 11, color: 'var(--muted)', fontFamily: 'var(--mono)', lineHeight: 1.6 }}>
                Estos datos rellenan automáticamente la cabecera del Cuaderno de Campo descargable (secciones 1.1, Titular y 1.5). Se guardan por finca — se aplicarán a: <strong style={{ color: 'var(--text)' }}>{deFincaIds.map(id => fincas.find(f => f.id === id)?.nombre).filter(Boolean).join(', ') || '—'}</strong>. Todos los campos son opcionales.
              </div>
              ...
              <div>
                <div style={{ fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--blue)', fontWeight: 700, letterSpacing: '0.06em', marginBottom: 8 }}>1.5 AGRUPACIÓN O ENTIDAD DE ASESORAMIENTO</div>
            ...
            <div style={{ padding: '14px 20px', borderTop: '1px solid var(--border)', display: 'flex', gap: 10 }}>
              <button onClick={guardarDatosExplotacion} disabled={deGuardando || deFincaIds.length === 0}
                style={{ flex: 1, padding: 11, borderRadius: 8, background: 'var(--green)', border: 'none', color: 'var(--text)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 13, cursor: (deGuardando || deFincaIds.length === 0) ? 'default' : 'pointer', opacity: (deGuardando || deFincaIds.length === 0) ? 0.6 : 1 }}>
                {deGuardando ? 'Guardando...' : '💾 GUARDAR'}
              </button>
              <button onClick={() => setFormDatosExplotacion(false)} style={{ flex: 1, padding: 11, borderRadius: 8, background: 'var(--surface2)', border: '1px solid var(--border)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 13, cursor: 'pointer' }}>Cancelar</button>
            </div>
          </div>
        </div>
      )}
      ...
      {horasPopupModo && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 3500, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: 'var(--surface)', border: '1px solid var(--green)', borderRadius: 12, width: '100%', maxWidth: 540, maxHeight: '90vh', overflow: 'auto' }}>
            <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontFamily: 'var(--mono)', fontSize: 13, fontWeight: 700, color: 'var(--green)', textShadow: 'var(--green-outline)' }}>🕒 TIEMPO DE TRABAJO</span>
              <button onClick={() => guardarHorasPopup(true)} disabled={horasPopupGuardando} style={{ background: 'transparent', border: 'none', color: 'var(--text)', cursor: 'pointer', fontSize: 18, lineHeight: 1 }}>✕</button>
            </div>
            <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div style={{ fontSize: 11, color: 'var(--muted)', fontFamily: 'var(--mono)', lineHeight: 1.6 }}>
              ...
              <div>
                ...
                {horasPopupPersonalId ? (
                  ...
                  <div style={{ background: 'rgba(138, 75, 18,0.08)', border: '1px solid rgba(138, 75, 18,0.25)', borderRadius: 6, padding: '9px 12px', color: '#8A4B12', fontSize: 11, fontFamily: 'var(--mono)' }}>
              ...
              {horasPopupError && (
                <div style={{ padding: '8px 12px', borderRadius: 6, background: 'rgba(110, 36, 23,0.08)', border: '1px solid rgba(110, 36, 23,0.2)', color: '#6E2417', fontSize: 12, fontFamily: 'var(--mono)' }}>
                  ⚠ {horasPopupError}
                </div>
              )}
            </div>
            <div style={{ padding: '14px 20px', borderTop: '1px solid var(--border)', display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              <button onClick={() => guardarHorasPopup(false)} disabled={horasPopupGuardando}
                style={{ flex: 1, padding: 11, borderRadius: 8, background: 'var(--green)', border: 'none', color: 'var(--text)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 13, cursor: horasPopupGuardando ? 'default' : 'pointer', opacity: horasPopupGuardando ? 0.6 : 1 }}>
              ...
              <button onClick={() => guardarHorasPopup(true)} disabled={horasPopupGuardando}
                style={{ flex: 1, padding: 11, borderRadius: 8, background: 'var(--surface2)', border: '1px solid var(--border)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 13, cursor: horasPopupGuardando ? 'default' : 'pointer' }}>
...
{/* - Modal formulario tratamiento - */}
      {formTrat && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, width: '100%', maxWidth: 480, maxHeight: '90vh', overflow: 'auto', display: 'flex', flexDirection: 'column' }}>
            <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontFamily: 'var(--mono)', fontSize: 13, fontWeight: 700, color: 'var(--green)', textShadow: 'var(--green-outline)' }}>{tratEditar ? '✏ EDITAR TRATAMIENTO' : '🧪 NUEVO TRATAMIENTO'}</span>
              <button onClick={() => { setFormTrat(false); setTareaEnCreacionId(null) }} style={{ background: 'transparent', border: 'none', color: 'var(--text)', cursor: 'pointer', fontSize: 18 }}>✕</button>
            </div>
            <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 10px', borderRadius: 6, background: 'var(--surface2)', border: '1px solid var(--border)' }}>
                ...
                <label htmlFor="chk-realizada" style={{ fontSize: 12, fontFamily: 'var(--mono)', color: tRealizada ? 'var(--green)' : 'var(--muted)', textShadow: tRealizada ? 'var(--green-outline)' : 'none', cursor: 'pointer' }}>
                  {tRealizada ? '✓ Realizada' : '☐ No realizada (se guarda como tarea pendiente)'}
                </label>
              </div>
              <div>
                <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Tipo de tratamiento <span style={{ color: '#6E2417' }}>*</span></label>
                <select value={tTipo} onChange={e => setTTipo(e.target.value)} style={{ width: '100%', background: 'var(--surface2)', border: `1px solid ${!tTipo && tError ? '#6E2417' : 'var(--border)'}`, borderRadius: 6, padding: '9px 12px', color: 'var(--text)', fontSize: 13, fontFamily: 'var(--sans)', outline: 'none' }}>
                  <option value="" disabled>Selecciona tipo...</option>
                  {TIPOS_TRATAMIENTO.map(t => <option key={t} value={t}>{t}</option>)}
                </select>
              </div>
              <div>
                <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Campaña <span style={{ color: '#6E2417' }}>*</span></label>
                {campanas.length === 0 ? (
                  <div style={{ fontSize: 11, color: '#6E2417', fontFamily: 'var(--mono)', padding: '6px 0' }}>
                    No hay campañas creadas. Crea una desde "Mis Parcelas" antes de registrar el tratamiento.
                  </div>
                ) : (
                  <select value={tCampanaId} onChange={e => setTCampanaId(e.target.value)} style={{ width: '100%', background: 'var(--surface2)', border: `1px solid ${!tCampanaId && tError ? '#6E2417' : 'var(--border)'}`, borderRadius: 6, padding: '9px 12px', color: 'var(--text)', fontSize: 13, fontFamily: 'var(--sans)', outline: 'none' }}>
              ...
              <div style={{ position: 'relative' }}>
                <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.08em' }}>
                  ...
                  {fitoSeleccionado && <span style={{ marginLeft: 8, color: 'var(--green)', textShadow: 'var(--green-outline)', fontSize: 9 }}>✓ {fitoSeleccionado.num_registro}</span>}
                </label>
                <div style={{ display: 'flex', gap: 6 }}>
                  <input
                    type="text"
                  ...
                  <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 3000, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 6, boxShadow: '0 8px 24px rgba(0,0,0,0.4)', maxHeight: 220, overflowY: 'auto', marginTop: 2 }}>
                    {fitoResultados.map(p => (
                      <div
                        ...
                        style={{ padding: '10px 14px', cursor: 'pointer', borderBottom: '1px solid var(--border)', borderLeft: p.eliminado ? '3px solid #6E2417' : '3px solid transparent' }}
                        onMouseEnter={e => (e.currentTarget.style.background = 'var(--surface2)')}
                        onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}
                      >
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
                          <span style={{ fontSize: 12, fontWeight: 600, color: p.eliminado ? '#6E2417' : 'var(--text)' }}>{p.nombre}</span>
                          {p.eliminado && <span style={{ fontSize: 8, fontFamily: 'var(--mono)', color: '#6E2417', background: 'rgba(110, 36, 23,0.1)', border: '1px solid rgba(110, 36, 23,0.3)', borderRadius: 3, padding: '1px 4px' }}>NO AUTORIZADO</span>}
                          {p.tiene_pdf && <span style={{ fontSize: 8, fontFamily: 'var(--mono)', color: 'var(--blue)', background: 'rgba(15, 74, 112,0.08)', border: '1px solid rgba(15, 74, 112,0.2)', borderRadius: 3, padding: '1px 4px' }}>PDF</span>}
                        </div>
                ...
                {fitoSeleccionado && (
                  <div style={{ marginTop: 6 }}>
                    {fitoSeleccionado.eliminado && (
                      <div style={{ padding: '8px 12px', borderRadius: 6, background: 'rgba(110, 36, 23,0.1)', border: '1px solid rgba(110, 36, 23,0.3)', marginBottom: 6 }}>
                        <div style={{ fontSize: 11, fontFamily: 'var(--mono)', fontWeight: 700, color: '#6E2417', marginBottom: 3 }}>PRODUCTO NO AUTORIZADO</div>
                        <div style={{ fontSize: 10, fontFamily: 'var(--mono)', color: '#6E2417', lineHeight: 1.5 }}>
                    ...
                    <div style={{ padding: '8px 10px', borderRadius: 6, background: fitoSeleccionado.eliminado ? 'rgba(110, 36, 23,0.04)' : 'rgba(31, 74, 40,0.06)', border: `1px solid ${fitoSeleccionado.eliminado ? 'rgba(110, 36, 23,0.2)' : 'rgba(31, 74, 40,0.2)'}`, fontSize: 10, fontFamily: 'var(--mono)' }}>
                      <div style={{ color: fitoSeleccionado.eliminado ? '#6E2417' : 'var(--green)', textShadow: fitoSeleccionado.eliminado ? 'none' : 'var(--green-outline)', fontWeight: 700, marginBottom: 3 }}>
                      ...
                      {fitoSeleccionado.tiene_pdf && fitoSeleccionado.pdf_url && (
                        <button
                          ...
                          style={{ marginTop: 8, padding: '5px 12px', borderRadius: 5, background: 'rgba(15, 74, 112, 1)', border: '1px solid rgba(15, 74, 112, 1)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 10, cursor: pdfCargando ? 'wait' : 'pointer', fontWeight: 700 }}
                        >
              ...
              <div>
                <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Producto utilizado <span style={{ color: '#6E2417' }}>*</span></label>
              ...
              <div>
                <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Materia activa <span style={{ color: '#6E2417' }}>*</span></label>
                <div style={{ display: 'flex', gap: 6 }}>
              ...
              {/* Resultado MRL */}
              {mrlResultado && (
                <div style={{ padding: '10px 12px', borderRadius: 7, background: mrlResultado.encontrado ? 'rgba(31, 74, 40,0.06)' : 'rgba(92, 63, 8,0.06)', border: `1px solid ${mrlResultado.encontrado ? 'rgba(31, 74, 40,0.25)' : 'rgba(92, 63, 8,0.25)'}` }}>
                  {mrlResultado.encontrado ? (
                    <>
                      <div style={{ fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--green)', textShadow: 'var(--green-outline)', fontWeight: 700, marginBottom: 4 }}>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: 16, color: 'var(--green)', textShadow: 'var(--green-outline)', fontWeight: 700 }}>{mrlResultado.mrl} {mrlResultado.unidad}</div>
                        </div>
                        <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 8 }}>
                <div>
                  <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Dosis <span style={{ color: '#6E2417' }}>*</span></label>
              ...
              {(unidadNecesitaCaldo(tUnidad) || unidadNecesitaSemilla(tUnidad)) && (
                <div style={{ padding: '10px 12px', borderRadius: 7, background: 'rgba(31, 74, 40,0.06)', border: '1px solid rgba(31, 74, 40,0.25)' }}>
                  {unidadNecesitaCaldo(tUnidad) && (
                    <div>
                      <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Litros de caldo aplicados/ha <span style={{ color: '#6E2417' }}>*</span></label>
                  ...
                  {unidadNecesitaSemilla(tUnidad) && (
                    <div style={{ marginTop: unidadNecesitaCaldo(tUnidad) ? 10 : 0 }}>
                      <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Kg de semilla sembrada/ha <span style={{ color: '#6E2417' }}>*</span></label>
              ...
              <div style={{ padding: '10px 12px', borderRadius: 7, background: 'var(--surface2)', border: '1px solid var(--border)', overflow: 'hidden', boxSizing: 'border-box' }}>
                ...
                <div style={{ marginTop: isMobile ? 12 : 8 }}>
                  ...
                  {tPorcentajeMaximo.trim() && Number(tPorcentajeAplicado) > Number(tPorcentajeMaximo) && (
                    <div style={{ marginTop: 6, padding: '7px 10px', borderRadius: 6, background: 'rgba(110, 36, 23,0.1)', border: '1px solid rgba(110, 36, 23,0.3)', fontSize: 10, color: '#6E2417', fontFamily: 'var(--mono)' }}>
              ...
              <div>
                <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Fecha del tratamiento <span style={{ color: '#6E2417' }}>*</span></label>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                ...
                <div>
                  ...
                  <select value={tEficacia} onChange={e => setTEficacia(e.target.value as any)} style={{ width: '100%', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '9px 12px', color: 'var(--text)', fontSize: 13, fontFamily: 'var(--sans)', outline: 'none' }}>
                    <option value="">Sin indicar</option>
                    <option value="Buena">Buena</option>
                    <option value="Regular">Regular</option>
                    <option value="Mala">Mala</option>
              ...
              {/* Asesoramiento GIP — al final del cuestionario, ya que solo aplica a cultivos y
                  superficies objeto de asesoramiento (cuaderno de campo, sección 3.1 bis). Al venir
                  de un tratamiento fitosanitario, esto rellena el bloque "Alternativas químicas de
                  intervención" de esa sección (deja en blanco el de alternativas no químicas). */}
              <div style={{ padding: '10px 12px', borderRadius: 8, background: 'var(--surface2)', border: '1px solid var(--border)' }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
                  <input type="checkbox" checked={tConAsesor} onChange={e => setTConAsesor(e.target.checked)} style={{ accentColor: 'var(--green)' }}/>
                  <span style={{ fontSize: 11, fontFamily: 'var(--mono)', color: 'var(--text)' }}>Hay un asesor — cultivo/superficie objeto de asesoramiento (rellena el punto 3.1 bis del Cuaderno de Campo)</span>
                </label>
                {tConAsesor && (
                  <div style={{ marginTop: 10 }}>
                    <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Justificación de la actuación</label>
                    <textarea value={tJustificacionActuacion} onChange={e => setTJustificacionActuacion(e.target.value)} rows={2} placeholder="Ej: Superación de umbrales, condiciones meteorológicas..." style={{ width: '100%', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 6, padding: '9px 12px', color: 'var(--text)', fontSize: 13, fontFamily: 'var(--sans)', outline: 'none', resize: 'vertical' }}/>
                  </div>
                )}
              </div>
              {!tratParcelaCtx && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  {/* Paso 1: Cultivo */}
                  <div>
                    <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.08em' }}>{pasoNumCultivo}. Cultivo <span style={{ color: '#6E2417' }}>*</span></label>
                  ...
                  {tCultivoSeleccionado && pasoFincaVisible && (
                    <div>
                      <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.08em' }}>{pasoNumFinca}. Finca <span style={{ color: '#6E2417' }}>*</span></label>
                  ...
                  {tCultivoSeleccionado && (
                    <div>
                      <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.08em' }}>
                        {pasoNumParcelas}. Parcelas <span style={{ color: '#6E2417' }}>*</span>
                        {parcelasFiltradas.length > 0 && <button onClick={() => setTParcelas(parcelasFiltradas.map(p => p.id))} style={{ marginLeft: 8, fontSize: 9, color: 'var(--text)', background: 'transparent', border: 'none', cursor: 'pointer', textDecoration: 'underline' }}>Seleccionar todas</button>}
                      </label>
                      {parcelasFiltradas.length === 0 ? (
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 5, maxHeight: 160, overflowY: 'auto' }}>
                          {parcelasFiltradas.map(p => {
                            ...
                            return (
                              <label key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', padding: '6px 8px', borderRadius: 5, background: tParcelas.includes(p.id) ? 'var(--green-dim)' : 'var(--surface2)', border: `1px solid ${tParcelas.includes(p.id) ? 'rgba(31, 74, 40,0.3)' : 'var(--border)'}`, transition: 'all 0.15s' }}>
                      ...
                      {tParcelas.length > 0 && (
                        <div style={{ marginTop: 8, padding: '6px 10px', borderRadius: 5, background: 'var(--surface2)', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--green)', textShadow: 'var(--green-outline)' }}>
              ...
              {tError && <div style={{ padding: '8px 12px', borderRadius: 6, background: 'rgba(110, 36, 23,0.08)', border: '1px solid rgba(110, 36, 23,0.2)', color: '#6E2417', fontSize: 12, fontFamily: 'var(--mono)' }}>⚠ {tError}</div>}
            </div>
            <div style={{ padding: '14px 20px', borderTop: '1px solid var(--border)', display: 'flex', gap: 10 }}>
              <button onClick={guardarTratamiento} style={{ flex: 1, padding: '11px', borderRadius: 8, background: 'var(--green)', border: 'none', color: 'var(--text)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>💾 GUARDAR</button>
              <button onClick={() => { setFormTrat(false); setTareaEnCreacionId(null) }} style={{ flex: 1, padding: '11px', borderRadius: 8, background: 'var(--surface2)', border: '1px solid var(--border)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 13, cursor: 'pointer' }}>CANCELAR</button>
            </div>
          </div>
        </div>
      )}
      ...
      {fichaVisible && fitoSeleccionado?.pdf_url && pdfBlobUrl && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 9000, display: 'flex', flexDirection: 'column', background: 'rgba(0,0,0,0.92)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 16px', background: 'var(--surface)', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
            <span style={{ fontFamily: 'var(--mono)', fontSize: 12, fontWeight: 700, color: 'var(--green)', textShadow: 'var(--green-outline)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '60%' }}>
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexShrink: 0 }}>
              <a
                ...
                style={{ padding: '6px 12px', borderRadius: 6, background: 'rgba(31, 74, 40,0.1)', border: '1px solid rgba(31, 74, 40,0.3)', color: 'var(--green)', textShadow: 'var(--green-outline)', fontFamily: 'var(--mono)', fontSize: 11, textDecoration: 'none', fontWeight: 700 }}
              >
                Descargar
              </a>
              <button
                onClick={() => { setFichaVisible(false); if (pdfBlobUrl) { URL.revokeObjectURL(pdfBlobUrl); setPdfBlobUrl(null) } }}
                style={{ padding: '6px 14px', borderRadius: 6, background: 'rgba(110, 36, 23, 1)', border: '1px solid rgba(110, 36, 23, 1)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 13, cursor: 'pointer', fontWeight: 700 }}
              >
                X
              </button>
            </div>
