...
interface Tratamiento {
  ...
  tipoMedida?: string               // Tipo de medida adoptada (cuaderno de campo, sección 3.1 bis)
  intensidadMedida?: string         // Intensidad de la medida (cuaderno de campo, sección 3.1 bis)
}
...
export default function Home() {
  ...
  const [tTipoMedida, setTTipoMedida] = useState('')
  const [tIntensidadMedida, setTIntensidadMedida] = useState('')
  ...
  const generarCuaderno = async () => {
    ...
    try {
      ...
      // objeto de asesoramiento GIP) — combina los tratamientos fitosanitarios marcados como "Con
      // asesor" y las tareas de tipo "Alternativa no química" (medidas de control no químicas
      // recomendadas por el asesor), ambos ya realizados dentro de la campaña/fincas elegidas.
      ...
      const registrosAsesoradosTrat = tratamientosAsesorados.map(t => {
        ...
        return {
          fechaISO: t.fecha,
          fila: [
            especie, variedad, idsParcelasTexto(t.parcelaIds),
            supCult > 0 ? formatSupEs(supCult, 2) : '',
            st > 0 ? formatSupEs(st, 2) : '',
            t.problemaFitosanitario || '', t.justificacionActuacion || '', t.tipoMedida || '', t.intensidadMedida || '',
            formatFechaEs(t.fecha), t.producto || '', t.numRegistro || '', dosisTexto, t.eficacia || '', t.observaciones || '',
          ],
        }
      })
      ...
      const registrosAlternativa = tareasAlternativaSel.map(t => {
        ...
        return {
          ...
          fila: [
            ...
            supCult > 0 ? formatSupEs(supCult, 2) : '',
            supCult > 0 ? formatSupEs(supCult, 2) : '',
            t.plaga || '', t.justificacionActuacion || '', t.tipoMedida || '', t.intensidadMedida || '',
            formatFechaEs(t.fechaPrevista), '', '', '', t.eficacia || '', t.comentarios || '',
          ],
        }
      })
      ...
      const secciones = [
        ...
        },
        {
          ...
          colWidths: [1, 1, 0.8, 1, 1, 1, 1.6, 1, 1.2, 0.9, 1.2, 0.9, 0.9, 1, 1.4], rows: filas31bis, filasVaciasMin: 5,
          notas: filas31bis.length > 0
            ? ['Incluye los tratamientos fitosanitarios marcados como "Con asesor" y las tareas de tipo "Alternativa no química" — solo aplica a cultivos con asesoramiento de gestión integrada de plagas.']
  ...
  const upsertTratamientoDB = async (t: Tratamiento): Promise<string | null> => {
    ...
    try {
      const { error } = await supabase.from('tratamientos').upsert({
        ...
        tipo_medida: t.tipoMedida || null, intensidad_medida: t.intensidadMedida || null,
      })
      if (error) {
  ...
  const abrirFormTrat = (parcelaId?: string, trat?: Tratamiento, personalId?: string) => {
    ...
    setTTipoMedida(trat?.tipoMedida || '')
    setTIntensidadMedida(trat?.intensidadMedida || '')
  ...
  const guardarTratamiento = async () => {
    ...
    const datosComunes = {
      ...
      tipoMedida: tConAsesor ? tTipoMedida.trim() || undefined : undefined,
      intensidadMedida: tConAsesor ? tIntensidadMedida.trim() || undefined : undefined,
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, width: '100%', maxWidth: 480, maxHeight: '90vh', overflow: 'auto', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              {/* Asesoramiento GIP — al final del cuestionario, ya que solo aplica a cultivos y
                  superficies objeto de asesoramiento (cuaderno de campo, sección 3.1 bis). */}
                ...
                {tConAsesor && (
                  <div style={{ marginTop: 10, display: 'flex', flexDirection: 'column', gap: 10 }}>
                    <div>
                      ...
                      <textarea value={tJustificacionActuacion} onChange={e => setTJustificacionActuacion(e.target.value)} rows={2} placeholder="Motivo por el que el asesor recomienda esta actuación..." style={{ width: '100%', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 6, padding: '9px 12px', color: 'var(--text)', fontSize: 13, fontFamily: 'var(--sans)', outline: 'none', resize: 'vertical' }}/>
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                      <div>
                        <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Tipo de medida</label>
                        <input type="text" value={tTipoMedida} onChange={e => setTTipoMedida(e.target.value)} placeholder="Ej: Química" style={{ width: '100%', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 6, padding: '9px 12px', color: 'var(--text)', fontSize: 13, fontFamily: 'var(--sans)', outline: 'none' }}/>
                      </div>
                      <div>
                        <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Intensidad de la medida</label>
                        <input type="text" value={tIntensidadMedida} onChange={e => setTIntensidadMedida(e.target.value)} placeholder="Ej: Puntual, Total..." style={{ width: '100%', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 6, padding: '9px 12px', color: 'var(--text)', fontSize: 13, fontFamily: 'var(--sans)', outline: 'none' }}/>
                      </div>
                    </div>
                  </div>
                )}
