...
const AbonadoTab = dynamic(() => import('./components/AbonadoTab'), { ssr: false })
...
type Pestaña = 'mapa' | 'mis_parcelas' | 'tratamientos' | 'personal' | 'equipos' | 'riego' | 'tareas' | 'cosechas' | 'abonado' | 'permisos'
...
export default function Home() {
  ...
  const [tareaAbonadoAAbrir, setTareaAbonadoAAbrir] = useState<Tarea | null>(null)
  ...
  const [horasPopupModo, setHorasPopupModo] = useState<
    null | { tipo: 'tarea_completar'; tareaId: string } | { tipo: 'tratamiento_guardado'; tratamientoId: string } | { tipo: 'abonado_guardado'; abonoPlanId: string }
  >(null)
    ...
    try {
        ...
        if (tareasRes.data) {
          setTareas(tareasRes.data.map((t: any) => ({
            ...
            abonoPlanId: t.abono_plan_id || undefined, borradorAbonado: t.borrador_abonado || undefined,
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              {data.map((d, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <span style={{ width: 8, height: 8, borderRadius: '50%', background: colorDeCultivo(d.cultivo), border: '1px solid #000000', flexShrink: 0 }} />
  ...
  const continuarNuevaTarea = () => {
    ...
    if (nuevaTareaTipo === 'Tratamiento fitosanitario') {
      ...
      abrirFormTrat()
      setTRealizada(false)
    } else if (nuevaTareaTipo === 'Abonado') {
      setTareaAbonadoAAbrir({ id: '', estado: 'pendiente', borradorAbonado: undefined } as Tarea)
    } else {
      setTarEditar(null)
  ...
  const iniciarMarcarTareaRealizada = (t: Tarea) => {
    ...
    if (t.tipo === 'Abonado') {
      // El abonado necesita repasar el plan completo (análisis, necesidades, dosis...),
      // así que "marcar como realizada" abre el mismo asistente en vez de completar directo.
      if (!t.borradorAbonado) { alert('Esta tarea no tiene datos de abonado guardados. Edítala primero.'); return }
      setTareaAbonadoAAbrir(t)
      return
    }
  ...
  const guardarHorasPopup = async (omitir: boolean = false) => {
    ...
    if (!omitir) {
      ...
      if (diasValidos.length > 0 && !horasPopupPersonalId) { setHorasPopupError(`No hay ejecutor/aplicador asignado. Asígnalo al editar ${horasPopupModo.tipo === 'tratamiento_guardado' ? 'el tratamiento' : horasPopupModo.tipo === 'abonado_guardado' ? 'el plan de abonado' : 'la tarea'} antes de registrar horas.`); return }
    }
    ...
    try {
      ...
      } else if (horasPopupModo.tipo === 'abonado_guardado') {
        const idPlan = horasPopupModo.abonoPlanId
        if (session) {
          try {
            await supabase.from('planes_abonado').update({
  ...
  const onAbonadoPlanGuardado = (abonoPlanId: string, personalEjecutaId?: string) => {
    setHorasPopupModo({ tipo: 'abonado_guardado', abonoPlanId })
    setHorasPopupDias([nuevoDiaHorario()])
    setHorasPopupPersonalId(personalEjecutaId || '')
    setHorasPopupError('')
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100dvh', overflow: 'hidden' }} onClick={e => { if (menuAbierto && !(e.target as HTMLElement).closest('[data-menu]')) setMenuAbierto(false) }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', minHeight: isMobile ? 54 : 48 }}>
            ...
            <div data-menu style={{ position: 'relative', flexShrink: 0 }}>
              ...
              {menuAbierto && (
                <div style={{ position: 'absolute', top: '100%', left: 0, zIndex: 6000, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '0 0 10px 10px', boxShadow: '0 8px 24px rgba(0,0,0,0.5)', minWidth: 220 }}>
                  {(['mapa', 'mis_parcelas', 'tratamientos', 'tareas', 'riego', 'cosechas', 'abonado', 'personal', 'equipos', 'permisos'] as Pestaña[]).filter(tab => puedeVerPestana(tab, misPermisos)).map(tab => (
                    ...
                    >
                      {tab === 'mapa' ? '🗺  MAPA' : tab === 'mis_parcelas' ? '📁  MIS PARCELAS' : tab === 'tratamientos' ? '🧪  TRATAMIENTOS' : tab === 'tareas' ? '📋  TAREAS' : tab === 'riego' ? '💧  RIEGO' : tab === 'cosechas' ? '🌾  COSECHAS' : tab === 'personal' ? '👷  PERSONAL' : tab === 'equipos' ? '🚜  EQUIPOS' : tab === 'abonado' ? '🌱  ABONADO' : '🔐  PERMISOS'}
                    </button>
                  ))}
                </div>
              )}
          ...
          {isMobile ? (
            <span style={{ flex: 1, fontFamily: 'var(--mono)', fontSize: 13, fontWeight: 700, color: 'var(--green)', textShadow: 'var(--green-outline)', paddingLeft: 4 }}>
              {pestana === 'mapa' ? '🗺 MAPA' : pestana === 'mis_parcelas' ? '📁 MIS PARCELAS' : pestana === 'tratamientos' ? '🧪 TRATAMIENTOS' : pestana === 'tareas' ? '📋 TAREAS' : pestana === 'riego' ? '💧 RIEGO' : pestana === 'cosechas' ? '🌾 COSECHAS' : pestana === 'personal' ? '👷 PERSONAL' : pestana === 'equipos' ? '🚜 EQUIPOS' : pestana === 'abonado' ? '🌱 ABONADO' : '🔐 PERMISOS'}
            </span>
          ) : (
            <div style={{ display: 'flex', flex: 1, minWidth: 0, overflowX: 'auto', overflowY: 'hidden', scrollbarWidth: 'thin' }}>
              {(['mapa', 'mis_parcelas', 'tratamientos', 'tareas', 'riego', 'cosechas', 'abonado', 'personal', 'equipos', 'permisos'] as Pestaña[]).filter(tab => puedeVerPestana(tab, misPermisos)).map(tab => (
                <button key={tab} onClick={() => setPestana(tab)}
                  style={{ padding: '12px 20px', border: 'none', background: 'transparent', fontFamily: 'var(--mono)', fontSize: 12, fontWeight: 700, cursor: 'pointer', color: 'var(--text)', borderBottom: `2px solid ${pestana === tab ? 'var(--green)' : 'transparent'}`, letterSpacing: '0.06em', transition: 'all 0.15s', whiteSpace: 'nowrap', flexShrink: 0 }}
                >
                  {tab === 'mapa' ? '🗺 MAPA' : tab === 'mis_parcelas' ? '📁 MIS PARCELAS' : tab === 'tratamientos' ? '🧪 TRATAMIENTOS' : tab === 'tareas' ? '📋 TAREAS' : tab === 'riego' ? '💧 RIEGO' : tab === 'cosechas' ? '🌾 COSECHAS' : tab === 'personal' ? '👷 PERSONAL' : tab === 'equipos' ? '🚜 EQUIPOS' : tab === 'abonado' ? '🌱 ABONADO' : '🔐 PERMISOS'}
                </button>
              ))}
      ...
      {/* - CONTENIDO - */}
      <div style={{ flex: (pestana === 'mapa' || pestana === 'mis_parcelas') ? 1 : '0 0 0px', overflow: 'hidden', display: 'flex' }}>
              ...
              <section>
                ...
                {parcGeojson && (
                  <div style={{ display: 'flex', gap: 6, marginTop: 6 }}>
                    <button onClick={deseleccionar} style={{ flex: 1, padding: '8px 16px', borderRadius: 8, background: 'var(--red)', border: 'none', color: 'var(--bg)', fontFamily: 'var(--mono)', fontSize: 12, fontWeight: 700, cursor: 'pointer', letterSpacing: '0.06em' }}>
                      ✕ DESELECCIONAR
                    </button>
                    <button onClick={abrirFormularioGuardar} style={{ flex: 1, padding: '8px 16px', borderRadius: 8, background: 'var(--green)', border: 'none', color: 'var(--bg)', fontFamily: 'var(--mono)', fontSize: 12, fontWeight: 700, cursor: 'pointer', letterSpacing: '0.06em' }}>
                      💾 GUARDAR
                    </button>
                  </div>
                )}
              ...
              {parcGeojson && (
                <>
                  ...
                  <section>
                    ...
                    <button onClick={buscarImagenes} disabled={cargando} style={{ width: '100%', padding: '8px', borderRadius: 6, background: 'rgba(15, 74, 112, 1)', border: '1px solid var(--blue)', color: 'var(--bg)', fontWeight: 700, fontSize: 11, fontFamily: 'var(--mono)', cursor: cargando ? 'wait' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
                    ...
                    {productos.length > 0 && (
                      <div style={{ marginTop: 8 }}>
                        ...
                        <button onClick={verImagenRGB} disabled={cargando} style={{ width: '100%', marginTop: 8, padding: '9px', borderRadius: 6, background: modoVista === 'rgb' ? 'rgba(15, 74, 112, 1)' : 'var(--kampo-surface-deep)', border: `1px solid var(--blue)`, color: 'var(--bg)', fontFamily: 'var(--mono)', fontSize: 11, fontWeight: 700, cursor: cargando ? 'wait' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, transition: 'all 0.2s' }}>
              ...
              {productos.length > 0 && (
                <>
                  ...
                  <section>
                    ...
                    <button onClick={calcularZonasNDVI} disabled={cargando} style={{ width: '100%', padding: '11px', borderRadius: 8, background: cargando ? 'var(--kampo-surface-deep)' : 'rgba(15, 74, 112, 1)', border: '1px solid var(--blue)', color: 'var(--bg)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 13, cursor: cargando ? 'wait' : 'pointer', letterSpacing: '0.08em', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, transition: 'all 0.2s' }}>
              ...
              {zonasConSup.length > 0 && (
                <>
                  ...
                  <section>
                    ...
                    <button onClick={() => calcularProduccion(kgHaCalculado)} disabled={Object.keys(kgHaCalculado).length === 0} style={{ width: '100%', padding: '10px', borderRadius: 8, background: Object.keys(kgHaCalculado).length > 0 ? 'var(--green)' : 'var(--kampo-surface-deep)', border: 'none', color: 'var(--bg)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 12, cursor: Object.keys(kgHaCalculado).length > 0 ? 'pointer' : 'not-allowed', letterSpacing: '0.06em', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
                      🌾 CALCULAR PRODUCCIÓN
                    </button>
                  </section>
                </>
                    ...
                    {produccion.zonas.filter((z: any) => z.kg_por_ha > 0).map((z: any) => (
                      <div key={z.zona} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '5px 0', borderBottom: '1px solid var(--border)', fontSize: 10, fontFamily: 'var(--mono)' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                          <div style={{ width: 7, height: 7, borderRadius: 2, background: z.color_hex, border: '1px solid #000000' }}/>
              ...
              {parcGeojson && (
                <>
                  ...
                  <section>
                    ...
                    <button onClick={calcularContornos} disabled={calculandoContornos} style={{ width: '100%', padding: '10px', borderRadius: 8, background: calculandoContornos ? 'var(--kampo-surface-deep)' : 'rgba(15, 74, 112, 1)', border: '1px solid var(--blue)', color: 'var(--bg)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 12, cursor: calculandoContornos ? 'wait' : 'pointer', letterSpacing: '0.06em', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, transition: 'all 0.2s' }}>
                      {calculandoContornos ? <><span className="spinner" style={{ borderTopColor: 'var(--blue)' }}/> CALCULANDO MDT...</> : '🗺 GENERAR CURVAS DE NIVEL'}
                    </button>
                    <button onClick={calcularMdtColor} disabled={calculandoMdtColor} style={{ width: '100%', marginTop: 8, padding: '10px', borderRadius: 8, background: calculandoMdtColor ? 'var(--kampo-surface-deep)' : 'rgba(15, 74, 112, 1)', border: '1px solid var(--blue)', color: 'var(--bg)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 12, cursor: calculandoMdtColor ? 'wait' : 'pointer', letterSpacing: '0.06em', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, transition: 'all 0.2s' }}>
                          ...
                          <div key={entrada.id} style={{ background: 'var(--kampo-surface-deep)', border: '1px solid var(--border)', borderRadius: 6, padding: '8px 10px', marginBottom: 6 }}>
                            ...
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 3, marginBottom: 6 }}>
                              {entrada.zonas?.map((z: any) => {
                                const zi = ZONAS_NDVI.find(zn => zn.zona === z.zona)!
                                return <div key={z.zona} title={`Z${z.zona}: ${z.sup_ha_real}ha (${z.pct}%)`} style={{ display: 'flex', alignItems: 'center', gap: 3, fontSize: 9, fontFamily: 'var(--mono)', color: 'var(--muted)' }}><div style={{ width: 7, height: 7, borderRadius: 1, background: zi?.color || '#888', border: '1px solid #000000' }}/><span>{z.pct}%</span></div>
                              })}
                            </div>
                            {entrada.geojson && (
                ...
                <div>
                  ...
                  {misPermisos['fincas.crear_editar'] && (
                    <button onClick={() => { setFincaEditar(null); setFNombre(''); setFDescripcion(''); setFError(''); setFormFinca(true) }}
                      style={{ padding: '8px 16px', borderRadius: 8, background: 'var(--green)', border: 'none', color: 'var(--bg)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 12, cursor: 'pointer', letterSpacing: '0.06em', marginRight: 8 }}>
                      + Crear finca
                    </button>
                  )}
                  {misPermisos['parcelas.crear_editar'] && (
                    <button onClick={() => setImportarExcelVisible(true)}
                      style={{ padding: '8px 16px', borderRadius: 8, background: 'var(--orange)', border: 'none', color: 'var(--bg)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 12, cursor: 'pointer', letterSpacing: '0.06em' }}>
                      📥 Importar Excel
                    </button>
                  )}
                  <div style={{ marginTop: 8, display: 'flex', gap: 8 }}>
                    <button onClick={() => { setCampanaEditar(null); setCampNombre(''); setCampFechaInicio(''); setCampFechaFin(''); setCampError(''); setFormCampanaVisible(true) }}
                      style={{ padding: '8px 16px', borderRadius: 8, background: 'var(--green)', border: 'none', color: 'var(--bg)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 12, cursor: 'pointer', letterSpacing: '0.06em' }}>
                      + Crear campaña
                    </button>
                ...
                {fincas.length > 0 && (
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    <button onClick={() => setFiltroFinca('')}
                      style={{ padding: '4px 10px', borderRadius: 20, fontFamily: 'var(--mono)', fontSize: 10, cursor: 'pointer', background: filtroFinca === '' ? 'var(--green)' : 'var(--surface2)', color: 'var(--bg)', border: '1px solid var(--border)', fontWeight: filtroFinca === '' ? 700 : 400 }}>
                      Todas
                    </button>
                    {fincas.map(f => (
                      <div key={f.id} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                        <button onClick={() => setFiltroFinca(filtroFinca === f.id ? '' : f.id)}
                          style={{ padding: '4px 10px', borderRadius: 20, fontFamily: 'var(--mono)', fontSize: 10, cursor: 'pointer', background: filtroFinca === f.id ? 'var(--green)' : 'var(--surface2)', color: 'var(--bg)', border: '1px solid var(--border)', fontWeight: filtroFinca === f.id ? 700 : 400 }}>
                          {f.nombre}
                        </button>
                        {misPermisos['fincas.crear_editar'] && (
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                    ...
                    {misPermisos['parcelas.ubicaciones.gestionar'] && (
                      <button onClick={iniciarNuevaUbicacion} disabled={fincas.length === 0}
                        style={{ padding: '8px 16px', borderRadius: 8, background: fincas.length === 0 ? 'var(--surface2)' : 'var(--blue)', border: 'none', color: fincas.length === 0 ? 'var(--muted)' : 'var(--bg)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 12, cursor: fincas.length === 0 ? 'default' : 'pointer', letterSpacing: '0.06em' }}>
                        + Añadir ubicación
                      </button>
                    )}
                  </div>
                ...
                <div style={{ marginBottom: 20 }}>
                  ...
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    <button onClick={() => setFiltroCampana('')}
                      style={{ padding: '4px 10px', borderRadius: 20, fontFamily: 'var(--mono)', fontSize: 10, cursor: 'pointer', background: filtroCampana === '' ? 'var(--amber)' : 'var(--surface2)', color: filtroCampana === '' ? 'var(--bg)' : 'var(--text)', border: '1px solid var(--border)', fontWeight: filtroCampana === '' ? 700 : 400 }}>
                      Todas
                    </button>
                    {campanas.map(c => (
                ...
                <div style={{ marginBottom: 20 }}>
                  ...
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    <button onClick={() => setFiltroCultivoParcelas('')}
                      style={{ padding: '4px 10px', borderRadius: 20, fontFamily: 'var(--mono)', fontSize: 10, cursor: 'pointer', background: filtroCultivoParcelas === '' ? 'var(--blue)' : 'var(--surface2)', color: 'var(--bg)', border: '1px solid var(--border)', fontWeight: filtroCultivoParcelas === '' ? 700 : 400 }}>
                      Todos
                    </button>
                    {Array.from(new Set(misParcelas.map(p => p.cultivo))).sort().map(c => (
                      <button key={c} onClick={() => setFiltroCultivoParcelas(filtroCultivoParcelas === c ? '' : c)}
                        style={{ padding: '4px 10px', borderRadius: 20, fontFamily: 'var(--mono)', fontSize: 10, cursor: 'pointer', background: filtroCultivoParcelas === c ? 'var(--blue)' : 'var(--surface2)', color: 'var(--bg)', border: '1px solid var(--border)', fontWeight: filtroCultivoParcelas === c ? 700 : 400 }}>
                        {c}
                      </button>
                    ))}
                  </div>
              ...
              {formUbicacionPaso === 'finca' && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
                  <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, width: '100%', maxWidth: 400, padding: 24 }}>
                    ...
                    <div style={{ display: 'flex', gap: 10 }}>
                      <button onClick={continuarUbicacionAMapa} style={{ flex: 1, padding: 11, borderRadius: 8, background: 'var(--blue)', border: 'none', color: 'var(--bg)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>CONTINUAR →</button>
                ...
                <div style={{ position: 'fixed', inset: 0, background: 'var(--bg)', zIndex: 2000 }}>
                  ...
                  <div style={{ position: 'absolute', bottom: 20, left: '50%', transform: 'translateX(-50%)', zIndex: 1000 }}>
                    <button onClick={confirmarPuntoUbicacion} disabled={!ubicacionPunto}
                      style={{ padding: '12px 28px', borderRadius: 10, background: ubicacionPunto ? 'var(--green)' : 'var(--surface2)', border: 'none', color: 'var(--bg)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 13, cursor: ubicacionPunto ? 'pointer' : 'default', letterSpacing: '0.06em' }}>
                  ...
                  <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, width: '100%', maxWidth: 440, padding: 24, maxHeight: '90vh', overflow: 'auto' }}>
                    ...
                    <div style={{ display: 'flex', gap: 10 }}>
                      <button onClick={guardarUbicacion} disabled={guardandoUbicacion}
                        style={{ flex: 1, padding: 11, borderRadius: 8, background: 'var(--green)', border: 'none', color: 'var(--bg)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 13, cursor: guardandoUbicacion ? 'default' : 'pointer', opacity: guardandoUbicacion ? 0.6 : 1 }}>
                    ...
                    <div style={{ marginBottom: 16 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                        ...
                        {misPermisos['cosechas.crear_editar_eliminar'] && (
                          <button onClick={() => abrirNuevaCosecha(parcelaDetalle)} style={{ padding: '5px 10px', borderRadius: 6, background: 'transparent', border: '1px solid var(--green)', color: 'var(--bg)', fontFamily: 'var(--mono)', fontSize: 10, fontWeight: 700, cursor: 'pointer' }}>+ NUEVA COSECHA</button>
                        )}
                      </div>
                                    ...
                                    <div style={{ padding: '0 10px 10px', borderTop: '1px solid var(--border)' }}>
                                      ...
                                      {comFormCosechaId === c.id && misPermisos['cosechas.comercializacion.crear_editar_eliminar'] ? (
                                        <div style={{ background: 'var(--surface)', border: '1px solid var(--blue)', borderRadius: 6, padding: 8, display: 'flex', flexDirection: 'column', gap: 6 }}>
                                          ...
                                          <div style={{ display: 'flex', gap: 6 }}>
                                            <button onClick={guardarComercializacion} style={{ padding: '6px 10px', borderRadius: 5, background: 'var(--blue)', border: 'none', color: 'var(--bg)', fontFamily: 'var(--mono)', fontSize: 10, fontWeight: 700, cursor: 'pointer' }}>Guardar</button>
                                        ...
                                        <button onClick={() => abrirNuevaComercializacion(c.id)} style={{ padding: '6px 10px', borderRadius: 5, background: 'transparent', border: '1px solid var(--blue)', color: 'var(--bg)', fontFamily: 'var(--mono)', fontSize: 10, fontWeight: 700, cursor: 'pointer' }}>+ Comercialización</button>
                                      ) : null}
                                      </>)}
                                    </div>
                                  )}
          ...
          <div style={{ width: '100%', height: '100%', overflow: 'auto', padding: isMobile ? 12 : 24, background: 'var(--bg)' }}>
            ...
            {personalDetalle && (
              <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, marginBottom: 20, overflow: 'hidden' }}>
                <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--surface2)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    {!isMobile && <button onClick={() => setPersonalDetalleId(null)} style={{ background: 'transparent', border: 'none', color: 'var(--text)', cursor: 'pointer', fontSize: 14, fontFamily: 'var(--mono)' }}>Volver</button>}
                ...
                {isMobile && (
                  <div style={{ padding: '10px 16px', borderBottom: '1px solid var(--border)', display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
                    <button onClick={() => setPersonalDetalleId(null)} style={{ padding: '6px 12px', borderRadius: 6, background: 'var(--surface2)', border: '1px solid var(--border)', color: 'var(--text)', cursor: 'pointer', fontSize: 11, fontFamily: 'var(--mono)' }}>← Volver</button>
                    {misPermisos['personal.descargar_excel'] && (
                      <>
                        <select value={excelAnioPersonal} onChange={e => setExcelAnioPersonal(Number(e.target.value))}
                          style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '6px 8px', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 11, outline: 'none' }}>
                        ...
                        <button onClick={() => descargarExcelPersonal(personalDetalle, excelAnioPersonal)}
                          style={{ padding: '6px 12px', borderRadius: 6, background: 'var(--surface2)', border: '1px solid var(--border)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 11, cursor: 'pointer' }}>📊 Excel</button>
                      </>
                    )}
                    {misPermisos['personal.crear_editar'] && (
                      <button onClick={() => abrirFormPersonal(personalDetalle)} style={{ padding: '6px 12px', borderRadius: 6, background: 'var(--green)', border: 'none', color: 'var(--bg)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 11, cursor: 'pointer' }}>✏ Editar</button>
                    )}
                  </div>
                )}
                <div style={{ padding: 16 }}>
...
{/* - Pestaña EQUIPOS - */}
                    ...
                    <div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16, flexWrap: 'wrap', gap: 10 }}>
                        ...
                        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                          ...
                          <button onClick={() => tareaSel.tipo === 'Tratamiento fitosanitario' ? abrirEditarTareaTratamiento(tareaSel) : tareaSel.tipo === 'Abonado' ? setTareaAbonadoAAbrir(tareaSel) : abrirEditarTareaGeneral(tareaSel)} style={{ padding: '8px 14px', borderRadius: 6, background: 'var(--surface2)', border: '1px solid var(--border)', color: 'var(--text)', fontFamily: 'var(--mono)', fontSize: 11, cursor: 'pointer' }}>EDITAR</button>
        ...
        <AbonadoTab session={session} fincas={fincas} misParcelas={misParcelas.map(p => ({
            ...p,
            referenciaSigpac: referenciaSigpacDeParcela(p),
            recintos: (p.parcelaInfo?.recintos && p.parcelaInfo.recintos.length > 0)
              ? p.parcelaInfo.recintos.map((r: RecintoRef) => ({
                  referencia: formatRefSigpac(r), supHa: r.supHa, provincia: r.provincia, municipio: r.municipio,
                  agregado: r.agregado, zona: r.zona, poligono: r.poligono, parcela: r.parcela, recinto: r.recinto, usoSigpac: r.usoSigpac,
                }))
              : [{ referencia: referenciaSigpacDeParcela(p), supHa: p.supHa }],
          }))} campanas={campanas}
            historicoCultivos={historicoCultivos} equipos={equipos} personal={personal} isMobile={isMobile} misPermisos={misPermisos}
            datosExplotacionPorFinca={datosExplotacionPorFinca} onEditarDatosExplotacion={abrirFormDatosExplotacion}
            activo={pestana === 'abonado'} tareaAbonadoAAbrir={tareaAbonadoAAbrir} onConsumirTareaAbonado={() => setTareaAbonadoAAbrir(null)}
            onPlanGuardado={onAbonadoPlanGuardado} />
          ...
          return (
            <div key="cosechas" style={{ flex: 1, height: '100%', overflow: 'auto', padding: isMobile ? 12 : 24, background: 'var(--bg)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10, marginBottom: 16 }}>
                ...
                {misPermisos['cosechas.crear_editar_eliminar'] && (
                  <button onClick={() => abrirNuevaCosecha()} style={{ padding: '7px 14px', borderRadius: 6, background: 'rgba(31, 74, 40, 1)', border: '1px solid var(--green)', color: 'var(--bg)', fontFamily: 'var(--mono)', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>+ NUEVA COSECHA</button>
                )}
              </div>
                        ...
                        {expandida && (
                          <div style={{ padding: '0 14px 14px', borderTop: '1px solid var(--border)' }}>
                            ...
                            {formAqui && misPermisos['cosechas.comercializacion.crear_editar_eliminar'] ? (
                              ...
                              <button onClick={() => abrirNuevaComercializacionParte(parte.key)} style={{ padding: '6px 10px', borderRadius: 5, background: 'transparent', border: '1px solid var(--blue)', color: 'var(--bg)', fontFamily: 'var(--mono)', fontSize: 10, fontWeight: 700, cursor: 'pointer' }}>+ Comercialización</button>
                            ) : null}
                            </>)}
                          </div>
                        )}
            ...
            <div style={{ background: 'var(--surface)', border: '1px solid var(--green)', borderRadius: 12, width: '100%', maxWidth: 420, padding: 24, maxHeight: '90vh', overflowY: 'auto' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                <div style={{ display: 'flex', gap: 10, marginTop: 4 }}>
                  <button onClick={guardarCosecha} style={{ flex: 1, padding: 11, borderRadius: 8, background: 'var(--green)', border: 'none', color: 'var(--bg)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>GUARDAR</button>
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--green)', borderRadius: 12, width: '100%', maxWidth: 460, maxHeight: '90vh', overflow: 'auto' }}>
            ...
            <div style={{ padding: '14px 20px', borderTop: '1px solid var(--border)', display: 'flex', gap: 10 }}>
              <button onClick={generarCuaderno} disabled={generandoCuaderno || !cuadernoCampanaId || (fincas.length > 0 && cuadernoFincaIds.length === 0)}
                style={{ flex: 1, padding: 11, borderRadius: 8, background: 'var(--green)', border: 'none', color: 'var(--bg)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 13, cursor: (generandoCuaderno || !cuadernoCampanaId || (fincas.length > 0 && cuadernoFincaIds.length === 0)) ? 'default' : 'pointer', opacity: (generandoCuaderno || !cuadernoCampanaId || (fincas.length > 0 && cuadernoFincaIds.length === 0)) ? 0.6 : 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--green)', borderRadius: 12, width: '100%', maxWidth: 640, maxHeight: '90vh', overflow: 'auto' }}>
            ...
            <div style={{ padding: '14px 20px', borderTop: '1px solid var(--border)', display: 'flex', gap: 10 }}>
              <button onClick={guardarDatosExplotacion} disabled={deGuardando || deFincaIds.length === 0}
                style={{ flex: 1, padding: 11, borderRadius: 8, background: 'var(--green)', border: 'none', color: 'var(--bg)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 13, cursor: (deGuardando || deFincaIds.length === 0) ? 'default' : 'pointer', opacity: (deGuardando || deFincaIds.length === 0) ? 0.6 : 1 }}>
