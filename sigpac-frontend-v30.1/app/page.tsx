...
export default function Home() {
  ...
  const [cuadernoAnio, setCuadernoAnio] = useState(new Date().getFullYear())
  ...
  const generarCuaderno = async () => {
    ...
    try {
      ...
      // Parcelas de las fincas seleccionadas (todas, si no se ha restringido ninguna finca)
      const parcelasSel = misParcelas.filter(p => !idsFincaSel || (p.fincaId && idsFincaSel.has(p.fincaId)))
      ...
      parcelasSel.forEach((p, i) => {
        ...
        recintos.forEach(r => {
          ...
          filasParcelas.push([
            ...
            supSigpac > 0 ? supSigpac.toFixed(4) : '',
            supCultivada > 0 ? supCultivada.toFixed(4) : p.supHa.toFixed(4),
            p.cultivo || '', p.variedad || '',
      ...
      // --- Tratamientos del 1 de enero al 31 de diciembre del año elegido, en las fincas elegidas ---
      const tratamientosSel = tratamientos.filter(t => {
        if (!t.fecha || t.fecha.slice(0, 4) !== String(anio)) return false
      ...
      const cultivoDeParcelas = (ids: string[]): { especie: string; variedad: string } => {
        const cultivos = Array.from(new Set(ids.map(id => misParcelas.find(p => p.id === id)?.cultivo).filter(Boolean))) as string[]
        const variedades = Array.from(new Set(ids.map(id => misParcelas.find(p => p.id === id)?.variedad).filter(Boolean))) as string[]
      ...
      const filas32 = tratamientosSemilla.map(t => {
        ...
        return [
          ...
          sup > 0 ? sup.toFixed(2) : '',
          kgSemillaHa > 0 && sup > 0 ? (kgSemillaHa * sup).toFixed(2) : '',
      ...
      await descargarCuadernoCampo({ anio, cabecera: nombreCabecera, datosGenerales, titular, secciones }, `cuaderno_explotacion_${anio}.docx`)
    } catch (e: any) {
          ...
          <div style={{ flex: 1, height: '100%', overflow: 'auto', padding: isMobile ? 12 : 24, background: 'var(--bg)' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, flexWrap: 'wrap', gap: 10 }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <button onClick={() => { setCuadernoFincaIds([]); setCuadernoAnio(new Date().getFullYear()); setCuadernoModalVisible(true) }}
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--green)', borderRadius: 12, width: '100%', maxWidth: 460, maxHeight: '90vh', overflow: 'auto' }}>
            ...
            <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div style={{ fontSize: 11, color: 'var(--muted)', fontFamily: 'var(--mono)', lineHeight: 1.6 }}>
                Genera el Cuaderno de Explotación en Word (1 de enero a 31 de diciembre del año elegido), con los tratamientos y parcelas de las fincas seleccionadas.
              </div>
              <div>
                <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase' }}>Año</label>
                <select value={cuadernoAnio} onChange={e => setCuadernoAnio(Number(e.target.value))}
                  style={{ width: '100%', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '9px 12px', color: 'var(--text)', fontSize: 13, fontFamily: 'var(--sans)', outline: 'none' }}>
                  {Array.from(new Set([new Date().getFullYear() + 1, new Date().getFullYear(), new Date().getFullYear() - 1, new Date().getFullYear() - 2, cuadernoAnio])).sort((a, b) => b - a).map(a => <option key={a} value={a}>{a}</option>)}
                </select>
              </div>
              <div>
                <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase' }}>
            ...
            <div style={{ padding: '14px 20px', borderTop: '1px solid var(--border)', display: 'flex', gap: 10 }}>
              <button onClick={generarCuaderno} disabled={generandoCuaderno}
                style={{ flex: 1, padding: 11, borderRadius: 8, background: 'var(--green)', border: 'none', color: 'var(--bg)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 13, cursor: generandoCuaderno ? 'default' : 'pointer', opacity: generandoCuaderno ? 0.6 : 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
