...
export default function Home() {
  ...
  const [buscarCosechasTab, setBuscarCosechasTab] = useState('')
        ...
        {pestana === 'cosechas' && (
          <div style={{ flex: 1, overflow: 'auto', padding: isMobile ? 12 : 24 }}>
            ...
            <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
              ...
              <input value={buscarCosechasTab} onChange={e => setBuscarCosechasTab(e.target.value)} placeholder="Buscar por parcela o cultivo..."
                style={{ flex: 1, minWidth: 200, background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '8px 12px', color: 'var(--text)', fontSize: 12, fontFamily: 'var(--mono)', outline: 'none' }} />
            </div>
            ...
            {(() => {
              const cosechasTab = cosechas
                ...
                .filter(c => {
                  if (!buscarCosechasTab.trim()) return true
                  const q = buscarCosechasTab.trim().toLowerCase()
                  const p = misParcelas.find(x => x.id === c.parcelaId)
                  return c.cultivo.toLowerCase().includes(q) || (p?.nombre || '').toLowerCase().includes(q)
                })
                .sort((a, b) => (b.fechaCosecha || '').localeCompare(a.fechaCosecha || ''))

              if (cosechas.length === 0) {
                return <div style={{ fontSize: 12, color: 'var(--muted)', fontFamily: 'var(--mono)', padding: '20px 0' }}>Todavía no hay cosechas registradas. Regístralas aquí o desde el histórico de cada parcela, en Mis Parcelas.</div>
              }
              if (cosechasTab.length === 0) {
                return <div style={{ fontSize: 12, color: 'var(--muted)', fontFamily: 'var(--mono)', padding: '20px 0' }}>Ninguna cosecha coincide con el filtro.</div>
              }
                      ...
                      <div key={c.id} style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 8, overflow: 'hidden' }}>
                        ...
                        {expandida && (
                          <div style={{ padding: '0 14px 14px', borderTop: '1px solid var(--border)' }}>
                            ...
                            <div style={{ display: 'flex', gap: 6, marginBottom: 10, flexWrap: 'wrap' }}>
                              {p && (
                                <button onClick={() => { setParcelaDetalleId(p.id); setPestana('mis_parcelas') }} style={{ padding: '5px 10px', borderRadius: 5, background: 'transparent', border: '1px solid var(--blue)', color: 'var(--blue)', fontFamily: 'var(--mono)', fontSize: 10, cursor: 'pointer' }}>Ver en Mis Parcelas →</button>
                              )}
                              {misPermisos['parcelas.crear_editar'] && (
                                <button onClick={() => abrirEditarCosecha(c)} style={{ padding: '5px 10px', borderRadius: 5, background: 'transparent', border: '1px solid var(--border)', color: 'var(--muted)', fontFamily: 'var(--mono)', fontSize: 10, cursor: 'pointer' }}>Editar</button>
                              )}
