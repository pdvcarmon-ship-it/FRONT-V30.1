...
const campoTexto = (props: any, ...keys: string[]): string | undefined => {
  for (const k of keys) {
...
const extraerRecinto = (props: any): RecintoRef => {
...
const formatRefSigpac = (r: RecintoRef): string =>
...
const fusionarPoligonosDeFeatures = (features: any[]): any => {
  const poligonos: any[] = []
...
const construirEstadoDesdeFeatures = (features: any[]) => {
      ...
      <div style={{ background: 'var(--surface)', borderBottom: '1px solid var(--border)', flexShrink: 0, position: 'relative' }}>
        <div style={{ display: 'flex', alignItems: 'center', minHeight: isMobile ? 54 : 48 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, borderLeft: '1px solid var(--border)', paddingLeft: 12 }}>
            ...
            <button onClick={() => { setShowVerifRomaRopo(true); setVerifResultado(null); setVerifError('') }} style={{ padding: '4px 10px', borderRadius: 5, background: 'transparent', border: '1px solid rgba(61,220,110,0.3)', color: 'var(--green)', fontFamily: 'var(--mono)', fontSize: 9, cursor: 'pointer' }}>
              COMPROBAR ROMA/ROPO
            </button>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 3000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, width: '100%', maxWidth: 460, maxHeight: '90vh', overflow: 'auto', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div>
                <label style={{ display: 'block', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Registro</label>
                <div style={{ display: 'flex', gap: 8 }}>
                  {(['ropo', 'roma'] as const).map(r => (
                    <button key={r} onClick={() => setVerifRegistro(r)} style={{ flex: 1, padding: '9px', borderRadius: 6, background: verifRegistro === r ? 'var(--green-dim)' : 'var(--surface2)', border: `1px solid ${verifRegistro === r ? 'rgba(61,220,110,0.3)' : 'var(--border)'}`, color: verifRegistro === r ? 'var(--green)' : 'var(--muted)', fontFamily: 'var(--mono)', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
                      {r.toUpperCase()}
                    </button>
                  ))}
                </div>
              ...
              {verifResultado && (
                <div style={{ padding: '10px 12px', borderRadius: 6, background: 'var(--surface2)', border: '1px solid var(--border)', fontSize: 12, fontFamily: 'var(--mono)', color: 'var(--text)' }}>
                  {verifResultado.encontrados === 0 ? (
                    ...
                    <div>
                      <div style={{ marginBottom: 8, color: 'var(--green)' }}>✅ {verifResultado.encontrados} inscripción(es) encontrada(s)</div>
