...
type Pestaña = 'mapa' | 'mis_parcelas' | 'tratamientos' | 'personal' | 'equipos' | 'riego'
...
export default function Home() {
      ...
      try {
        ...
        if (histCultivosRes.data) {
          setHistoricoCultivos(histCultivosRes.data.map((h: any) => ({
            id: h.id, parcelaId: h.parcela_id, cultivo: h.cultivo,
  ...
  const generarPdfTratamiento = async (t: Tratamiento) => {
    ...
    try {
      ...
      const maquinariaStr = (t.equipoIds || []).map(id => equipos.find(e => e.id === id)?.nombre).filter(Boolean).join(', ') || '—'
      ...
      body.push(
        ...
        [{ content: 'Recomendado por', styles: { fontStyle: 'bold' } }, { content: t.recomendadoPor || '—', colSpan: 3 }],
        [{ content: 'Aplicado por', styles: { fontStyle: 'bold' } }, { content: t.aplicadoPor || t.aplicador || '—', colSpan: 3 }],
  ...
  const confirmarGuardarCultivoAnterior = async (campanaId: string) => {
    ...
    try {
      await supabase.from('historico_cultivos_parcela').upsert({
        ...
        cultivo: nuevo.cultivo, campana_id: nuevo.campanaId || null, fecha_registro: nuevo.fechaRegistro,
      })
  ...
  const guardarEdicionHistorico = async () => {
    ...
    const actualizado = { cultivo: historicoEditCultivo.trim(), campanaId: historicoEditCampanaId || undefined }
    ...
    try {
      await supabase.from('historico_cultivos_parcela').update({
        cultivo: actualizado.cultivo, campana_id: actualizado.campanaId || null,
  ...
  interface FilaCambioCampana { parcelaId: string; nombre: string; cultivoActual: string; cultivoNuevo: string; campanaActualId: string; campanaNuevaId: string }
  ...
  const actualizarFilaCambioCampana = (parcelaId: string, campo: 'cultivoNuevo' | 'campanaNuevaId', valor: string) => {
    setFilasCambioCampana(filasCambioCampana.map(f => f.parcelaId === parcelaId ? { ...f, [campo]: valor } : f))
  }

  const guardarCambiosCampana = async () => {
    if (!session) return
    const filasConCambios = filasCambioCampana.filter(f => f.cultivoNuevo.trim() !== f.cultivoActual || f.campanaNuevaId)
    ...
    try {
      ...
      for (const fila of filasConCambios) {
        const cambioCultivo = fila.cultivoNuevo.trim() !== fila.cultivoActual
        // Si cambia el cultivo, se guarda primero el anterior en el histórico (junto a la
        // campaña que tenía hasta ahora, aunque no se haya elegido campaña nueva).
        if (cambioCultivo) {
          const hist: HistoricoCultivo = {
            id: String(Date.now()) + '_' + fila.parcelaId,
            parcelaId: fila.parcelaId, cultivo: fila.cultivoActual,
          ...
          await supabase.from('historico_cultivos_parcela').upsert({
            ...
            cultivo: hist.cultivo, campana_id: hist.campanaId || null, fecha_registro: hist.fechaRegistro,
          })
        }
        ...
        await supabase.from('parcelas').update({
          cultivo: actualizada.cultivo, campana_id: actualizada.campanaId || null,
        ...
        <div style={{ display: 'flex', alignItems: 'center', minHeight: isMobile ? 54 : 48 }}>
            ...
            <div data-menu style={{ position: 'relative', flexShrink: 0 }}>
              ...
              {menuAbierto && (
                <div style={{ position: 'absolute', top: '100%', left: 0, zIndex: 6000, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '0 0 10px 10px', boxShadow: '0 8px 24px rgba(0,0,0,0.5)', minWidth: 220 }}>
                  {(['mapa', 'mis_parcelas', 'tratamientos', 'riego', 'personal', 'equipos'] as Pestaña[]).map(tab => (
                    ...
                    >
                      {tab === 'mapa' ? '🗺  MAPA' : tab === 'mis_parcelas' ? '📁  MIS PARCELAS' : tab === 'tratamientos' ? '🧪  TRATAMIENTOS' : tab === 'riego' ? '💧  RIEGO' : tab === 'personal' ? '👷  PERSONAL' : '🚜  EQUIPOS'}
                    </button>
                  ))}
                </div>
              )}
          ...
          {isMobile ? (
            <span style={{ flex: 1, fontFamily: 'var(--mono)', fontSize: 13, fontWeight: 700, color: 'var(--green)', paddingLeft: 4 }}>
              {pestana === 'mapa' ? '🗺 MAPA' : pestana === 'mis_parcelas' ? '📁 MIS PARCELAS' : pestana === 'tratamientos' ? '🧪 TRATAMIENTOS' : pestana === 'riego' ? '💧 RIEGO' : pestana === 'personal' ? '👷 PERSONAL' : '🚜 EQUIPOS'}
            </span>
          ) : (
        ...
        {pestana === 'mis_parcelas' && (
          <div style={{ display: 'flex', width: '100%', height: '100%', overflow: 'hidden' }}>
            ...
            {formularioVisible && (
              <div style={{ width: isMobile ? '100%' : 480, height: '100%', background: 'var(--surface)', borderRight: isMobile ? 'none' : '1px solid var(--border)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                ...
                <div style={{ flex: 1, overflow: 'auto', padding: 20, display: 'flex', flexDirection: 'column', gap: 16 }}>
                  ...
                  {/* Guardar cultivo anterior con campaña: solo al editar, y solo si el cultivo ha cambiado */}
                  {parcelaEditar && formCultivo.trim() !== parcelaEditar.cultivo && (
                      ...
                      {!cultivoAnteriorGuardado ? (
                        <>
                          <div style={{ fontSize: 11, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 8, lineHeight: 1.5 }}>
                            Vas a cambiar el cultivo de <strong style={{ color: 'var(--text)' }}>{parcelaEditar.cultivo}</strong> a <strong style={{ color: 'var(--text)' }}>{formCultivo.trim() || '—'}</strong>. Si es por rotación de cultivo (fin de campaña), guarda antes el cultivo anterior para no perder el historial.
                          </div>
                        ...
                        <div style={{ fontSize: 11, fontFamily: 'var(--mono)', color: 'var(--green)' }}>
                          ✓ Guardado: {parcelaEditar.cultivo}{parcelaEditar.campanaId ? ` — ${campanas.find(c => c.id === parcelaEditar.campanaId)?.nombre || ''}` : ' (sin campaña)'}
                        </div>
                      )}
                    </div>
                  )}
            ...
            <div style={{ flex: 1, height: '100%', overflow: 'auto', padding: 24, background: 'var(--bg)' }}>
              ...
              {/* Popup: elegir campaña del cultivo anterior, cuando no tenía ninguna asociada */}
              ...
              {/* Popup: cambio masivo de cultivo/campaña de todas las parcelas */}
              {popupCambioCampanas && (
                      ...
                      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11, fontFamily: 'var(--mono)' }}>
                        <thead>
                          <tr style={{ position: 'sticky', top: 0, background: 'var(--surface)' }}>
                            {['Parcela', 'Cultivo', 'Campaña actual', 'Campaña nueva'].map(h => (
              ...
              {parcelaDetalle && (
                <div ref={parcelaDetalleRef} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, marginBottom: 20, overflow: 'hidden' }}>
                  ...
                  <div style={{ padding: 16 }}>
                    ...
                    {/* Histórico de cultivos: rotaciones anteriores guardadas al editar la parcela */}
                      ...
                      <div style={{ marginBottom: 16 }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(61,220,110,0.08)', border: '1px solid var(--green)', borderRadius: 6, padding: '8px 10px', fontSize: 11, fontFamily: 'var(--mono)' }}>
                            <span style={{ color: 'var(--green)' }}>● Actual: {parcelaDetalle.cultivo}</span>
                            ...
                            .map(h => (
                              historicoEditarId === h.id ? (
                                <div key={h.id} style={{ background: 'var(--surface2)', border: '1px solid #ffb454', borderRadius: 6, padding: '8px 10px' }}>
                                  <input value={historicoEditCultivo} onChange={e => setHistoricoEditCultivo(e.target.value)}
                                ...
                                <div key={h.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '8px 10px', fontSize: 11, fontFamily: 'var(--mono)' }}>
                                  <span style={{ color: 'var(--text)' }}>{h.cultivo}</span>
