...
export default function Home() {
  ...
  const [mostrarMdtColor, setMostrarMdtColor] = useState(false)
  ...
  const resetear = () => {
    ...
    setMdtColorUrl(null); setMdtColorStats(null); setMostrarMdtColor(false); setCalculandoMdtColor(false)
  }
  ...
  const guardarTratamiento = async () => {
    ...
    const nuevo: Tratamiento = {
      ...
      ...datosComunes,
      fechaRegistro: tratEditar?.fechaRegistro || (tareaOrigen?.tratamientoId ? tratamientos.find(t => t.id === tareaOrigen.tratamientoId)?.fechaRegistro : undefined) || new Date().toLocaleDateString('es-ES'),
        ...
        {pestana === 'mapa' && (
          <div style={{ display: 'flex', width: '100%', height: '100%', position: 'relative' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: isMobile ? '0 16px 16px' : '0', flexDirection: 'column', gap: 14, display: panelAbierto || !isMobile ? 'flex' : 'none' }}>
              ...
              {/* Mapa de colores de altitud (MDT hipsométrico) */}
              {parcGeojson && (
                <>
                  <hr style={{ borderColor: 'var(--border)', borderWidth: '0 0 1px 0' }} />
                  <section>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 10 }}>
                      ...
                      <span style={{ fontSize: 11, fontFamily: 'var(--mono)', color: 'var(--blue)', letterSpacing: '0.08em', fontWeight: 700 }}>MAPA DE COLORES DE ALTITUD</span>
                    </div>
                    <div style={{ fontSize: 10, color: 'var(--muted)', fontFamily: 'var(--mono)', marginBottom: 10, lineHeight: 1.6 }}>MDT del IGN . Rojo = cota más alta, azul claro = cota más baja</div>
                    <button onClick={calcularMdtColor} disabled={calculandoMdtColor} style={{ width: '100%', padding: '10px', borderRadius: 8, background: calculandoMdtColor ? 'var(--surface2)' : 'rgba(77,184,255,0.15)', border: '1px solid var(--blue)', color: 'var(--blue)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 12, cursor: calculandoMdtColor ? 'wait' : 'pointer', letterSpacing: '0.06em', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, transition: 'all 0.2s' }}>
                      {calculandoMdtColor ? <><span className="spinner" style={{ borderTopColor: 'var(--blue)' }}/> CALCULANDO MDT...</> : 'GENERAR MAPA DE COLORES'}
                    </button>
                    {mdtColorStats && (
                      <div style={{ marginTop: 10 }}>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 5 }}>
                          {[{ k: 'Alt. mín', v: `${mdtColorStats.altitud_min}m` }, { k: 'Alt. máx', v: `${mdtColorStats.altitud_max}m` }, { k: 'Alt. media', v: `${mdtColorStats.altitud_media}m` }, { k: 'Área', v: `${mdtColorStats.area_ha}ha` }].map(s => (
                            <div key={s.k} style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '7px 10px' }}>
                              <div style={{ fontSize: 8, fontFamily: 'var(--mono)', color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>{s.k}</div>
                              <div style={{ fontSize: 13, fontFamily: 'var(--mono)', fontWeight: 700, color: 'var(--blue)', marginTop: 2 }}>{s.v}</div>
                            </div>
                        ...
                        {Array.isArray(mdtColorStats.leyenda) && mdtColorStats.leyenda.length > 0 && (
                          <div style={{ marginTop: 8, background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, padding: '8px 10px' }}>
                            <div style={{ fontSize: 8, fontFamily: 'var(--mono)', color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 6 }}>Escala de altitud</div>
                            <div style={{ width: '100%', height: 10, borderRadius: 5, background: `linear-gradient(90deg, ${mdtColorStats.leyenda.map((l: { color: string }) => l.color).join(', ')})` }} />
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 }}>
                              <span style={{ fontSize: 9, fontFamily: 'var(--mono)', color: 'var(--text)' }}>{mdtColorStats.leyenda[0].cota}m</span>
                              <span style={{ fontSize: 9, fontFamily: 'var(--mono)', color: 'var(--text)' }}>{mdtColorStats.leyenda[mdtColorStats.leyenda.length - 1].cota}m</span>
                            </div>
                          </div>
                        )}
                        <button onClick={() => { setMostrarMdtColor(f => !f); if (!mostrarMdtColor && mdtColorUrl) { setImagenUrl(mdtColorUrl); setModoVista('zonas') } else { setImagenUrl(null); setModoVista('ninguna') } }} style={{ width: '100%', marginTop: 8, padding: '8px', borderRadius: 6, background: mostrarMdtColor ? 'rgba(77,184,255,0.15)' : 'var(--surface2)', border: `1px solid ${mostrarMdtColor ? 'var(--blue)' : 'var(--border)'}`, color: mostrarMdtColor ? 'var(--blue)' : 'var(--muted)', fontFamily: 'var(--mono)', fontSize: 10, cursor: 'pointer', fontWeight: 600 }}>
                          {mostrarMdtColor ? 'OCULTAR MAPA DE COLORES' : 'VER MAPA DE COLORES'}
                        </button>
                      </div>
                    )}
                  </section>
