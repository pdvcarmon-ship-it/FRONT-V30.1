...
// Punto de captación de agua (pozo, canal, balsa...) ya registrado en Mis Parcelas > Ubicaciones.
interface UbicacionLite { id: string; fincaId: string; nombre: string; lat: number; lng: number }
...
interface SistemaRiego {
  ...
  nombre: string
  fincaId?: string
  ubicacionId?: string // ubicación (pozo, canal, balsa...) de la que toma el agua este sistema
  origenAgua: string
...
const COLORES_PIEZAS = ['#1F4A28', '#0F4A70', '#ffb454', '#c084fc', '#6E2417', '#5eead4', '#facc15']
...
export default function RiegoTab({ session, fincas, misParcelas, ubicaciones, isMobile, sectorInicial, onConsumirSectorInicial, misPermisos }: Props) {
  ...
  const [sUbicacionId, setSUbicacionId] = useState('')
      ...
      try {
        ...
        if (sisRes.data) {
          setSistemas(sisRes.data.map((s: any) => ({
            ...
            ubicacionId: s.ubicacion_id || undefined,
  ...
  const abrirFormSistema = (s?: SistemaRiego) => {
    ...
    if (s) {
      ...
      setSistemaEditar(null); setSNombre(''); setSFincaId(''); setSUbicacionId(''); setSOrigenAgua('')
      setSOrigenEnergia(''); setSBomba(''); setSFiltros(''); setSObs(''); setSParcelaIds([])
    }
    setSError(''); setFormSistema(true)
  }

  // Si cambia la finca del sistema, la ubicación elegida deja de tener sentido si era de otra finca
  const cambiarFincaSistema = (id: string) => {
    setSFincaId(id)
    if (sUbicacionId && !ubicaciones.some(u => u.id === sUbicacionId && u.fincaId === id)) {
      setSUbicacionId('')
    }
  }
  ...
  const guardarSistema = async () => {
    ...
    const nuevo: SistemaRiego = {
      ...
      ubicacionId: sUbicacionId || undefined,
    ...
    try {
      await supabase.from('sistemas_riego').upsert({
        ...
        ubicacion_id: nuevo.ubicacionId || null,
      ...
      {opciones.map(p => {
        ...
        return (
          <button key={p.id} onClick={() => onToggle(p.id)}
            style={{ padding: '5px 10px', borderRadius: 14, background: sel ? 'rgba(15, 74, 112,0.15)' : 'var(--surface)', border: `1px solid ${sel ? 'var(--blue)' : 'var(--border)'}`, color: sel ? 'var(--blue)' : 'var(--muted)', fontFamily: 'var(--mono)', fontSize: 10, cursor: 'pointer' }}>
            {nombreParcela(p)}
          </button>
        )
      })}
    ...
    <div style={{ width: '100%', height: '100%', overflow: 'auto', padding: isMobile ? 12 : 24, background: 'var(--bg)' }}>
      ...
      {fincas.length > 0 && (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 16 }}>
          <button onClick={() => setFiltroFincaId('')} style={{ padding: '5px 12px', borderRadius: 14, background: !filtroFincaId ? 'rgba(31, 74, 40,0.12)' : 'var(--surface)', border: `1px solid ${!filtroFincaId ? 'var(--green)' : 'var(--border)'}`, color: !filtroFincaId ? 'var(--green)' : 'var(--muted)', fontFamily: 'var(--mono)', fontSize: 11, cursor: 'pointer', fontWeight: !filtroFincaId ? 700 : 400 }}>Todas las fincas</button>
          {fincas.map(f => (
            <button key={f.id} onClick={() => setFiltroFincaId(f.id)} style={{ padding: '5px 12px', borderRadius: 14, background: filtroFincaId === f.id ? 'rgba(31, 74, 40,0.12)' : 'var(--surface)', border: `1px solid ${filtroFincaId === f.id ? 'var(--green)' : 'var(--border)'}`, color: filtroFincaId === f.id ? 'var(--green)' : 'var(--muted)', fontFamily: 'var(--mono)', fontSize: 11, cursor: 'pointer', fontWeight: filtroFincaId === f.id ? 700 : 400 }}>{f.nombre}</button>
          ))}
        </div>
      )}
      ...
      {sistemasVista.map(sistema => {
        ...
        const ubicacionSistema = ubicaciones.find(u => u.id === sistema.ubicacionId)
            ...
            <div style={{ padding: '12px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--surface2)', cursor: 'pointer' }}
              ...
              <div style={{ display: 'flex', gap: 8 }} onClick={e => e.stopPropagation()}>
                ...
                {misPermisos['riego.eliminar'] && (
                  <button onClick={() => eliminarSistema(sistema.id)} style={{ padding: '6px 12px', borderRadius: 6, background: 'transparent', border: '1px solid rgba(110, 36, 23,0.3)', color: '#6E2417', fontFamily: 'var(--mono)', fontSize: 11, cursor: 'pointer' }}>Eliminar</button>
                )}
              </div>
            </div>
            ...
            {abierto && (
              <div style={{ padding: 16 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px,1fr))', gap: 8, marginBottom: 16 }}>
                  {[
                    { k: 'Ubicación', v: ubicaciones.find(u => u.id === sistema.ubicacionId)?.nombre || 'Sin asignar' },
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    {sectoresDelSistema.length > 0 && (
                      <button onClick={() => setSistemaVerSectoresId(sistemaVerSectoresId === sistema.id ? null : sistema.id)}
                        style={{ padding: '6px 12px', borderRadius: 6, background: sistemaVerSectoresId === sistema.id ? 'rgba(31, 74, 40,0.12)' : 'var(--surface)', border: `1px solid ${sistemaVerSectoresId === sistema.id ? 'var(--green)' : 'var(--border)'}`, color: sistemaVerSectoresId === sistema.id ? 'var(--green)' : 'var(--muted)', fontFamily: 'var(--mono)', fontSize: 11, cursor: 'pointer' }}>
                        👁 {sistemaVerSectoresId === sistema.id ? 'Ocultar mapa' : 'Ver sectores'}
                      </button>
                    )}
                    {misPermisos['riego.crear_editar'] && (
                      <button onClick={() => abrirFormSector(sistema.id)} style={{ padding: '6px 12px', borderRadius: 6, background: 'rgba(15, 74, 112,0.1)', border: '1px solid var(--blue)', color: 'var(--blue)', fontFamily: 'var(--mono)', fontSize: 11, cursor: 'pointer' }}>+ Añadir sector</button>
                    )}
                  </div>
                </div>
                ...
                {sistemaVerSectoresId === sistema.id && sectoresDelSistema.length > 0 && (
                  <div style={{ height: 360, borderRadius: 8, overflow: 'hidden', border: '1px solid var(--green)', marginBottom: 12 }}>
                    <MapView
                      ...
                      puntosMarcador={ubicacionSistema ? [{ id: ubicacionSistema.id, nombre: ubicacionSistema.nombre, lat: ubicacionSistema.lat, lng: ubicacionSistema.lng }] : undefined}
                    />
                  </div>
                )}
                    ...
                    <div key={sector.id} style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 8, padding: '10px 12px', marginBottom: 10 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 6, flexWrap: 'wrap', gap: 8 }}>
                        ...
                        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                          ...
                          <button onClick={() => abrirCalculoRiego(sector.id)} style={{ padding: '5px 10px', borderRadius: 6, background: 'rgba(31, 74, 40,0.1)', border: '1px solid var(--green)', color: 'var(--green)', fontFamily: 'var(--mono)', fontSize: 10, fontWeight: 700, cursor: 'pointer' }}>📊 Calcular necesidades</button>
                          */}
                          {misPermisos['riego.crear_editar'] && (
                          ...
                          {misPermisos['riego.eliminar'] && (
                            <button onClick={() => eliminarSector(sector.id)} style={{ padding: '5px 10px', borderRadius: 6, background: 'transparent', border: '1px solid rgba(110, 36, 23,0.3)', color: '#6E2417', fontFamily: 'var(--mono)', fontSize: 10, cursor: 'pointer' }}>Eliminar</button>
                          )}
                        </div>
                      </div>
                      ...
                      {calculandoRiegoSectorId === sector.id && (
                        <div style={{ marginTop: 10, padding: 12, background: 'var(--surface)', border: '1px solid var(--green)', borderRadius: 8 }}>
                              ...
                              <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 6 }}>
                                {predKcPeriodos.map(p => (
                                  <div key={p.id} style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : '1fr 1fr 70px 24px', gap: 6, alignItems: 'center' }}>
                                    ...
                                    <button onClick={() => eliminarPeriodoKc(p.id)} style={{ background: 'transparent', border: 'none', color: '#6E2417', cursor: 'pointer' }}>✕</button>
                                  </div>
                                ))}
                              </div>
                          ...
                          {predError && <div style={{ color: '#6E2417', fontSize: 11, fontFamily: 'var(--mono)', marginBottom: 8 }}>{predError}</div>}
                            ...
                            <div style={{ marginTop: 12 }}>
                              ...
                              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(120px,1fr))', gap: 8, marginBottom: 10 }}>
                                {[
                                  ...
                                  <div key={s.k} style={{ background: s.destacado ? 'rgba(31, 74, 40,0.08)' : 'var(--surface2)', border: `1px solid ${s.destacado ? 'var(--green)' : 'var(--border)'}`, borderRadius: 6, padding: '8px 10px' }}>
                                    <div style={labelStyle}>{s.k}</div>
                                  ...
                                  <tbody>
                                    {predResultado.tabla.map(fila => (
                                      <tr key={fila.fecha} style={{ borderBottom: '1px solid var(--border)' }}>
                                        ...
                                        <td style={{ padding: '5px 8px', color: '#0F4A70' }}>{fila.precipitacion !== null ? fila.precipitacion.toFixed(1) : '—'}</td>
                        ...
                        <div style={{ borderTop: '1px solid var(--border)', paddingTop: 8, marginTop: 4 }}>
                          ...
                          {eventosDelSector.map(ev => (
                            <div key={ev.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--text)', padding: '4px 0' }}>
                              ...
                              {misPermisos['riego.eliminar'] && (
                                <button onClick={() => eliminarRiego(ev.id)} style={{ background: 'transparent', border: 'none', color: '#6E2417', cursor: 'pointer', fontSize: 10 }}>✕</button>
                              )}
                            </div>
                          ))}
                        </div>
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--blue)', borderRadius: 10, padding: 16 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: 8, marginBottom: 8 }}>
            <div>
              ...
              <select value={sFincaId} onChange={e => cambiarFincaSistema(e.target.value)} style={inputStyle}>
                <option value="">Sin finca</option>
                {fincas.map(f => <option key={f.id} value={f.id}>{f.nombre}</option>)}
              </select>
            </div>
            <div>
              <label style={labelStyle}>Ubicación (punto de captación)</label>
              <select value={sUbicacionId} onChange={e => setSUbicacionId(e.target.value)} style={inputStyle}>
                <option value="">Sin asignar</option>
                {ubicaciones.filter(u => !sFincaId || u.fincaId === sFincaId).map(u => (
                  <option key={u.id} value={u.id}>{u.nombre}</option>
                ))}
              </select>
              {ubicaciones.filter(u => !sFincaId || u.fincaId === sFincaId).length === 0 && (
                <div style={{ fontSize: 10, color: 'var(--muted)', fontFamily: 'var(--mono)', marginTop: 4 }}>
                  {sFincaId ? 'Esta finca no tiene ubicaciones (pozos, canales...). Créalas en Mis Parcelas.' : 'No hay ubicaciones registradas. Créalas en Mis Parcelas.'}
                </div>
              )}
            </div>
          ...
          {sError && <div style={{ color: '#6E2417', fontSize: 11, fontFamily: 'var(--mono)', marginBottom: 8 }}>{sError}</div>}
      ...
      <div style={{ marginTop: 12, padding: 14, background: 'var(--surface2)', border: '1px solid var(--blue)', borderRadius: 8 }}>
          ...
          <div style={{ marginBottom: 8 }}>
            ...
            <div style={{ display: 'flex', gap: 6, marginBottom: 8 }}>
              {(['completa', 'dibujado'] as ModoSuperficie[]).map(m => (
                <button key={m} onClick={() => setSecModo(m)} style={{ flex: 1, padding: '6px 8px', borderRadius: 6, background: secModo === m ? 'rgba(15, 74, 112,0.15)' : 'var(--surface)', border: `1px solid ${secModo === m ? 'var(--blue)' : 'var(--border)'}`, color: secModo === m ? 'var(--blue)' : 'var(--muted)', fontFamily: 'var(--mono)', fontSize: 10, cursor: 'pointer', fontWeight: secModo === m ? 700 : 400 }}>
            ...
            {secModo === 'dibujado' && (
              <div style={{ marginTop: 8 }}>
                {secGeojsonDibujado ? (
                  <div>
                    <div style={{ padding: '8px 10px', borderRadius: 6, background: 'rgba(31, 74, 40,0.1)', border: '1px solid var(--green)', color: 'var(--green)', fontFamily: 'var(--mono)', fontSize: 11, marginBottom: 6 }}>
                  ...
                  <div style={{ fontSize: 10, fontFamily: 'var(--mono)', color: '#6E2417', padding: '8px 0' }}>
                      ...
                      {piezasCorte.map((p, i) => {
                        ...
                        return (
                          <button key={p.id} onClick={() => togglePiezaSeleccionada(p.id)}
                            style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 10px', borderRadius: 6, background: sel ? 'rgba(31, 74, 40,0.1)' : 'var(--surface)', border: `1px solid ${sel ? 'var(--green)' : COLORES_PIEZAS[i % COLORES_PIEZAS.length]}`, cursor: 'pointer' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    <button onClick={() => seleccionarPieza({ geojson: restanteGeojson, supHa: restanteSupHa })} style={{ width: '100%', padding: '8px', borderRadius: 6, background: 'rgba(31, 74, 40,0.1)', border: '1px solid var(--green)', color: 'var(--green)', fontFamily: 'var(--mono)', fontSize: 11, cursor: 'pointer' }}>
                      ✓ Usar toda la superficie restante ({restanteSupHa.toFixed(3)} ha)
                    </button>
                    <button onClick={() => { setCortando(true); setModoCorte(true) }} style={{ width: '100%', padding: '8px', borderRadius: 6, background: 'rgba(110, 36, 23,0.1)', border: '1px solid #6E2417', color: '#6E2417', fontFamily: 'var(--mono)', fontSize: 11, cursor: 'pointer' }}>
                  ...
                  <div>
                    ...
                    <div style={{ height: 340, borderRadius: 8, overflow: 'hidden', border: '1px solid var(--border)', position: 'relative' }}>
                      <div style={{ position: 'absolute', top: 8, left: 8, zIndex: 1000, background: 'rgba(225, 231, 211,0.9)', border: '1px solid #ffb454', borderRadius: 6, padding: '4px 8px', fontSize: 9, fontFamily: 'var(--mono)', color: '#ffb454' }}>
                ...
                {errorCorte && <div style={{ color: '#6E2417', fontSize: 10, fontFamily: 'var(--mono)', marginTop: 6 }}>{errorCorte}</div>}
              </div>
            )}
          </div>
          ...
          {secError && <div style={{ color: '#6E2417', fontSize: 11, fontFamily: 'var(--mono)', marginBottom: 8 }}>{secError}</div>}
