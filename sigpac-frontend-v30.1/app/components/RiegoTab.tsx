...
interface FilaCalculo { fecha: string; eto: number | null; kc: number; etc: number | null; necesidadBruta: number | null; necesidadM3Ha: number | null }

interface PrediccionRiego {
  fechaInicio: string; fechaFin: string; numDias: number
  eficienciaUsada: number; estacion: string
  etoAcumulada: number; necesidadBrutaAcumulada: number; necesidadM3Ha: number
...
export default function RiegoTab({ session, fincas, misParcelas, isMobile, sectorInicial, onConsumirSectorInicial }: Props) {
  ...
  const anadirPeriodoKc = () => {
    ...
    const siguienteDesde = ultima?.hasta ? new Date(new Date(ultima.hasta + 'T00:00:00').getTime() + 86400000).toISOString().slice(0, 10) : predFechaInicio
  ...
  const calcularNecesidadesRiego = async (sector: SectorRiego) => {
    ...
    try {
      ...
      let etoAcumulada = 0, necesidadBrutaAcumulada = 0, necesidadM3HaTotal = 0, diasSinDato = 0
      const tabla: FilaCalculo[] = dias.map(fecha => {
        ...
        if (eto === null) { diasSinDato++; return { fecha, eto: null, kc, etc: null, necesidadBruta: null, necesidadM3Ha: null } }
        const etc = eto * kc
        const necesidadBruta = etc / eficiencia
        ...
        return { fecha, eto, kc, etc, necesidadBruta, necesidadM3Ha }
      })
      ...
      setPredResultado({
        ...
        eficienciaUsada: eficiencia * 100, estacion: data.estacion || '',
        etoAcumulada, necesidadBrutaAcumulada, necesidadM3Ha: necesidadM3HaTotal,
                          ...
                          {predResultado && (
                            <div style={{ marginTop: 12 }}>
                              <div style={{ fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--muted)', marginBottom: 8 }}>
                                {predResultado.fechaInicio} → {predResultado.fechaFin} ({predResultado.numDias} días) · Estación: {predResultado.estacion}
                                ...
                                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 10, fontFamily: 'var(--mono)' }}>
                                  <thead>
                                    <tr style={{ position: 'sticky', top: 0, background: 'var(--surface2)' }}>
                                      {['Fecha', 'ETo', 'Kc', 'ETc', 'Nec. bruta', 'm³/ha'].map(h => (
                                  ...
                                  <tbody>
                                    {predResultado.tabla.map(fila => (
                                      <tr key={fila.fecha} style={{ borderBottom: '1px solid var(--border)' }}>
                                        ...
                                        <td style={{ padding: '5px 8px', color: 'var(--text)' }}>{fila.necesidadBruta !== null ? fila.necesidadBruta.toFixed(2) : '—'}</td>
