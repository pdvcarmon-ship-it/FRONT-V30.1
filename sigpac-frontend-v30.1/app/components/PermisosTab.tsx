...
// Fase 5.1: las fincas asignadas (personal.finca_ids) ya NO se editan aquí —
// se gestionan en el alta/edición de PERSONAL (una sola fuente de verdad).
// Esta pestaña solo gestiona el permiso "Restringir a solo las fincas
// asignadas" (checkbox normal, dentro del checklist de Parcelas).
...
export default function PermisosTab({ session, personal, setPersonal, isMobile }: Props) {
  ...
  const habilitarAcceso = async () => {
    ...
    if (!confirm(`Se habilita el acceso a la app para ${seleccionado.nombre}. La próxima vez que entre a KULTAGRO podrá crear su contraseña con el email de la empresa y su DNI (${seleccionado.dni}). ¿Continuar?`)) return
            ...
            <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, padding: 18 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16, padding: '10px 12px', borderRadius: 8, background: 'var(--surface2)', border: '1px solid var(--border)' }}>
                ...
                {seleccionado.estadoAcceso === 'activo' && (
                  <span style={{ fontSize: 11, color: 'var(--green)', textShadow: 'var(--green-outline)', fontFamily: 'var(--mono)' }}>✓ Acceso activo</span>
                )}
              </div>
              ...
              {guardadoOk && <div style={{ color: 'var(--green)', textShadow: 'var(--green-outline)', fontSize: 11, fontFamily: 'var(--mono)', marginBottom: 10 }}>✓ Guardado</div>}
