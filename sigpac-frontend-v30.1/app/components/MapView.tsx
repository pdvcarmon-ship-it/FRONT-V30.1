...
interface Props {
  ...
  piezasResaltadas?: { geojson: any; color: string }[] // trozos resultantes del corte, para elegir uno
}
...
export default function MapView({
  ...
  // Dibuja los trozos resultantes de un corte, cada uno con su color, para que el usuario elija
  useEffect(() => {
    const map = mapInstanceRef.current
    if (!map) return
    const L = require('leaflet')
    ...
    const grupo = L.featureGroup()
    piezasResaltadas.forEach(({ geojson, color }) => {
      if (!geojson) return
      L.geoJSON(geojson, { style: { color, weight: 2, fillColor: color, fillOpacity: 0.25 } }).addTo(grupo)
    })
    grupo.addTo(map)
