...
export default function MapView({
  ...
  const iniciarDibujoLinea = (map: any, L: any, drawnLayers: any) => {
    ...
    const drawHandler = new (L as any).Draw.Polyline(map, {
      shapeOptions: { color: '#ff6b6b', weight: 3 },
  ...
  const mostrarOpcionesCorte = (map: any, L: any, drawnLayers: any) => {
    const CorteControl = L.Control.extend({
      onAdd: () => {
        ...
        div.innerHTML = `
          <div style="font-family:'Space Mono',monospace;font-size:10px;color:#4a7a56;margin-bottom:4px;letter-spacing:0.06em;">LÍNEA DE CORTE DIBUJADA</div>
          <button id="btn-recortar" style="background:#ff6b6b;border:none;border-radius:5px;padding:6px 10px;color:#080c0a;font-family:'Space Mono',monospace;font-size:11px;font-weight:700;cursor:pointer;">✂ Recortar</button>
          <button id="btn-relinea" style="background:rgba(255,107,107,0.1);border:1px solid #ff6b6b;border-radius:5px;padding:6px 10px;color:#ff6b6b;font-family:'Space Mono',monospace;font-size:11px;cursor:pointer;">✕ Borrar y repetir</button>
        `
  ...
  useEffect(() => {
    ...
    puntosMarcador.forEach(({ nombre, lat, lng }) => {
      ...
      const marker = L.circleMarker([lat, lng], {
        radius: 8, color: '#4db8ff', weight: 3, fillColor: '#4db8ff', fillOpacity: 0.75,
      }).addTo(grupo)
