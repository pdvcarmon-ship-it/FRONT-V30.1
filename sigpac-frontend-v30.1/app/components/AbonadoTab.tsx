...
interface ParcelaLite { id: string; nombre: string; cultivo: string; fincaId?: string; referenciaSigpac?: string }
interface HistoricoCultivoLite { parcelaId: string; cultivo: string; campanaId?: string }
interface EquipoLite { id: string; tipo: string; subtipo?: string; nombre: string; titularidad?: string; fincaId?: string; nroRoma?: string }
interface PersonalLite { id: string; nombre: string; activo: boolean; permisos?: Record<string, boolean> }
        ...
        return (
          <div key={nu.clave}>
            ...
            <div style={{ fontSize: 8, color: 'var(--muted)', fontFamily: 'var(--mono)', marginTop: 2 }}>
              = {((valores[nu.clave] || 0) * factorDeUnidad(unidad, opciones)).toFixed(3)} {baseEtiqueta}
            </div>
          </div>
        )
      })}
...
export default function AbonadoTab({ session, fincas, misParcelas, campanas, historicoCultivos, equipos, personal, isMobile, misPermisos }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (pRes.data) setPlanes(pRes.data.map((pl: any) => ({
        ...
        abonoId: pl.abono_id || undefined, abonoNombre: pl.abono_nombre || '', abonoFabricante: pl.abono_fabricante || undefined, abonoTipo: pl.abono_tipo || '',
  ...
  const personalConPermiso = personal.filter(p => p.activo && p.permisos?.['abonado.crear_editar'])
  ...
  const guardarPlan = async () => {
    ...
    const nuevo: PlanAbonado = {
      ...
      abonoId: abonoUsadoId || undefined, abonoNombre: abonoNombreFinal, abonoFabricante: abonoFabricanteFinal || undefined, abonoTipo: abonoTipoFinal,
    ...
    try {
      const { error } = await supabase.from('planes_abonado').insert({
        ...
        abono_id: nuevo.abonoId || null, abono_nombre: nuevo.abonoNombre, abono_fabricante: nuevo.abonoFabricante || null, abono_tipo: nuevo.abonoTipo,
  ...
  const descargarFicha = async (pl: PlanAbonado) => {
    ...
    try {
      ...
      await descargarFichaAbonado({
        ...
        parcelas: [{ referencia: parcela?.referenciaSigpac || parcela?.nombre || '—' }],
        nutrientes: NUTRIENTES_LISTA.map(nu => ({
          etiqueta: nu.etiqueta,
          analisis: `${(pl.sueloInicial[nu.clave] || 0).toFixed(1)} kg/ha`,
          necesidades: `${(pl.necesidadesCultivo[nu.clave] || 0).toFixed(1)} kg/ha`,
          composicion: `${pl.abonoComposicion[nu.clave] || 0} ${etiquetaUnidad(pl.abonoUnidades[nu.clave], OPCIONES_CONCENTRACION)}`,
          aplicado: `${(aporte[nu.clave] || 0).toFixed(1)} kg/ha`,
        })),
        materiaOrganica: {
          analisis: pl.sueloInicial.materiaOrganica != null ? `${pl.sueloInicial.materiaOrganica} %` : '—',
          necesidades: '—', composicion: '—', aplicado: '—',
        },
        ph: pl.sueloInicial.ph != null ? String(pl.sueloInicial.ph) : undefined,
        ce: pl.sueloInicial.ce != null ? String(pl.sueloInicial.ce) : undefined,
        fecha: pl.fechaAplicacion || pl.fecha,
        maquinaria: equiposDelPlan.map(eq => `${eq.nombre}${eq.nroRoma ? ` (${eq.nroRoma})` : ''}`),
      }, `ficha-abonado-${pl.fecha}-${(parcela?.nombre || pl.parcelaId).replace(/\s+/g, '_')}.docx`)
    } catch (e) {
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
                  <div><div style={{ fontSize: 9, color: 'var(--muted)', fontFamily: 'var(--mono)' }}>Quién recomienda</div>
                    <select value={pRecomienda} onChange={e => setPRecomienda(e.target.value)} style={inputStyle()}>
                      ...
                      {personalConPermiso.map(p => <option key={p.id} value={p.id}>{p.nombre}</option>)}
                    </select></div>
                    ...
                    {NUTRIENTES_LISTA.map(nu => {
                      ...
                      return (
                        <tr key={nu.clave}>
                          ...
                          <td style={{ border: '1px solid var(--border)', padding: '4px 6px', textAlign: 'center', color: 'var(--text)' }}>{(planVer.sueloInicial[nu.clave] || 0).toFixed(1)}</td>
                          <td style={{ border: '1px solid var(--border)', padding: '4px 6px', textAlign: 'center', color: 'var(--text)' }}>{(planVer.necesidadesCultivo[nu.clave] || 0).toFixed(1)}</td>
                          <td style={{ border: '1px solid var(--border)', padding: '4px 6px', textAlign: 'center', color: 'var(--green)', fontWeight: 700 }}>{(aporte[nu.clave] || 0).toFixed(1)}</td>
                          <td style={{ border: '1px solid var(--border)', padding: '4px 6px', textAlign: 'center', color: 'var(--text)', fontWeight: 700 }}>{resultadoEsperado.toFixed(1)}</td>
                          <td style={{ border: '1px solid var(--border)', padding: '4px 6px', textAlign: 'center', color: 'var(--text)' }}>{(planVer.sueloObjetivo[nu.clave] || 0).toFixed(1)}</td>
                          <td style={{ border: '1px solid var(--border)', padding: '4px 6px', textAlign: 'center', color: balanceColor(planVer.balance[nu.clave] || 0), fontWeight: 700 }}>{(planVer.balance[nu.clave] || 0).toFixed(1)}</td>
                        </tr>
                      )
                    })}
                  </tbody>
