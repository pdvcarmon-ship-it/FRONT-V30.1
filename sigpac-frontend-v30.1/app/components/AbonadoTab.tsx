...
export default function AbonadoTab({ session, fincas, misParcelas, campanas, historicoCultivos, equipos, personal, isMobile, misPermisos, datosExplotacionPorFinca, onEditarDatosExplotacion }: Props) {
  ...
  const personalRecomendadores = personal.filter(p => p.activo && esRecomendador(p) && trabajaEnFinca(p, pFincaId))
  const personalAplicadores = personal.filter(p => p.activo && esAplicador(p) && trabajaEnFinca(p, pFincaId))
