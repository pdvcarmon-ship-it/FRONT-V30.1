...
interface ParcelaLite { id: string; nombre: string; cultivo: string; fincaId?: string; referenciaSigpac?: string; supHa?: number }
...
interface PersonalLite { id: string; nombre: string; activo: boolean; permisos?: Record<string, boolean>; nroRopo?: string }
  ...
  const descargarFicha = async (pl: PlanAbonado) => {
    ...
    try {
      ...
      await descargarFichaAbonado({
        ...
        parcelas: [{ referencia: parcela?.referenciaSigpac || parcela?.nombre || '—', superficieHa: parcela?.supHa }],
