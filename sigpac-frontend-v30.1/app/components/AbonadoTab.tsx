...
interface PersonalLite { id: string; nombre: string; activo: boolean; permisos?: Record<string, boolean>; nroRopo?: string; dni?: string; nivelCapacitacion?: string; funciones?: string[] }
...
export default function AbonadoTab({ session, fincas, misParcelas, campanas, historicoCultivos, equipos, personal, isMobile, misPermisos }: Props) {
  ...
  const [datosExplotacionPorFinca, setDatosExplotacionPorFinca] = useState<Record<string, DatosExplotacionLite>>({})
  ...
  useEffect(() => {
    ...
    (async () => {
      const [aRes, nRes, abRes, pRes, deRes] = await Promise.all([
        ...
        supabase.from('datos_explotacion').select('*').eq('user_id', session.user.id),
      ])
      if (deRes.data) {
        const mapa: Record<string, DatosExplotacionLite> = {}
        deRes.data.forEach((fila: any) => { if (fila.finca_id && fila.datos) mapa[fila.finca_id] = fila.datos })
  ...
  const [cuadFincaId, setCuadFincaId] = useState('')
  ...
  const generarCuaderno = async () => {
    if (!cuadFincaId || !cuadCampanaId) { alert('Selecciona finca y campaña'); return }
    setGenerandoCuaderno(true)
    try {
      const finca = fincas.find(f => f.id === cuadFincaId)
      const campana = campanas.find(c => c.id === cuadCampanaId)
      const de = datosExplotacionPorFinca[cuadFincaId] || {}
      ...
      const filas12 = personal.filter(p => p.activo).map((p, i) => [
        String(i + 1), p.nombre, p.dni || '', p.nroRopo || '', p.nivelCapacitacion || '',
        (p.funciones || []).some(f => f.toLowerCase().includes('asesor')) ? 'X' : '',
      ])
      const equiposDeFinca = equipos.filter(e => !e.fincaId || e.fincaId === cuadFincaId || e.titularidad === 'Externa')
      const filas13 = equiposDeFinca.filter(e => e.titularidad !== 'Externa').map((e, i) => [
        String(i + 1), `${e.tipo}${e.subtipo ? ' ' + e.subtipo : ''} — ${e.nombre}`, e.nroRoma || '', e.fechaAdquisicion || '', e.fechaUltimaInspeccion || '',
      ])
      const filas14 = equiposDeFinca.filter(e => e.titularidad === 'Externa').map((e, i) => [
      ...
      const parcelasDeFinca = misParcelas.filter(p => p.fincaId === cuadFincaId)
      const filasParcelas: string[][] = []
      parcelasDeFinca.forEach((p, ip) => {
      ...
      // --- Sección 3.1: planes de esta finca+campaña, agrupados por parcela ---
      const planesFiltrados = planes.filter(pl => pl.fincaId === cuadFincaId && pl.campanaId === cuadCampanaId)
      if (planesFiltrados.length === 0) { alert('No hay ningún abonado guardado para esa finca y campaña.'); setGenerandoCuaderno(false); return }
      ...
      await descargarCuadernoAbonado({
        ...
        cabecera: `${finca?.nombre || ''} — ${de.nombreRazonSocial || de.titularNombre || ''}`,
        datosGenerales, titular, seccionesPrevias, parcelas: parcelasFicha,
      }, `cuaderno-abonado-${finca?.nombre || 'finca'}-${campana?.nombre || 'campana'}.docx`.replace(/\s+/g, '_'))
    } catch (e) {
      ...
      {subTab === 'plan' && (
        <div>
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, padding: 14, marginBottom: 20 }}>
            <div style={{ fontFamily: 'var(--mono)', fontSize: 12, fontWeight: 700, color: 'var(--text)', marginBottom: 8 }}>📓 Cuaderno de Abonado (documento completo de la campaña)</div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto', gap: 8, alignItems: 'end' }}>
              <div><div style={{ fontSize: 9, color: 'var(--muted)', fontFamily: 'var(--mono)' }}>Finca</div>
                <select value={cuadFincaId} onChange={e => setCuadFincaId(e.target.value)} style={inputStyle()}>
                  <option value="">Selecciona...</option>{fincas.map(f => <option key={f.id} value={f.id}>{f.nombre}</option>)}
                </select></div>
              <div><div style={{ fontSize: 9, color: 'var(--muted)', fontFamily: 'var(--mono)' }}>Campaña</div>
              ...
              <button onClick={generarCuaderno} disabled={generandoCuaderno} style={{ padding: '8px 16px', borderRadius: 8, background: 'var(--green)', border: 'none', color: 'var(--bg)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 12, cursor: generandoCuaderno ? 'wait' : 'pointer', whiteSpace: 'nowrap' }}>
                {generandoCuaderno ? 'Generando...' : '⬇ Descargar cuaderno'}
              </button>
            </div>
          </div>
