...
interface Props {
  ...
  activo: boolean
  tareaAbonadoAAbrir: TareaAbonadoLite | null
  onConsumirTareaAbonado: () => void
  onPlanGuardado: (abonoPlanId: string, personalEjecutaId?: string) => void
}
...
export default function AbonadoTab({ session, fincas, misParcelas, campanas, historicoCultivos, equipos, personal, isMobile, misPermisos, datosExplotacionPorFinca, onEditarDatosExplotacion, activo, tareaAbonadoAAbrir, onConsumirTareaAbonado, onPlanGuardado }: Props) {
  ...
  const abrirFormPlan = () => { resetearFormularioPlan(); setPRealizado(true); setTareaOrigenId(null); setFormPlan(true) }

  // Abre el mismo asistente cuando se selecciona una tarea de tipo "Abonado"
  // desde la pestaña TAREAS — sin cambiar de pestaña (el modal es un overlay).
  useEffect(() => {
    if (!tareaAbonadoAAbrir) return
    resetearFormularioPlan()
    setTareaOrigenId(tareaAbonadoAAbrir.id || null)
    setPRealizado(true)
    const b = tareaAbonadoAAbrir.borradorAbonado
    if (b) {
      setPCampanaId(b.campanaId || ''); setPFincaId(b.fincaId || ''); setPParcelaId(b.parcelaId || '')
      setPModoObjetivo(b.modoObjetivo || 'mantener')
      if (b.sueloObjetivo) setPSueloObjetivo(b.sueloObjetivo)
      if (b.sueloObjetivoUnidades) setPSueloObjetivoUnidades(b.sueloObjetivoUnidades)
      setPAbonoId(b.abonoId || ''); setPDosis(b.dosisKgHa || 0)
      setPRecomienda(b.recomienda || ''); setPEjecuta(b.ejecuta || '')
      setPFechaAplicacion(b.fechaAplicacion || ''); setPEquipoIds(b.equipoIds || [])
      if (b.necNuevaDraft) setNecNuevaDraft(b.necNuevaDraft)
      if (b.necNuevaDraftUnidades) setNecNuevaDraftUnidades(b.necNuevaDraftUnidades)
      if (b.abonoNuevoNombre) setAbonoNuevoNombre(b.abonoNuevoNombre)
      if (b.abonoNuevoTipo) setAbonoNuevoTipo(b.abonoNuevoTipo)
      if (b.abonoNuevoNut) setAbonoNuevoNut(b.abonoNuevoNut)
      if (b.abonoNuevoUnidades) setAbonoNuevoUnidades(b.abonoNuevoUnidades)
    }
    setFormPlan(true)
  ...
  const [pRealizado, setPRealizado] = useState(true)
  const [tareaOrigenId, setTareaOrigenId] = useState<string | null>(null)
  ...
  const guardarPlan = async (forzarNoRealizado?: boolean) => {
    ...
    // "No realizado": no se calcula ni se guarda un plan de verdad todavía — se deja como
    // borrador dentro de una tarea (pendiente), igual que un tratamiento fitosanitario.
    if (!pRealizado || forzarNoRealizado) {
      const borrador = {
        campanaId: pCampanaId, fincaId: pFincaId, parcelaId: pParcelaId, cultivo: cultivoActual,
        modoObjetivo: pModoObjetivo, sueloObjetivo: pSueloObjetivo, sueloObjetivoUnidades: pSueloObjetivoUnidades,
        abonoId: pAbonoId, dosisKgHa: pDosis, recomienda: pRecomienda, ejecuta: pEjecuta,
        fechaAplicacion: pFechaAplicacion, equipoIds: pEquipoIds,
        necNuevaDraft, necNuevaDraftUnidades, abonoNuevoNombre, abonoNuevoTipo, abonoNuevoNut, abonoNuevoUnidades,
      }
      const idTarea = tareaOrigenId || String(Date.now())
      try {
        const { error } = await supabase.from('tareas').upsert({
          id: idTarea, user_id: session.user.id, tipo: 'Abonado', tipo_otro: null, estado: 'pendiente',
          campana_id: pCampanaId, finca_id: pFincaId, parcela_ids: [pParcelaId],
          creado_por: pRecomienda || null, ejecutor_id: pEjecuta || null,
          fecha_prevista: pFechaAplicacion || new Date().toISOString().slice(0, 10), equipo_ids: pEquipoIds, comentarios: '',
          borrador_abonado: borrador, abono_plan_id: null,
          fecha_registro: tareaOrigenId ? undefined : new Date().toLocaleDateString('es-ES'),
        })
        if (error) { setPError('No se pudo guardar la tarea: ' + error.message); return }
      } catch (e) { setPError('No se pudo guardar la tarea en la base de datos.'); return }
      resetearFormularioPlan(); setFormPlan(false); setTareaOrigenId(null)
      if (tareaAbonadoAAbrir) onConsumirTareaAbonado()
      return
    }
    ...
    // Si venía de una tarea pendiente, esa tarea queda vinculada y marcada como realizada.
    if (tareaOrigenId) {
      try { await supabase.from('tareas').update({ estado: 'realizada', abono_plan_id: nuevo.id, borrador_abonado: null }).eq('id', tareaOrigenId) } catch (e) { console.error(e) }
    } else {
      // Abonado creado directamente desde esta pestaña: también genera su propia tarea, ya realizada.
      try {
        await supabase.from('tareas').insert({
          id: String(Date.now() + 1), user_id: session.user.id, tipo: 'Abonado', tipo_otro: null, estado: 'realizada',
          campana_id: nuevo.campanaId, finca_id: nuevo.fincaId, parcela_ids: [nuevo.parcelaId],
          creado_por: nuevo.recomienda || null, ejecutor_id: nuevo.ejecuta || null,
          fecha_prevista: nuevo.fechaAplicacion || nuevo.fecha, equipo_ids: nuevo.equipoIds, comentarios: '',
          borrador_abonado: null, abono_plan_id: nuevo.id, fecha_registro: new Date().toLocaleDateString('es-ES'),
        })
    ...
    resetearFormularioPlan(); setFormPlan(false); setTareaOrigenId(null)
    if (tareaAbonadoAAbrir) onConsumirTareaAbonado()
    onPlanGuardado(nuevo.id, nuevo.ejecuta)
  }
  ...
  const asistentePlanJSX = (
          <ModalForm titulo="🧮 NUEVO ABONADO" onClose={() => { setFormPlan(false); setTareaOrigenId(null); if (tareaAbonadoAAbrir) onConsumirTareaAbonado() }}>
          ...
          <div style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 8, padding: 14, display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            {pParcelaId && !analisisVigente && (
              <div style={{ color: 'var(--red)', fontSize: 11, fontFamily: 'var(--mono)' }}>
                ⚠ Esta parcela no tiene un análisis de suelo vigente. Puedes guardarlo igualmente como tarea pendiente (sin balance todavía) y completarlo cuando tengas el análisis, o ve a "Análisis de suelo" y añade uno primero.
              </div>
            )}
          </div>

          {/* Sin análisis todavía: solo se puede dejar como tarea pendiente, sin el resto del cálculo */}
          {pParcelaId && !analisisVigente && (
            <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, padding: 14, marginBottom: 16 }}>
              <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
                ...
                <span style={{ fontSize: 12, fontFamily: 'var(--mono)', color: 'var(--muted)', fontWeight: 700 }}>¿Ya realizado? (no disponible sin análisis de suelo)</span>
              </label>
              {pError && <div style={{ color: 'var(--red)', fontSize: 11, fontFamily: 'var(--mono)', marginTop: 8 }}>{pError}</div>}
              {puedeEditar && (
                <button onClick={() => guardarPlan(true)} style={{ marginTop: 10, padding: '10px 20px', borderRadius: 8, background: 'var(--amber)', border: 'none', color: 'var(--bg)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 13, cursor: 'pointer', letterSpacing: '0.06em' }}>📋 GUARDAR COMO TAREA PENDIENTE</button>
              )}
            </div>
          )}
          ...
          {pParcelaId && analisisVigente && (
            <>
              ...
              <label style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10, cursor: 'pointer' }}>
                <input type="checkbox" checked={pRealizado} onChange={e => setPRealizado(e.target.checked)} />
                <span style={{ fontSize: 12, fontFamily: 'var(--mono)', color: 'var(--text)', fontWeight: 700 }}>¿Ya realizado?</span>
              </label>
              {!pRealizado && (
                <div style={{ fontSize: 10, color: 'var(--amber)', fontFamily: 'var(--mono)', marginBottom: 10 }}>
                  Se guardará como tarea pendiente en TAREAS, sin calcular el balance todavía. Podrás completarlo más tarde marcando la tarea como realizada.
                </div>
              )}
              {puedeEditar && (
                <button onClick={() => guardarPlan()} style={{ padding: '10px 20px', borderRadius: 8, background: 'var(--green)', border: 'none', color: 'var(--bg)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 13, cursor: 'pointer', letterSpacing: '0.06em' }}>
                  {pRealizado ? '💾 GUARDAR PLAN DE ABONADO' : '📋 GUARDAR COMO TAREA PENDIENTE'}
                </button>
              )}
            </>
          )}
    ...
    <div style={{ flex: 1, height: '100%', overflow: 'auto', padding: isMobile ? 12 : 24, background: 'var(--bg)' }}>
      ...
      {subTab === 'plan' && (
        <div>
          ...
          {/* El asistente se puede abrir aquí, o desde una tarea de tipo "Abonado" en TAREAS
              (ver {formPlan && asistentePlanJSX} más abajo, fuera de esta subpestaña) */}
