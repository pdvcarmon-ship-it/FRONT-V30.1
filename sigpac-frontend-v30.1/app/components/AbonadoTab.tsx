    ...
    try {
      ...
      await descargarFichaAbonado({
        ...
        nutrientes: NUTRIENTES_LISTA.map(nu => ({
          ...
          aplicado: `${(pl.aporteReal[nu.clave] || 0).toFixed(1)} kg/ha`,
        })),
        materiaOrganica: {
          ...
          {planVer && (
            <ModalForm titulo={`🧮 ABONADO — ${planVer.fecha}`} onClose={() => setPlanVer(null)}>
              <div style={{ fontSize: 11, color: 'var(--text)', fontFamily: 'var(--mono)', lineHeight: 1.8 }}>
                ...
                <div><b>Fecha de aplicación:</b> {planVer.fechaAplicacion || '—'} &nbsp; <b>Maquinaria:</b> {planVer.equipoIds.map(id => equipos.find(e => e.id === id)?.nombre).filter(Boolean).join(', ') || '—'}</div>
              </div>
              <div style={{ fontSize: 10, color: 'var(--muted)', fontFamily: 'var(--mono)', marginTop: 8 }}>Balance por nutriente (kg/ha; P y K en equivalente óxido)</div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(68px,1fr))', gap: 6 }}>
                {NUTRIENTES_LISTA.map(nu => (
                  <div key={nu.clave} style={{ textAlign: 'center', background: 'var(--surface2)', borderRadius: 5, padding: '6px 4px' }}>
                    <div style={{ fontSize: 9, color: 'var(--muted)', fontFamily: 'var(--mono)' }}>{nu.etiqueta}</div>
                    <div style={{ fontSize: 12, fontWeight: 700, fontFamily: 'var(--mono)', color: balanceColor(planVer.balance[nu.clave] || 0) }}>{(planVer.balance[nu.clave] || 0).toFixed(1)}</div>
                  </div>
                ))}
              </div>
