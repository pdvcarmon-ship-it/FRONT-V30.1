...
// Unidad en la que se declara cada nutriente del ABONO (composición garantizada).
// Todas se normalizan a "gramos de nutriente por kg de producto" para poder
// calcular el aporte real con independencia de en qué unidad lo hayas metido.
type UnidadNutriente = 'pct' | 'g_kg' | 'mg_kg'
const FACTOR_A_G_KG: Record<UnidadNutriente, number> = { pct: 10, g_kg: 1, mg_kg: 0.001 }
const ETIQUETA_UNIDAD: Record<UnidadNutriente, string> = { pct: '%', g_kg: 'g/kg', mg_kg: 'mg/kg' }
const gramosPorKg = (valor: number | undefined, unidad: UnidadNutriente): number => (valor || 0) * FACTOR_A_G_KG[unidad]
const UNIDADES_VACIO: Partial<Record<keyof Nutrientes, UnidadNutriente>> = {}
...
interface AbonoCatalogo {
  ...
  unidades: Partial<Record<keyof Nutrientes, UnidadNutriente>>
  pBase: BaseNutriente
  kBase: BaseNutriente
}
...
interface PlanAbonado {
  ...
  abonoUnidades: Partial<Record<keyof Nutrientes, UnidadNutriente>>
  dosisKgHa: number
  balance: Nutrientes
  recomienda?: string
  ejecuta?: string
...
function FilaNutrientesAbono({ valores, unidades, onChangeValor, onChangeUnidad, soloLectura }: {
  valores: Nutrientes
  unidades: Partial<Record<keyof Nutrientes, UnidadNutriente>>
  onChangeValor?: (n: Nutrientes) => void
  onChangeUnidad?: (u: Partial<Record<keyof Nutrientes, UnidadNutriente>>) => void
  soloLectura?: boolean
}) {
  return (
          ...
          <div key={nu.clave}>
            ...
            <div style={{ display: 'flex', gap: 3 }}>
              ...
              <select disabled={soloLectura} value={unidad}
                onChange={e => onChangeUnidad && onChangeUnidad({ ...unidades, [nu.clave]: e.target.value as UnidadNutriente })}
                style={{ ...inputStyle(soloLectura), width: '42%', padding: '6px 1px', fontSize: 9 }}>
                <option value="pct">%</option><option value="g_kg">g/kg</option><option value="mg_kg">mg/kg</option>
              </select>
            </div>
            <div style={{ fontSize: 8, color: 'var(--muted)', fontFamily: 'var(--mono)', marginTop: 2 }}>
              = {gramosPorKg(valores[nu.clave], unidad).toFixed(2)} g/kg de producto
            </div>
          </div>
        )
      })}
...
export default function AbonadoTab({ session, fincas, misParcelas, campanas, historicoCultivos, equipos, personal, isMobile, misPermisos }: Props) {
  ...
  const guardarNecesidad = async (override?: { cultivo: string; nutrientes: Nutrientes }): Promise<NecesidadCultivo | null> => {
    ...
    const nuevo: NecesidadCultivo = {
      ...
      nutrientes, pBase: override ? 'elemental' : nPBase, kBase: override ? 'elemental' : nKBase,
    }
  ...
  const [abUnidades, setAbUnidades] = useState<Partial<Record<keyof Nutrientes, UnidadNutriente>>>({ ...UNIDADES_VACIO })
  ...
  const guardarAbono = async (override?: { nombre: string; tipo: 'organico' | 'inorganico'; nutrientes: Nutrientes; unidades: Partial<Record<keyof Nutrientes, UnidadNutriente>> }): Promise<AbonoCatalogo | null> => {
  ...
  const [abonoNuevoUnidades, setAbonoNuevoUnidades] = useState<Partial<Record<keyof Nutrientes, UnidadNutriente>>>({ ...UNIDADES_VACIO })
  ...
  const composicionAbonoActiva: Nutrientes = pAbonoId
    ...
    : abonoNuevoNut
  const composicionAbonoUnidades: Partial<Record<keyof Nutrientes, UnidadNutriente>> = pAbonoId
    ? (abonoSel?.unidades || {})
    : abonoNuevoUnidades
  ...
  const aporteAbono: Nutrientes = Object.fromEntries(
    NUTRIENTES_LISTA.map(nu => {
      ...
      const gKg = gramosPorKg(composicionAbonoActiva[nu.clave], unidad)
  ...
  const resetearFormularioPlan = () => {
    ...
    setNecNuevaDraft({ ...NUTRIENTES_VACIO }); setAbonoNuevoNombre(''); setAbonoNuevoTipo(''); setAbonoNuevoNut({ ...NUTRIENTES_VACIO }); setAbonoNuevoUnidades({ ...UNIDADES_VACIO })
    setPError('')
  }
  ...
  const guardarPlan = async () => {
    ...
    if (!necesidadExistente) {
      ...
      const creada = await guardarNecesidad({ cultivo: cultivoActual, nutrientes: necNuevaDraft })
    ...
    const nuevo: PlanAbonado = {
      ...
      abonoComposicion: composicionAbonoActiva, abonoUnidades: composicionAbonoUnidades, dosisKgHa: pDosis, balance,
    ...
    try {
      const { error } = await supabase.from('planes_abonado').insert({
        ...
        abono_composicion: nuevo.abonoComposicion, abono_unidades: nuevo.abonoUnidades, dosis_kg_ha: nuevo.dosisKgHa, balance: nuevo.balance,
        ...
        <div>
          ...
          {formAbono && (
            <ModalForm titulo={abonoEditar ? '✏ EDITAR ABONO' : '📦 NUEVO ABONO'} onClose={() => { setFormAbono(false); setAbonoEditar(null) }}>
              ...
              <FilaNutrientesAbono valores={abNut} unidades={abUnidades} onChangeValor={setAbNut} onChangeUnidad={setAbUnidades} />
              ...
              <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, padding: 14, marginBottom: 16 }}>
                ...
                {!pAbonoId && (
                  <div style={{ border: '1px dashed var(--border)', borderRadius: 6, padding: 8, marginBottom: 8 }}>
                    ...
                    <FilaNutrientesAbono valores={abonoNuevoNut} unidades={abonoNuevoUnidades} onChangeValor={setAbonoNuevoNut} onChangeUnidad={setAbonoNuevoUnidades} />
                  </div>
                )}
