...
interface EquipoLite { id: string; tipo: string; nombre: string }
...
export default function AbonadoTab({ session, fincas, misParcelas, campanas, historicoCultivos, equipos, isMobile, misPermisos }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (pRes.data) setPlanes(pRes.data.map((pl: any) => ({
        ...
        equipoId: pl.equipo_id || undefined, maquinariaExterna: pl.maquinaria_externa || undefined,
      })))
      setCargado(true)
    })()
  }, [session, cargado])
  ...
  const [pEquipoId, setPEquipoId] = useState('')
  const [pMaquinariaExterna, setPMaquinariaExterna] = useState('')
  ...
  const composicionAbonoActiva: Nutrientes = pAbonoId
    ? (abonoSel ? normalizarPK(abonoSel.nutrientes, abonoSel.pBase, abonoSel.kBase) : { ...NUTRIENTES_VACIO })
    : normalizarPK(abonoNuevoNut, 'elemental', 'elemental')
  ...
  const balance: Nutrientes = Object.fromEntries(
    NUTRIENTES_LISTA.map(nu => [nu.clave,
      (sueloInicialNorm[nu.clave] || 0) + (aporteAbono[nu.clave] || 0) - (necesidadesNorm[nu.clave] || 0) - (sueloObjetivoNorm[nu.clave] || 0)
    ])
  ) as Nutrientes
  ...
  const resetearFormularioPlan = () => {
    ...
    setPRecomienda(''); setPEjecuta(''); setPFechaAplicacion(''); setPEquipoId(''); setPMaquinariaExterna('')
  ...
  const guardarPlan = async () => {
    ...
    const nuevo: PlanAbonado = {
      ...
      fechaAplicacion: pFechaAplicacion || undefined, equipoId: pEquipoId || undefined, maquinariaExterna: pMaquinariaExterna.trim() || undefined,
    }
    ...
    try {
      const { error } = await supabase.from('planes_abonado').insert({
        ...
        equipo_id: nuevo.equipoId || null, maquinaria_externa: nuevo.maquinariaExterna || null,
      })
              ...
              <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, padding: 14, marginBottom: 16, display: 'flex', flexDirection: 'column', gap: 8 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
                  <div><div style={{ fontSize: 9, color: 'var(--muted)', fontFamily: 'var(--mono)' }}>Quién recomienda</div>
                    <input value={pRecomienda} onChange={e => setPRecomienda(e.target.value)} style={inputStyle()} /></div>
                  <div><div style={{ fontSize: 9, color: 'var(--muted)', fontFamily: 'var(--mono)' }}>Quién ejecuta</div>
                    <input value={pEjecuta} onChange={e => setPEjecuta(e.target.value)} style={inputStyle()} /></div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                  <div><div style={{ fontSize: 9, color: 'var(--muted)', fontFamily: 'var(--mono)' }}>Maquinaria (catálogo de Equipos)</div>
                    <select value={pEquipoId} onChange={e => setPEquipoId(e.target.value)} style={inputStyle()}>
                      <option value="">— Ninguna / externa —</option>
                      {equipos.map(eq => <option key={eq.id} value={eq.id}>{eq.tipo} · {eq.nombre}</option>)}
                    </select></div>
                  <div><div style={{ fontSize: 9, color: 'var(--muted)', fontFamily: 'var(--mono)' }}>Maquinaria externa/alquilada (si no está en el catálogo)</div>
                    <input value={pMaquinariaExterna} onChange={e => setPMaquinariaExterna(e.target.value)} disabled={!!pEquipoId} style={inputStyle(!!pEquipoId)} /></div>
                </div>
              </div>
