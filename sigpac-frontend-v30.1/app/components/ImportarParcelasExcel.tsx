...
//   con varios recintos SIGPAC (una fila por cada sigpac_ref).
...
type Fase = 'subida' | 'procesando' | 'preview' | 'importando' | 'hecho'

const COLUMNAS_ESPERADAS = ['grupo', 'nombre_parcela', 'sigpac_ref', 'cultivo', 'fecha_plantacion', 'finca', 'observaciones']
...
export default function ImportarParcelasExcel({ session, fincas, setFincas, setMisParcelas, onClose }: Props) {
  ...
  const [resumenFinal, setResumenFinal] = useState({ parcelas: 0, fincas: 0 })
  ...
  const descargarPlantilla = useCallback(async () => {
    ...
    const filaEjemplo1 = {
      ...
      observaciones: 'Riego por goteo',
    }
    const filaEjemplo2 = {
      grupo: '1', nombre_parcela: '', sigpac_ref: '41/7/0/0/6/8/1',
      cultivo: '', fecha_plantacion: '', finca: '', observaciones: '',
    }
    const filaEjemplo3 = {
    ...
    ws['!cols'] = [{ wch: 8 }, { wch: 20 }, { wch: 20 }, { wch: 14 }, { wch: 16 }, { wch: 18 }, { wch: 30 }]
  ...
  const procesarArchivo = useCallback(async (file: File) => {
    ...
    try {
      ...
      const filasRaw: any[] = XLSX.utils.sheet_to_json(ws, { defval: '', raw: false })
      ...
      filas.filter(f => !f.grupo).forEach(f => {
        erroresLocal.push({ grupo: '?', fila: f._fila, motivo: 'Fila sin valor en la columna "grupo".' })
      })
      ...
      grupos.forEach((filasGrupo, grupo) => {
        ...
        const maestra = filasGrupo.find(f => f.nombre_parcela || f.cultivo || f.finca || f.observaciones) || filasGrupo[0]
  ...
  const confirmarImportacion = async () => {
    ...
    try {
      ...
      for (let i = 0; i < parcelasOk.length; i++) {
        ...
        const { fusionado } = construirEstadoDesdeFeatures(p.features)
        const { info, supTotal } = construirEstadoDesdeFeatures(p.features)
      ...
      setResumenFinal({ parcelas: parcelasCreadas.length, fincas: fincasCreadas.length })
      setFase('hecho')
    } catch (e: any) {
          ...
          {fase === 'subida' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div style={{ fontSize: 13, color: 'var(--text)', lineHeight: 1.6 }}>
                Sube un Excel con las columnas <code>grupo, nombre_parcela, sigpac_ref, cultivo, fecha_plantacion, finca, observaciones</code>.
              ...
              {parcelasOk.map((p, i) => (
                <div key={i} style={{ padding: '10px 14px', borderRadius: 8, background: 'var(--green-dim)', border: '1px solid rgba(61,220,110,0.2)', fontSize: 12 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontWeight: 700, color: 'var(--text)', marginBottom: 4 }}>
                    <span>{p.nombre || `(grupo ${p.grupo})`} — {p.cultivo || 'sin cultivo'}</span>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                    {errores.map((e, i) => (
                      <div key={i} style={{ fontSize: 11, color: '#fca5a5', fontFamily: 'var(--mono)' }}>
                        Fila {e.fila} (grupo {e.grupo}): {e.motivo}
                      </div>
                    ))}
                  </div>
                </div>
        ...
        <div style={{ padding: '14px 20px', borderTop: '1px solid var(--border)', display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
          {fase === 'preview' && (
            <>
              ...
              <button onClick={confirmarImportacion} disabled={parcelasOk.length === 0} style={{ ...botonPrimario, opacity: parcelasOk.length === 0 ? 0.5 : 1 }}>
