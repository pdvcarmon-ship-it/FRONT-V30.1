...
// explotación"), 1.2, 1.3, 1.4 (parcial), 2.1, 3.1, 3.1 bis (parcial, ver más abajo), 3.2.
// Secciones sin fuente de datos en la app (se generan en blanco, igual que el
// original, para rellenar a mano): 2.2 (sin ubicaciones), 3.3, 3.4, 3.5, 4 y 5.
...
export interface SeccionTabla {
  ...
  // Fila opcional de cabeceras agrupadas (celdas combinadas) por encima de `headers`, p.ej. para
  // agrupar "Especie"/"Variedad" bajo "CULTIVO". La suma de `span` debe ser igual a headers.length.
...
const BORDE = 'A6A6A6'
const VERDE_MARCA = '2FA85A' // versión ligeramente más oscura del verde de la app (3DDC6E), para que se lea bien sobre fondo blanco
...
export const generarCuadernoCampoDocx = async (data: CuadernoCampoData): Promise<Blob> => {
  ...
  const tablaSeccion = (s: SeccionTabla) => {
    ...
    // Fila opcional de cabeceras agrupadas (celdas combinadas), por encima de la fila de
    // cabeceras de columna — p.ej. "CULTIVO" combinada sobre "Especie"/"Variedad".
    ...
    const tabla = new Table({
      ...
      columnWidths: anchos,
      rows: [filaTitulo, ...(filaGrupos ? [filaGrupos] : []), filaCabecera, ...filasCuerpo],
    })
  ...
  // Logotipo "KULTAGRO" como texto con color por letra (K y A en verde de marca, resto en gris oscuro),
  // igual que en la cabecera de la app. `size` en semipuntos (docx), p.ej. 24 = 12pt.
  ...
  children.push(new Paragraph({
    alignment: AlignmentType.CENTER,
    spacing: { after: 120 },
    children: logoRuns(36),
  }))
  children.push(parrafoTitulo('CUADERNO DE CAMPO'))
  children.push(new Paragraph({
    spacing: { after: 200 },
    children: [
      ...
      footers: {
        default: new Footer({
          children: [new Paragraph({
            ...
            children: [new TextRun({ text: 'Cuaderno de Campo — generado automáticamente', size: 11, italics: true, color: '808080', font: 'Arial' })],
          })],
        }),
      },
      children,
