...
export default function LoginScreen({ onLogin }: Props) {
  ...
  const inputStyle: React.CSSProperties = { width: '100%', background: '#000000', border: '1px solid var(--border)', borderRadius: 6, padding: '10px 12px', color: '#FFFFFF', fontSize: 14, fontFamily: 'var(--sans)', outline: 'none' }
    ...
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100vh', background: 'var(--bg)' }}>
      <div style={{ width: '100%', maxWidth: 380, padding: 32, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12 }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          {info && <div style={{ padding: '8px 12px', borderRadius: 6, background: 'rgba(61,220,110,0.08)', border: '1px solid rgba(61,220,110,0.2)', color: 'var(--green)', textShadow: 'var(--green-outline)', fontSize: 12, fontFamily: 'var(--mono)' }}>✓ {info}</div>}

          <button
            type="submit"
            disabled={cargando}
            style={{ padding: '12px', borderRadius: 8, background: 'var(--green)', border: 'none', color: 'var(--text)', fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 13, cursor: cargando ? 'wait' : 'pointer', letterSpacing: '0.06em' }}
          >
